
var Sys_Address = [{
		name: "北京",
		city: [{
			name: "北京市",
			district: [{
				name: "东城区",
				zip: "100010"
			}, {
				name: "西城区",
				zip: "100032"
			}, {
				name: "崇文区",
				zip: "100061"
			}, {
				name: "宣武区",
				zip: "100054"
			}, {
				name: "朝阳区",
				zip: "100011"
			}, {
				name: "丰台区",
				zip: "100071"
			}, {
				name: "石景山区",
				zip: "100071"
			}, {
				name: "海淀区",
				zip: "100091"
			}, {
				name: "昌平区",
				zip: "102200"
			}, {
				name: "延庆区",
				zip: "102100"
			}, {
				name: "密云区",
				zip: "101500"
			}, {
				name: "平谷区",
				zip: "101200"
			}, {
				name: "怀柔区",
				zip: "101400"
			}, {
				name: "顺义区",
				zip: "101300"
			}, {
				name: "通州区",
				zip: "101100"
			}, {
				name: "大兴区",
				zip: "102600"
			}, {
				name: "房山区",
				zip: "102488"
			}, {
				name: "门头沟区",
				zip: "102300"
			}]
		}]
	}, {
		name: "天津",
		city: [{
			name: "天津市",
			district: [{
				name: "静海区",
				zip: "301600"
			}, {
				name: "宁河区",
				zip: "301500"
			}, {
				name: "和平区",
				zip: "300041"
			}, {
				name: "河东区",
				zip: "300171"
			}, {
				name: "河西区",
				zip: "300202"
			}, {
				name: "汉沽区",
				zip: "300480"
			}, {
				name: "大港区",
				zip: "300270"
			}, {
				name: "东丽区",
				zip: "300300"
			}, {
				name: "西青区",
				zip: "300380"
			}, {
				name: "南开区",
				zip: "300100"
			}, {
				name: "河北区",
				zip: "300143"
			}, {
				name: "红桥区",
				zip: "300131"
			}, {
				name: "塘沽区",
				zip: "300450"
			}, {
				name: "滨海新区",
				zip: "300457"
			}, {
				name: "北辰区",
				zip: "300400"
			}, {
				name: "津南区",
				zip: "300350"
			}, {
				name: "宝坻区",
				zip: "301800"
			}, {
				name: "武清区",
				zip: "301700"
			}, {
				name: "蓟县",
				zip: "301900"
			}]
		}]
	}, {
		name: "上海",
		city: [{
			name: "上海市",
			district: [{
				name: "川沙区",
				zip: ""
			}, {
				name: "崇明区",
				zip: "202150"
			}, {
				name: "闸北区",
				zip: "200070"
			}, {
				name: "虹口区",
				zip: "200080"
			}, {
				name: "杨浦区",
				zip: "200082"
			}, {
				name: "徐汇区",
				zip: "200030"
			}, {
				name: "长宁区",
				zip: "200050"
			}, {
				name: "静安区",
				zip: "200040"
			}, {
				name: "普陀区",
				zip: "200062"
			}, {
				name: "黄浦区",
				zip: "200001"
			}, {
				name: "卢湾区",
				zip: "200020"
			}, {
				name: "奉贤区",
				zip: "201400"
			}, {
				name: "南汇区",
				zip: "201300"
			}, {
				name: "青浦区",
				zip: "201700"
			}, {
				name: "松江区",
				zip: "201600"
			}, {
				name: "金山区",
				zip: "201540"
			}, {
				name: "浦东新区",
				zip: "200120"
			}, {
				name: "嘉定区",
				zip: "201800"
			}, {
				name: "宝山区",
				zip: "201900"
			}, {
				name: "闵行区",
				zip: "201100"
			}]
		}]
	}, {
		name: "重庆",
		city: [{
			name: "重庆市",
			district: [{
				name: "涪陵区",
				zip: "408000"
			}, {
				name: "渝中区",
				zip: "400012"
			}, {
				name: "万州区",
				zip: "404000"
			}, {
				name: "万盛区",
				zip: "400800"
			}, {
				name: "双桥区",
				zip: "400900"
			}, {
				name: "南岸区",
				zip: "400060"
			}, {
				name: "北碚区",
				zip: "400700"
			}, {
				name: "沙坪坝区",
				zip: "400020"
			}, {
				name: "九龙坡区",
				zip: "400039"
			}, {
				name: "大渡口区",
				zip: "400084"
			}, {
				name: "江北区",
				zip: "400021"
			}, {
				name: "长寿区",
				zip: "401220"
			}, {
				name: "黔江区",
				zip: "409700"
			}, {
				name: "巴南区",
				zip: "401320"
			}, {
				name: "渝北区",
				zip: "401120"
			}, {
				name: "潼南县",
				zip: "402660"
			}, {
				name: "綦江区",
				zip: "401420"
			}, {
				name: "石柱土家族自治县",
				zip: "409100"
			}, {
				name: "秀山土家族苗族自治县",
				zip: "409900"
			}, {
				name: "酉阳土家族苗族自治县",
				zip: "409800"
			}, {
				name: "彭水苗族土家族自治县",
				zip: "409600"
			}, {
				name: "忠县",
				zip: "404300"
			}, {
				name: "武隆县",
				zip: "408500"
			}, {
				name: "云阳县",
				zip: "404500"
			}, {
				name: "开县",
				zip: "405400"
			}, {
				name: "巫山县",
				zip: "404700"
			}, {
				name: "奉节县",
				zip: "404600"
			}, {
				name: "巫溪县",
				zip: "405800"
			}, {
				name: "大足区",
				zip: "402360"
			}, {
				name: "铜梁县",
				zip: "402560"
			}, {
				name: "璧山县",
				zip: "402760"
			}, {
				name: "荣昌县",
				zip: "402460"
			}, {
				name: "城口县",
				zip: "405900"
			}, {
				name: "梁平县",
				zip: "405200"
			}, {
				name: "垫江县",
				zip: "408300"
			}, {
				name: "丰都县",
				zip: "408200"
			}, {
				name: "江津区",
				zip: "402260"
			}, {
				name: "永川区",
				zip: "402160"
			}, {
				name: "合川区",
				zip: "401520"
			}, {
				name: "南川区",
				zip: "408400"
			}]
		}]
	}, {
		name: "河北",
		city: [{
			name: "衡水市",
			district: [{
				name: "枣强县",
				zip: "053100"
			}, {
				name: "武强县",
				zip: "053300"
			}, {
				name: "武邑县",
				zip: "053400"
			}, {
				name: "安平县",
				zip: "053600"
			}, {
				name: "饶阳县",
				zip: "053900"
			}, {
				name: "景县",
				zip: "053500"
			}, {
				name: "故城县",
				zip: "253800"
			}, {
				name: "阜城县",
				zip: "053700"
			}, {
				name: "桃城区",
				zip: "053000"
			}, {
				name: "冀州市",
				zip: "053200"
			}, {
				name: "深州市",
				zip: "053800"
			}]
		}, {
			name: "石家庄市",
			district: [{
				name: "桥东区",
				zip: "050011"
			}, {
				name: "长安区",
				zip: "050011"
			}, {
				name: "裕华区",
				zip: "050081"
			}, {
				name: "新华区",
				zip: "050051"
			}, {
				name: "桥西区",
				zip: "050051"
			}, {
				name: "井陉矿区",
				zip: "050100"
			}, {
				name: "正定县",
				zip: "050800"
			}, {
				name: "井陉县",
				zip: "050300"
			}, {
				name: "灵寿县",
				zip: "050500"
			}, {
				name: "高邑县",
				zip: "051330"
			}, {
				name: "栾城县",
				zip: "051430"
			}, {
				name: "行唐县",
				zip: "050600"
			}, {
				name: "平山县",
				zip: "050400"
			}, {
				name: "无极县",
				zip: "052460"
			}, {
				name: "赞皇县",
				zip: "051230"
			}, {
				name: "深泽县",
				zip: "052560"
			}, {
				name: "赵县",
				zip: "051530"
			}, {
				name: "元氏县",
				zip: "051130"
			}, {
				name: "鹿泉市",
				zip: "050200"
			}, {
				name: "新乐市",
				zip: "050700"
			}, {
				name: "晋州市",
				zip: "052260"
			}, {
				name: "藁城市",
				zip: "052160"
			}, {
				name: "辛集市",
				zip: "052360"
			}]
		}, {
			name: "唐山市",
			district: [{
				name: "滦县",
				zip: "063700"
			}, {
				name: "丰润区",
				zip: "064000"
			}, {
				name: "玉田县",
				zip: "064100"
			}, {
				name: "曹妃甸区",
				zip: "063200"
			}, {
				name: "滦南县",
				zip: "063500"
			}, {
				name: "乐亭县",
				zip: "063600"
			}, {
				name: "迁西县",
				zip: "064300"
			}, {
				name: "丰南区",
				zip: "063300"
			}, {
				name: "古冶区",
				zip: "063104"
			}, {
				name: "开平区",
				zip: "063021"
			}, {
				name: "路南区",
				zip: "063017"
			}, {
				name: "路北区",
				zip: "063015"
			}, {
				name: "遵化市",
				zip: "064200"
			}, {
				name: "迁安市",
				zip: "064400"
			}]
		}, {
			name: "秦皇岛市",
			district: [{
				name: "海港区",
				zip: "066000"
			}, {
				name: "山海关区",
				zip: "066200"
			}, {
				name: "卢龙县",
				zip: "066400"
			}, {
				name: "青龙满族自治县",
				zip: "066500"
			}, {
				name: "抚宁县",
				zip: "066300"
			}, {
				name: "昌黎县",
				zip: "066600"
			}, {
				name: "北戴河区",
				zip: "066100"
			}]
		}, {
			name: "邯郸市",
			district: [{
				name: "临漳县",
				zip: "056600"
			}, {
				name: "邯郸县",
				zip: "056105"
			}, {
				name: "磁县",
				zip: "056500"
			}, {
				name: "涉县",
				zip: "056400"
			}, {
				name: "大名县",
				zip: "056900"
			}, {
				name: "成安县",
				zip: "056700"
			}, {
				name: "鸡泽县",
				zip: "057350"
			}, {
				name: "邱县",
				zip: "057450"
			}, {
				name: "永年县",
				zip: "057150"
			}, {
				name: "肥乡县",
				zip: "057550"
			}, {
				name: "邯山区",
				zip: "056001"
			}, {
				name: "丛台区",
				zip: "056004"
			}, {
				name: "峰峰矿区",
				zip: "056200"
			}, {
				name: "复兴区",
				zip: "056003"
			}, {
				name: "武安市",
				zip: "056300"
			}, {
				name: "馆陶县",
				zip: "057750"
			}, {
				name: "广平县",
				zip: "057650"
			}, {
				name: "曲周县",
				zip: "057250"
			}, {
				name: "魏县",
				zip: "056800"
			}]
		}, {
			name: "邢台市",
			district: [{
				name: "广宗县",
				zip: "054600"
			}, {
				name: "新河县",
				zip: "055650"
			}, {
				name: "巨鹿县",
				zip: "055250"
			}, {
				name: "宁晋县",
				zip: "055550"
			}, {
				name: "临西县",
				zip: "054900"
			}, {
				name: "清河县",
				zip: "054800"
			}, {
				name: "威县",
				zip: "054700"
			}, {
				name: "平乡县",
				zip: "054500"
			}, {
				name: "邢台县",
				zip: "054001"
			}, {
				name: "临城县",
				zip: "054300"
			}, {
				name: "内丘县",
				zip: "054200"
			}, {
				name: "柏乡县",
				zip: "055450"
			}, {
				name: "隆尧县",
				zip: "055350"
			}, {
				name: "任县",
				zip: "055150"
			}, {
				name: "南和县",
				zip: "054400"
			}, {
				name: "桥西区",
				zip: "054000"
			}, {
				name: "桥东区",
				zip: "054001"
			}, {
				name: "沙河市",
				zip: "054100"
			}, {
				name: "南宫市",
				zip: "055750"
			}]
		}, {
			name: "保定市",
			district: [{
				name: "新市区",
				zip: "071052"
			}, {
				name: "北市区",
				zip: "071000"
			}, {
				name: "南市区",
				zip: "071000"
			}, {
				name: "满城县",
				zip: "072150"
			}, {
				name: "涞水县",
				zip: "074100"
			}, {
				name: "清苑县",
				zip: "071100"
			}, {
				name: "涞源县",
				zip: "074300"
			}, {
				name: "望都县",
				zip: "072450"
			}, {
				name: "高阳县",
				zip: "071500"
			}, {
				name: "容城县",
				zip: "071700"
			}, {
				name: "定兴县",
				zip: "072650"
			}, {
				name: "唐县",
				zip: "072350"
			}, {
				name: "阜平县",
				zip: "073200"
			}, {
				name: "徐水县",
				zip: "072550"
			}, {
				name: "雄县",
				zip: "071800"
			}, {
				name: "顺平县",
				zip: "072250"
			}, {
				name: "博野县",
				zip: "071300"
			}, {
				name: "曲阳县",
				zip: "073100"
			}, {
				name: "蠡县",
				zip: "071400"
			}, {
				name: "安新县",
				zip: "071600"
			}, {
				name: "易县",
				zip: "074200"
			}, {
				name: "高碑店市",
				zip: "074000"
			}, {
				name: "涿州市",
				zip: "072750"
			}, {
				name: "安国市",
				zip: "071200"
			}, {
				name: "定州市",
				zip: "073000"
			}]
		}, {
			name: "张家口市",
			district: [{
				name: "桥西区",
				zip: "075061"
			}, {
				name: "桥东区",
				zip: "075000"
			}, {
				name: "下花园区",
				zip: "075300"
			}, {
				name: "宣化区",
				zip: "075100"
			}, {
				name: "万全县",
				zip: "076250"
			}, {
				name: "怀安县",
				zip: "076150"
			}, {
				name: "涿鹿县",
				zip: "075600"
			}, {
				name: "怀来县",
				zip: "075400"
			}, {
				name: "崇礼县",
				zip: "076350"
			}, {
				name: "赤城县",
				zip: "075500"
			}, {
				name: "宣化县",
				zip: "075100"
			}, {
				name: "康保县",
				zip: "076650"
			}, {
				name: "张北县",
				zip: "076450"
			}, {
				name: "尚义县",
				zip: "076750"
			}, {
				name: "沽源县",
				zip: "076550"
			}, {
				name: "阳原县",
				zip: "075800"
			}, {
				name: "蔚县",
				zip: "075700"
			}]
		}, {
			name: "承德市",
			district: [{
				name: "鹰手营子矿区",
				zip: "067000"
			}, {
				name: "双桥区",
				zip: "067000"
			}, {
				name: "双滦区",
				zip: "067000"
			}, {
				name: "承德县",
				zip: "067400"
			}, {
				name: "兴隆县",
				zip: "067300"
			}, {
				name: "平泉县",
				zip: "067500"
			}, {
				name: "滦平县",
				zip: "068250"
			}, {
				name: "隆化县",
				zip: "068150"
			}, {
				name: "丰宁满族自治县",
				zip: "068350"
			}, {
				name: "宽城满族自治县",
				zip: "067600"
			}, {
				name: "围场满族蒙古族自治县",
				zip: "068450"
			}]
		}, {
			name: "沧州市",
			district: [{
				name: "运河区",
				zip: "061000"
			}, {
				name: "新华区",
				zip: "061000"
			}, {
				name: "孟村回族自治县",
				zip: "061400"
			}, {
				name: "献县",
				zip: "062250"
			}, {
				name: "吴桥县",
				zip: "061800"
			}, {
				name: "肃宁县",
				zip: "062350"
			}, {
				name: "南皮县",
				zip: "061500"
			}, {
				name: "海兴县",
				zip: "061200"
			}, {
				name: "盐山县",
				zip: "061300"
			}, {
				name: "青县",
				zip: "062650"
			}, {
				name: "东光县",
				zip: "061600"
			}, {
				name: "沧县",
				zip: "061035"
			}, {
				name: "河间市",
				zip: "062450"
			}, {
				name: "黄骅市",
				zip: "061100"
			}, {
				name: "任丘市",
				zip: "062550"
			}, {
				name: "泊头市",
				zip: "062150"
			}]
		}, {
			name: "廊坊市",
			district: [{
				name: "三河市",
				zip: "065200"
			}, {
				name: "霸州市",
				zip: "065700"
			}, {
				name: "安次区",
				zip: "065000"
			}, {
				name: "广阳区",
				zip: "065000"
			}, {
				name: "大厂回族自治县",
				zip: "065300"
			}, {
				name: "香河县",
				zip: "065400"
			}, {
				name: "大城县",
				zip: "065900"
			}, {
				name: "文安县",
				zip: "065800"
			}, {
				name: "永清县",
				zip: "065600"
			}, {
				name: "固安县",
				zip: "065500"
			}]
		}]
	}, {
		name: "山西",
		city: [{
			name: "运城市",
			district: [{
				name: "盐湖区",
				zip: "044300"
			}, {
				name: "平陆县",
				zip: "044300"
			}, {
				name: "夏县",
				zip: "044400"
			}, {
				name: "芮城县",
				zip: "044600"
			}, {
				name: "新绛县",
				zip: "043100"
			}, {
				name: "稷山县",
				zip: "043200"
			}, {
				name: "垣曲县",
				zip: "043700"
			}, {
				name: "绛县",
				zip: "043600"
			}, {
				name: "临猗县",
				zip: "044100"
			}, {
				name: "闻喜县",
				zip: "043800"
			}, {
				name: "万荣县",
				zip: "044200"
			}, {
				name: "永济市",
				zip: "044500"
			}, {
				name: "河津市",
				zip: "043300"
			}]
		}, {
			name: "忻州市",
			district: [{
				name: "忻府区",
				zip: "034000"
			}, {
				name: "代县",
				zip: "034200"
			}, {
				name: "五台县",
				zip: "035500"
			}, {
				name: "定襄县",
				zip: "035400"
			}, {
				name: "神池县",
				zip: "036100"
			}, {
				name: "静乐县",
				zip: "035100"
			}, {
				name: "宁武县",
				zip: "036700"
			}, {
				name: "繁峙县",
				zip: "034300"
			}, {
				name: "偏关县",
				zip: "036400"
			}, {
				name: "岢岚县",
				zip: "036300"
			}, {
				name: "五寨县",
				zip: "036100"
			}, {
				name: "保德县",
				zip: "036600"
			}, {
				name: "河曲县",
				zip: "036500"
			}, {
				name: "原平市",
				zip: "034100"
			}]
		}, {
			name: "临汾市",
			district: [{
				name: "尧都区",
				zip: "041000"
			}, {
				name: "曲沃县",
				zip: "043400"
			}, {
				name: "翼城县",
				zip: "043500"
			}, {
				name: "襄汾县",
				zip: "041500"
			}, {
				name: "浮山县",
				zip: "042600"
			}, {
				name: "安泽县",
				zip: "042500"
			}, {
				name: "古县",
				zip: "042400"
			}, {
				name: "洪洞县",
				zip: "041600"
			}, {
				name: "隰县",
				zip: "041300"
			}, {
				name: "大宁县",
				zip: "042300"
			}, {
				name: "乡宁县",
				zip: "042100"
			}, {
				name: "吉县",
				zip: "042200"
			}, {
				name: "汾西县",
				zip: "031500"
			}, {
				name: "蒲县",
				zip: "041200"
			}, {
				name: "永和县",
				zip: "041400"
			}, {
				name: "霍州市",
				zip: "031400"
			}, {
				name: "侯马市",
				zip: "043000"
			}]
		}, {
			name: "吕梁市",
			district: [{
				name: "离石区",
				zip: "033000"
			}, {
				name: "交口县",
				zip: "032400"
			}, {
				name: "方山县",
				zip: "033100"
			}, {
				name: "中阳县",
				zip: "033400"
			}, {
				name: "交城县",
				zip: "030500"
			}, {
				name: "兴县",
				zip: "033601"
			}, {
				name: "文水县",
				zip: "032100"
			}, {
				name: "石楼县",
				zip: "032500"
			}, {
				name: "岚县",
				zip: "033500"
			}, {
				name: "临县",
				zip: "033200"
			}, {
				name: "柳林县",
				zip: "033300"
			}, {
				name: "孝义市",
				zip: "032208"
			}, {
				name: "汾阳市",
				zip: "032200"
			}]
		}, {
			name: "阳泉市",
			district: [{
				name: "盂县",
				zip: "045100"
			}, {
				name: "平定县",
				zip: "045200"
			}, {
				name: "城区",
				zip: "045000"
			}, {
				name: "矿区",
				zip: "045000"
			}, {
				name: "郊区",
				zip: "045011"
			}]
		}, {
			name: "长治市",
			district: [{
				name: "长治县",
				zip: "047100"
			}, {
				name: "襄垣县",
				zip: "046200"
			}, {
				name: "平顺县",
				zip: "047400"
			}, {
				name: "屯留县",
				zip: "046100"
			}, {
				name: "壶关县",
				zip: "047300"
			}, {
				name: "黎城县",
				zip: "047600"
			}, {
				name: "武乡县",
				zip: "046300"
			}, {
				name: "长子县",
				zip: "046600"
			}, {
				name: "沁源县",
				zip: "046500"
			}, {
				name: "沁县",
				zip: "046400"
			}, {
				name: "潞城市",
				zip: "047500"
			}, {
				name: "郊区",
				zip: "046011"
			}, {
				name: "城区",
				zip: "046011"
			}]
		}, {
			name: "晋城市",
			district: [{
				name: "泽州县",
				zip: "048012"
			}, {
				name: "陵川县",
				zip: "048300"
			}, {
				name: "阳城县",
				zip: "048100"
			}, {
				name: "沁水县",
				zip: "048200"
			}, {
				name: "城区",
				zip: "048000"
			}, {
				name: "高平市",
				zip: "048400"
			}]
		}, {
			name: "朔州市",
			district: [{
				name: "平鲁区",
				zip: "038600"
			}, {
				name: "朔城区",
				zip: "038500"
			}, {
				name: "怀仁县",
				zip: "038300"
			}, {
				name: "应县",
				zip: "037600"
			}, {
				name: "右玉县",
				zip: "037200"
			}, {
				name: "山阴县",
				zip: "036900"
			}]
		}, {
			name: "晋中市",
			district: [{
				name: "榆社县",
				zip: "031800"
			}, {
				name: "左权县",
				zip: "032600"
			}, {
				name: "和顺县",
				zip: "032700"
			}, {
				name: "昔阳县",
				zip: "045300"
			}, {
				name: "寿阳县",
				zip: "045400"
			}, {
				name: "太谷县",
				zip: "030800"
			}, {
				name: "祁县",
				zip: "030900"
			}, {
				name: "平遥县",
				zip: "031100"
			}, {
				name: "灵石县",
				zip: "031300"
			}, {
				name: "榆次区",
				zip: "030600"
			}, {
				name: "介休市",
				zip: "032000"
			}]
		}, {
			name: "太原市",
			district: [{
				name: "娄烦县",
				zip: "030300"
			}, {
				name: "阳曲县",
				zip: "030100"
			}, {
				name: "清徐县",
				zip: "030400"
			}, {
				name: "迎泽区",
				zip: "030024"
			}, {
				name: "杏花岭区",
				zip: "030001"
			}, {
				name: "小店区",
				zip: "030032"
			}, {
				name: "晋源区",
				zip: "030025"
			}, {
				name: "尖草坪区",
				zip: "030003"
			}, {
				name: "万柏林区",
				zip: "030027"
			}, {
				name: "古交市",
				zip: "030200"
			}]
		}, {
			name: "大同市",
			district: [{
				name: "大同县",
				zip: "037300"
			}, {
				name: "左云县",
				zip: "037100"
			}, {
				name: "浑源县",
				zip: "037400"
			}, {
				name: "灵丘县",
				zip: "034400"
			}, {
				name: "阳高县",
				zip: "038100"
			}, {
				name: "天镇县",
				zip: "038200"
			}, {
				name: "广灵县",
				zip: "037500"
			}, {
				name: "新荣区",
				zip: "037002"
			}, {
				name: "南郊区",
				zip: "037001"
			}, {
				name: "矿区",
				zip: "037001"
			}, {
				name: "城区",
				zip: "037008"
			}]
		}]
	}, {
		name: "辽宁",
		city: [{
			name: "大连市",
			district: [{
				name: "西岗区",
				zip: "116011"
			}, {
				name: "中山区",
				zip: "116001"
			}, {
				name: "沙河口区",
				zip: "116021"
			}, {
				name: "甘井子区",
				zip: "116033"
			}, {
				name: "旅顺口区",
				zip: "116041"
			}, {
				name: "金州区",
				zip: "116100"
			}, {
				name: "长海县",
				zip: "116500"
			}, {
				name: "瓦房店市",
				zip: "116300"
			}, {
				name: "普兰店市",
				zip: "116200"
			}, {
				name: "庄河市",
				zip: "116400"
			}, {
				name: "岭前区",
				zip: ""
			}]
		}, {
			name: "鞍山市",
			district: [{
				name: "铁西区",
				zip: "110023"
			}, {
				name: "铁东区",
				zip: "114001"
			}, {
				name: "立山区",
				zip: "114031"
			}, {
				name: "千山区",
				zip: "114041"
			}, {
				name: "岫岩满族自治县",
				zip: "114300"
			}, {
				name: "台安县",
				zip: "114100"
			}, {
				name: "海城市",
				zip: "114200"
			}]
		}, {
			name: "抚顺市",
			district: [{
				name: "望花区",
				zip: "113001"
			}, {
				name: "东洲区",
				zip: "113003"
			}, {
				name: "新抚区",
				zip: "113006"
			}, {
				name: "顺城区",
				zip: "113006"
			}, {
				name: "抚顺县",
				zip: "113100"
			}, {
				name: "新宾满族自治县",
				zip: "113200"
			}, {
				name: "清原满族自治县",
				zip: "113300"
			}]
		}, {
			name: "沈阳市",
			district: [{
				name: "沈北新区",
				zip: ""
			}, {
				name: "浑南新区",
				zip: ""
			}, {
				name: "张士开发区",
				zip: ""
			}, {
				name: "新民市",
				zip: "110300"
			}, {
				name: "和平区",
				zip: "110002"
			}, {
				name: "沈河区",
				zip: "110013"
			}, {
				name: "铁西区",
				zip: "110023"
			}, {
				name: "大东区",
				zip: "110044"
			}, {
				name: "皇姑区",
				zip: "110031"
			}, {
				name: "苏家屯区",
				zip: "110102"
			}, {
				name: "新城子区",
				zip: "110121"
			}, {
				name: "东陵区",
				zip: "110015"
			}, {
				name: "于洪区",
				zip: "110024"
			}, {
				name: "法库县",
				zip: "110400"
			}, {
				name: "康平县",
				zip: "110500"
			}, {
				name: "辽中县",
				zip: "110200"
			}]
		}, {
			name: "锦州市",
			district: [{
				name: "黑山县",
				zip: "121400"
			}, {
				name: "义县",
				zip: "121100"
			}, {
				name: "古塔区",
				zip: "121001"
			}, {
				name: "凌河区",
				zip: "121000"
			}, {
				name: "太和区",
				zip: "121011"
			}, {
				name: "北镇市",
				zip: "121300"
			}, {
				name: "凌海市",
				zip: "121200"
			}]
		}, {
			name: "营口市",
			district: [{
				name: "老边区",
				zip: "115005"
			}, {
				name: "西市区",
				zip: "115004"
			}, {
				name: "站前区",
				zip: "115002"
			}, {
				name: "鲅鱼圈区",
				zip: "115007"
			}, {
				name: "大石桥市",
				zip: "115100"
			}, {
				name: "盖州市",
				zip: "115200"
			}]
		}, {
			name: "阜新市",
			district: [{
				name: "阜新蒙古族自治县",
				zip: "123100"
			}, {
				name: "彰武县",
				zip: "123200"
			}, {
				name: "海州区",
				zip: "123000"
			}, {
				name: "新邱区",
				zip: "123005"
			}, {
				name: "太平区",
				zip: "123003"
			}, {
				name: "清河门区",
				zip: "123006"
			}, {
				name: "细河区",
				zip: "123000"
			}]
		}, {
			name: "本溪市",
			district: [{
				name: "本溪满族自治县",
				zip: "117100"
			}, {
				name: "桓仁满族自治县",
				zip: "117200"
			}, {
				name: "明山区",
				zip: "117021"
			}, {
				name: "南芬区",
				zip: "117014"
			}, {
				name: "平山区",
				zip: "117000"
			}, {
				name: "溪湖区",
				zip: "117002"
			}]
		}, {
			name: "丹东市",
			district: [{
				name: "振安区",
				zip: "118001"
			}, {
				name: "振兴区",
				zip: "118002"
			}, {
				name: "元宝区",
				zip: "118000"
			}, {
				name: "凤城市",
				zip: "118100"
			}, {
				name: "东港市",
				zip: "118300"
			}, {
				name: "宽甸满族自治县",
				zip: "118200"
			}]
		}, {
			name: "葫芦岛市",
			district: [{
				name: "建昌县",
				zip: "125300"
			}, {
				name: "绥中县",
				zip: "125200"
			}, {
				name: "南票区",
				zip: "125027"
			}, {
				name: "龙港区",
				zip: "125004"
			}, {
				name: "连山区",
				zip: "125001"
			}, {
				name: "兴城市",
				zip: "125100"
			}]
		}, {
			name: "朝阳市",
			district: [{
				name: "北票市",
				zip: "122100"
			}, {
				name: "凌源市",
				zip: "122500"
			}, {
				name: "喀喇沁左翼蒙古族自治县",
				zip: "122300"
			}, {
				name: "朝阳县",
				zip: "122000"
			}, {
				name: "建平县",
				zip: "122400"
			}, {
				name: "双塔区",
				zip: "122000"
			}, {
				name: "龙城区",
				zip: "122000"
			}]
		}, {
			name: "铁岭市",
			district: [{
				name: "开原市",
				zip: "112300"
			}, {
				name: "调兵山市",
				zip: "112700"
			}, {
				name: "西丰县",
				zip: "112400"
			}, {
				name: "铁岭县",
				zip: "112600"
			}, {
				name: "昌图县",
				zip: "112500"
			}, {
				name: "银州区",
				zip: "112000"
			}, {
				name: "清河区",
				zip: "112003"
			}]
		}, {
			name: "盘锦市",
			district: [{
				name: "双台子区",
				zip: "124000"
			}, {
				name: "兴隆台区",
				zip: "124010"
			}, {
				name: "盘山县",
				zip: "124100"
			}, {
				name: "大洼县",
				zip: "124000"
			}]
		}, {
			name: "辽阳市",
			district: [{
				name: "灯塔市",
				zip: "111300"
			}, {
				name: "太子河区",
				zip: "111000"
			}, {
				name: "辽阳县",
				zip: "111200"
			}, {
				name: "文圣区",
				zip: "111000"
			}, {
				name: "白塔区",
				zip: "111000"
			}, {
				name: "弓长岭区",
				zip: "111008"
			}, {
				name: "宏伟区",
				zip: "111003"
			}]
		}]
	}, {
		name: "吉林",
		city: [{
			name: "延边朝鲜族自治州",
			district: [{
				name: "汪清县",
				zip: "133200"
			}, {
				name: "安图县",
				zip: "133600"
			}, {
				name: "延吉市",
				zip: "133000"
			}, {
				name: "图们市",
				zip: "133100"
			}, {
				name: "敦化市",
				zip: "133700"
			}, {
				name: "珲春市",
				zip: "133300"
			}, {
				name: "龙井市",
				zip: "133400"
			}, {
				name: "和龙市",
				zip: "133500"
			}]
		}, {
			name: "长春市",
			district: [{
				name: "农安县",
				zip: "130200"
			}, {
				name: "双阳区",
				zip: "130600"
			}, {
				name: "二道区",
				zip: "130031"
			}, {
				name: "朝阳区",
				zip: "130012"
			}, {
				name: "绿园区",
				zip: "130062"
			}, {
				name: "宽城区",
				zip: "130051"
			}, {
				name: "南关区",
				zip: "130022"
			}, {
				name: "净月旅游开发区",
				zip: ""
			}, {
				name: "经济技术开发区",
				zip: ""
			}, {
				name: "汽车产业开发区",
				zip: ""
			}, {
				name: "高新技术产业开发区",
				zip: ""
			}, {
				name: "德惠市",
				zip: "130300"
			}, {
				name: "榆树市",
				zip: "130400"
			}, {
				name: "九台市",
				zip: "130500"
			}]
		}, {
			name: "松原市",
			district: [{
				name: "前郭尔罗斯蒙古族自治县",
				zip: "131100"
			}, {
				name: "乾安县",
				zip: "131400"
			}, {
				name: "长岭县",
				zip: "131500"
			}, {
				name: "扶余市",
				zip: "131200"
			}, {
				name: "宁江区",
				zip: "138000"
			}]
		}, {
			name: "白城市",
			district: [{
				name: "大安市",
				zip: "131300"
			}, {
				name: "洮南市",
				zip: "137100"
			}, {
				name: "洮北区",
				zip: "137000"
			}, {
				name: "通榆县",
				zip: "137200"
			}, {
				name: "镇赉县",
				zip: "137300"
			}]
		}, {
			name: "通化市",
			district: [{
				name: "二道江区",
				zip: "134303"
			}, {
				name: "东昌区",
				zip: "134001"
			}, {
				name: "辉南县",
				zip: "135100"
			}, {
				name: "通化县",
				zip: "134100"
			}, {
				name: "柳河县",
				zip: "135300"
			}, {
				name: "集安市",
				zip: "134200"
			}, {
				name: "梅河口市",
				zip: "135000"
			}]
		}, {
			name: "白山市",
			district: [{
				name: "临江市",
				zip: "134600"
			}, {
				name: "江源区",
				zip: "134700"
			}, {
				name: "抚松县",
				zip: "134500"
			}, {
				name: "长白朝鲜族自治县",
				zip: "134400"
			}, {
				name: "靖宇县",
				zip: "135200"
			}, {
				name: "浑江区",
				zip: "134300"
			}]
		}, {
			name: "吉林市",
			district: [{
				name: "蛟河市",
				zip: "132500"
			}, {
				name: "舒兰市",
				zip: "132600"
			}, {
				name: "桦甸市",
				zip: "132400"
			}, {
				name: "磐石市",
				zip: "132300"
			}, {
				name: "船营区",
				zip: "132011"
			}, {
				name: "昌邑区",
				zip: "132002"
			}, {
				name: "龙潭区",
				zip: "132021"
			}, {
				name: "永吉县",
				zip: "132100"
			}, {
				name: "丰满区",
				zip: "132113"
			}]
		}, {
			name: "辽源市",
			district: [{
				name: "东丰县",
				zip: "136300"
			}, {
				name: "东辽县",
				zip: "136600"
			}, {
				name: "龙山区",
				zip: "136200"
			}, {
				name: "西安区",
				zip: "136201"
			}]
		}, {
			name: "四平市",
			district: [{
				name: "双辽市",
				zip: "136400"
			}, {
				name: "公主岭市",
				zip: "136100"
			}, {
				name: "铁东区",
				zip: "136001"
			}, {
				name: "铁西区",
				zip: "136000"
			}, {
				name: "伊通满族自治县",
				zip: "130700"
			}, {
				name: "梨树县",
				zip: "136500"
			}]
		}]
	}, {
		name: "黑龙江",
		city: [{
			name: "大兴安岭地区",
			district: [{
				name: "呼玛县",
				zip: "165100"
			}, {
				name: "塔河县",
				zip: "165200"
			}, {
				name: "漠河县",
				zip: "165300"
			}, {
				name: "加格达奇区",
				zip: "165300"
			}, {
				name: "呼中区",
				zip: ""
			}, {
				name: "新林区",
				zip: ""
			}, {
				name: "松岭区",
				zip: ""
			}]
		}, {
			name: "绥化市",
			district: [{
				name: "肇东市",
				zip: "151100"
			}, {
				name: "海伦市",
				zip: "152300"
			}, {
				name: "安达市",
				zip: "151400"
			}, {
				name: "北林区",
				zip: "152000"
			}, {
				name: "兰西县",
				zip: "151500"
			}, {
				name: "青冈县",
				zip: "151600"
			}, {
				name: "望奎县",
				zip: "152100"
			}, {
				name: "绥棱县",
				zip: "152200"
			}, {
				name: "庆安县",
				zip: "152400"
			}, {
				name: "明水县",
				zip: "151700"
			}]
		}, {
			name: "牡丹江市",
			district: [{
				name: "东安区",
				zip: "157000"
			}, {
				name: "阳明区",
				zip: "157013"
			}, {
				name: "爱民区",
				zip: "157009"
			}, {
				name: "西安区",
				zip: "157000"
			}, {
				name: "东宁县",
				zip: "157200"
			}, {
				name: "林口县",
				zip: "157600"
			}, {
				name: "宁安市",
				zip: "157400"
			}, {
				name: "穆棱市",
				zip: "157500"
			}, {
				name: "绥芬河市",
				zip: "157300"
			}, {
				name: "海林市",
				zip: "157100"
			}]
		}, {
			name: "黑河市",
			district: [{
				name: "北安市",
				zip: "164000"
			}, {
				name: "五大连池市",
				zip: "164100"
			}, {
				name: "逊克县",
				zip: "164400"
			}, {
				name: "嫩江县",
				zip: "161400"
			}, {
				name: "孙吴县",
				zip: "164200"
			}, {
				name: "爱辉区",
				zip: "164300"
			}]
		}, {
			name: "伊春市",
			district: [{
				name: "铁力市",
				zip: "152500"
			}, {
				name: "嘉荫县",
				zip: "153200"
			}, {
				name: "南岔区",
				zip: "153100"
			}, {
				name: "伊春区",
				zip: "153000"
			}, {
				name: "翠峦区",
				zip: "153013"
			}, {
				name: "新青区",
				zip: "153036"
			}, {
				name: "友好区",
				zip: "153031"
			}, {
				name: "西林区",
				zip: "153025"
			}, {
				name: "五营区",
				zip: "153033"
			}, {
				name: "乌马河区",
				zip: "153011"
			}, {
				name: "美溪区",
				zip: "153021"
			}, {
				name: "金山屯区",
				zip: "152026"
			}, {
				name: "乌伊岭区",
				zip: "153038"
			}, {
				name: "红星区",
				zip: "153035"
			}, {
				name: "汤旺河区",
				zip: "153037"
			}, {
				name: "带岭区",
				zip: "153106"
			}, {
				name: "上甘岭区",
				zip: "153032"
			}]
		}, {
			name: "七台河市",
			district: [{
				name: "勃利县",
				zip: "154500"
			}, {
				name: "茄子河区",
				zip: "154622"
			}, {
				name: "桃山区",
				zip: "154600"
			}, {
				name: "新兴区",
				zip: "154604"
			}]
		}, {
			name: "佳木斯市",
			district: [{
				name: "富锦市",
				zip: "156100"
			}, {
				name: "同江市",
				zip: "156400"
			}, {
				name: "桦川县",
				zip: "154300"
			}, {
				name: "汤原县",
				zip: "154700"
			}, {
				name: "桦南县",
				zip: "154400"
			}, {
				name: "抚远县",
				zip: "156500"
			}, {
				name: "郊区",
				zip: "154004"
			}, {
				name: "向阳区",
				zip: "154002"
			}, {
				name: "永红区",
				zip: "154003"
			}, {
				name: "东风区",
				zip: "154005"
			}, {
				name: "前进区",
				zip: "154002"
			}]
		}, {
			name: "双鸭山市",
			district: [{
				name: "饶河县",
				zip: "155700"
			}, {
				name: "集贤县",
				zip: "155900"
			}, {
				name: "友谊县",
				zip: "155800"
			}, {
				name: "宝清县",
				zip: "155600"
			}, {
				name: "岭东区",
				zip: "155120"
			}, {
				name: "尖山区",
				zip: "155100"
			}, {
				name: "四方台区",
				zip: "155130"
			}, {
				name: "宝山区",
				zip: "155131"
			}]
		}, {
			name: "鹤岗市",
			district: [{
				name: "绥滨县",
				zip: "156200"
			}, {
				name: "萝北县",
				zip: "154200"
			}, {
				name: "工农区",
				zip: "154101"
			}, {
				name: "向阳区",
				zip: "154100"
			}, {
				name: "兴山区",
				zip: "154105"
			}, {
				name: "东山区",
				zip: "154106"
			}, {
				name: "兴安区",
				zip: "154102"
			}, {
				name: "南山区",
				zip: "154104"
			}]
		}, {
			name: "大庆市",
			district: [{
				name: "杜尔伯特蒙古族自治县",
				zip: "166200"
			}, {
				name: "林甸县",
				zip: "166300"
			}, {
				name: "肇源县",
				zip: "166500"
			}, {
				name: "肇州县",
				zip: "166400"
			}, {
				name: "大同区",
				zip: "163814"
			}, {
				name: "让胡路区",
				zip: "163453"
			}, {
				name: "红岗区",
				zip: "163512"
			}, {
				name: "萨尔图区",
				zip: "163311"
			}, {
				name: "龙凤区",
				zip: "163711"
			}]
		}, {
			name: "齐齐哈尔市",
			district: [{
				name: "富拉尔基区",
				zip: "161041"
			}, {
				name: "碾子山区",
				zip: "161046"
			}, {
				name: "铁锋区",
				zip: "161000"
			}, {
				name: "昂昂溪区",
				zip: "161031"
			}, {
				name: "龙沙区",
				zip: "161000"
			}, {
				name: "建华区",
				zip: "161006"
			}, {
				name: "龙江县",
				zip: "161100"
			}, {
				name: "依安县",
				zip: "161500"
			}, {
				name: "梅里斯达斡尔族区",
				zip: "161021"
			}, {
				name: "泰来县",
				zip: "162400"
			}, {
				name: "甘南县",
				zip: "162100"
			}, {
				name: "富裕县",
				zip: "161200"
			}, {
				name: "克山县",
				zip: "161600"
			}, {
				name: "克东县",
				zip: "164800"
			}, {
				name: "拜泉县",
				zip: "164700"
			}, {
				name: "讷河市",
				zip: "161300"
			}]
		}, {
			name: "鸡西市",
			district: [{
				name: "恒山区",
				zip: "158130"
			}, {
				name: "鸡冠区",
				zip: "158100"
			}, {
				name: "城子河区",
				zip: "158170"
			}, {
				name: "麻山区",
				zip: "158180"
			}, {
				name: "滴道区",
				zip: "158150"
			}, {
				name: "梨树区",
				zip: "158160"
			}, {
				name: "鸡东县",
				zip: "158200"
			}, {
				name: "密山市",
				zip: "158300"
			}, {
				name: "虎林市",
				zip: "158400"
			}]
		}, {
			name: "哈尔滨市",
			district: [{
				name: "阿城市",
				zip: "150300"
			}, {
				name: "五常市",
				zip: "150200"
			}, {
				name: "尚志市",
				zip: "150600"
			}, {
				name: "双城市",
				zip: "150100"
			}, {
				name: "阿城区",
				zip: "150300"
			}, {
				name: "南岗区",
				zip: "150006"
			}, {
				name: "道里区",
				zip: "150010"
			}, {
				name: "动力区",
				zip: "150040"
			}, {
				name: "香坊区",
				zip: "150036"
			}, {
				name: "道外区",
				zip: "150026"
			}, {
				name: "呼兰区",
				zip: "150500"
			}, {
				name: "松北区",
				zip: "150028"
			}, {
				name: "平房区",
				zip: "150060"
			}, {
				name: "延寿县",
				zip: "150700"
			}, {
				name: "通河县",
				zip: "150900"
			}, {
				name: "依兰县",
				zip: "154800"
			}, {
				name: "方正县",
				zip: "150800"
			}, {
				name: "宾县",
				zip: "150400"
			}, {
				name: "巴彦县",
				zip: "151800"
			}, {
				name: "木兰县",
				zip: "151900"
			}]
		}]
	}, {
		name: "江苏",
		city: [{
			name: "宿迁市",
			district: [{
				name: "泗洪县",
				zip: "223900"
			}, {
				name: "沭阳县",
				zip: "223600"
			}, {
				name: "泗阳县",
				zip: "223700"
			}, {
				name: "宿城区",
				zip: "223800"
			}, {
				name: "宿豫区",
				zip: "223800"
			}]
		}, {
			name: "泰州市",
			district: [{
				name: "姜堰区",
				zip: "225500"
			}, {
				name: "兴化市",
				zip: "225700"
			}, {
				name: "靖江市",
				zip: "214500"
			}, {
				name: "泰兴市",
				zip: "225400"
			}, {
				name: "海陵区",
				zip: "225300"
			}, {
				name: "高港区",
				zip: "225321"
			}]
		}, {
			name: "镇江市",
			district: [{
				name: "扬中市",
				zip: "212200"
			}, {
				name: "句容市",
				zip: "212400"
			}, {
				name: "丹阳市",
				zip: "212300"
			}, {
				name: "京口区",
				zip: "212001"
			}, {
				name: "润州区",
				zip: "212004"
			}, {
				name: "丹徒区",
				zip: "212001"
			}]
		}, {
			name: "扬州市",
			district: [{
				name: "江都区",
				zip: "225200"
			}, {
				name: "经济开发区",
				zip: ""
			}, {
				name: "高邮市",
				zip: "225600"
			}, {
				name: "仪征市",
				zip: "211400"
			}, {
				name: "宝应县",
				zip: "225800"
			}, {
				name: "维扬区",
				zip: "225002"
			}, {
				name: "邗江区",
				zip: "225100"
			}, {
				name: "广陵区",
				zip: "225002"
			}]
		}, {
			name: "盐城市",
			district: [{
				name: "东台市",
				zip: "224200"
			}, {
				name: "大丰市",
				zip: "224100"
			}, {
				name: "响水县",
				zip: "224600"
			}, {
				name: "滨海县",
				zip: "224500"
			}, {
				name: "阜宁县",
				zip: "224400"
			}, {
				name: "射阳县",
				zip: "224300"
			}, {
				name: "建湖县",
				zip: "224700"
			}, {
				name: "盐都区",
				zip: "224055"
			}, {
				name: "亭湖区",
				zip: "224005"
			}]
		}, {
			name: "淮安市",
			district: [{
				name: "涟水县",
				zip: "223400"
			}, {
				name: "金湖县",
				zip: "211600"
			}, {
				name: "盱眙县",
				zip: "211700"
			}, {
				name: "洪泽县",
				zip: "223100"
			}, {
				name: "清河区",
				zip: "223001"
			}, {
				name: "淮安区",
				zip: "223200"
			}, {
				name: "淮阴区",
				zip: "223000"
			}, {
				name: "清浦区",
				zip: "223002"
			}]
		}, {
			name: "连云港市",
			district: [{
				name: "海州区",
				zip: "222023"
			}, {
				name: "新浦区",
				zip: "222003"
			}, {
				name: "灌南县",
				zip: "222500"
			}, {
				name: "东海县",
				zip: "222300"
			}, {
				name: "灌云县",
				zip: "222200"
			}, {
				name: "赣榆县",
				zip: "222100"
			}, {
				name: "连云区",
				zip: "222042"
			}]
		}, {
			name: "南通市",
			district: [{
				name: "启东市",
				zip: "226200"
			}, {
				name: "通州市",
				zip: "226300"
			}, {
				name: "如皋市",
				zip: "226500"
			}, {
				name: "海门市",
				zip: "226100"
			}, {
				name: "开发区",
				zip: ""
			}, {
				name: "崇川区",
				zip: "226001"
			}, {
				name: "通州区",
				zip: "226321"
			}, {
				name: "港闸区",
				zip: "226001"
			}, {
				name: "海安县",
				zip: "226600"
			}, {
				name: "如东县",
				zip: "226400"
			}]
		}, {
			name: "常州市",
			district: [{
				name: "钟楼区",
				zip: "213002"
			}, {
				name: "戚墅堰区",
				zip: "213011"
			}, {
				name: "天宁区",
				zip: "213003"
			}, {
				name: "武进区",
				zip: "213161"
			}, {
				name: "新北区",
				zip: "213001"
			}, {
				name: "金坛市",
				zip: "213200"
			}, {
				name: "溧阳市",
				zip: "213300"
			}]
		}, {
			name: "苏州市",
			district: [{
				name: "张家港市",
				zip: "215600"
			}, {
				name: "昆山市",
				zip: "215300"
			}, {
				name: "常熟市",
				zip: "215500"
			}, {
				name: "吴江区",
				zip: "215200"
			}, {
				name: "太仓市",
				zip: "215400"
			}, {
				name: "园区",
				zip: ""
			}, {
				name: "新区",
				zip: ""
			}, {
				name: "沧浪区",
				zip: "215006"
			}, {
				name: "平江区",
				zip: "215005"
			}, {
				name: "吴中区",
				zip: "215128"
			}, {
				name: "相城区",
				zip: "215131"
			}, {
				name: "金阊区",
				zip: "215008"
			}, {
				name: "虎丘区",
				zip: "215004"
			}, {
				name: "姑苏区",
				zip: ""
			}]
		}, {
			name: "徐州市",
			district: [{
				name: "泉山区",
				zip: "220006"
			}, {
				name: "贾汪区",
				zip: "220011"
			}, {
				name: "九里区",
				zip: "220040"
			}, {
				name: "鼓楼区",
				zip: "220005"
			}, {
				name: "云龙区",
				zip: "220009"
			}, {
				name: "邳州市",
				zip: "221300"
			}, {
				name: "新沂市",
				zip: "221400"
			}, {
				name: "丰县",
				zip: "221700"
			}, {
				name: "沛县",
				zip: "221600"
			}, {
				name: "铜山区",
				zip: "221112"
			}, {
				name: "睢宁县",
				zip: "221200"
			}]
		}, {
			name: "无锡市",
			district: [{
				name: "新区",
				zip: ""
			}, {
				name: "江阴市",
				zip: "214400"
			}, {
				name: "宜兴市",
				zip: "214200"
			}, {
				name: "南长区",
				zip: "214021"
			}, {
				name: "崇安区",
				zip: "214002"
			}, {
				name: "惠山区",
				zip: "214187"
			}, {
				name: "锡山区",
				zip: "214101"
			}, {
				name: "北塘区",
				zip: "214044"
			}, {
				name: "滨湖区",
				zip: "214062"
			}]
		}, {
			name: "南京市",
			district: [{
				name: "秦淮区",
				zip: "210001"
			}, {
				name: "建邺区",
				zip: "210004"
			}, {
				name: "鼓楼区",
				zip: "210009"
			}, {
				name: "下关区",
				zip: "210011"
			}, {
				name: "浦口区",
				zip: "211800"
			}, {
				name: "玄武区",
				zip: "210018"
			}, {
				name: "白下区",
				zip: "210002"
			}, {
				name: "高淳区",
				zip: "211300"
			}, {
				name: "溧水区",
				zip: "211200"
			}, {
				name: "栖霞区",
				zip: "210046"
			}, {
				name: "江宁区",
				zip: "211100"
			}, {
				name: "雨花台区",
				zip: "210012"
			}, {
				name: "六合区",
				zip: "211500"
			}]
		}]
	}, {
		name: "浙江",
		city: [{
			name: "嘉兴市",
			district: [{
				name: "桐乡市",
				zip: "314500"
			}, {
				name: "平湖市",
				zip: "314200"
			}, {
				name: "海宁市",
				zip: "314400"
			}, {
				name: "秀洲区",
				zip: "314001"
			}, {
				name: "南湖区",
				zip: "314000"
			}, {
				name: "海盐县",
				zip: "314300"
			}, {
				name: "嘉善县",
				zip: "314100"
			}]
		}, {
			name: "温州市",
			district: [{
				name: "瑞安市",
				zip: "325200"
			}, {
				name: "乐清市",
				zip: "325600"
			}, {
				name: "瓯海区",
				zip: "325005"
			}, {
				name: "永嘉县",
				zip: "325100"
			}, {
				name: "平阳县",
				zip: "325400"
			}, {
				name: "苍南县",
				zip: "325800"
			}, {
				name: "洞头县",
				zip: "325700"
			}, {
				name: "文成县",
				zip: "325300"
			}, {
				name: "泰顺县",
				zip: "325500"
			}, {
				name: "鹿城区",
				zip: "325000"
			}, {
				name: "龙湾区",
				zip: "325024"
			}]
		}, {
			name: "金华市",
			district: [{
				name: "浦江县",
				zip: "322200"
			}, {
				name: "磐安县",
				zip: "322300"
			}, {
				name: "武义县",
				zip: "321200"
			}, {
				name: "婺城区",
				zip: "321051"
			}, {
				name: "金东区",
				zip: "321002"
			}, {
				name: "兰溪市",
				zip: "321100"
			}, {
				name: "义乌市",
				zip: "322000"
			}, {
				name: "东阳市",
				zip: "322100"
			}, {
				name: "永康市",
				zip: "321300"
			}]
		}, {
			name: "绍兴市",
			district: [{
				name: "诸暨市",
				zip: "311800"
			}, {
				name: "嵊州市",
				zip: "312400"
			}, {
				name: "上虞市",
				zip: "312300"
			}, {
				name: "新昌县",
				zip: "312500"
			}, {
				name: "绍兴县",
				zip: "312000"
			}, {
				name: "越城区",
				zip: "312000"
			}]
		}, {
			name: "湖州市",
			district: [{
				name: "长兴县",
				zip: "313100"
			}, {
				name: "安吉县",
				zip: "313300"
			}, {
				name: "德清县",
				zip: "313200"
			}, {
				name: "南浔区",
				zip: "313009"
			}, {
				name: "吴兴区",
				zip: "313000"
			}]
		}, {
			name: "宁波市",
			district: [{
				name: "奉化市",
				zip: "315500"
			}, {
				name: "慈溪市",
				zip: "315300"
			}, {
				name: "余姚市",
				zip: "315400"
			}, {
				name: "海曙区",
				zip: "315000"
			}, {
				name: "北仑区",
				zip: "315800"
			}, {
				name: "江北区",
				zip: "315020"
			}, {
				name: "江东区",
				zip: "315040"
			}, {
				name: "象山县",
				zip: "315700"
			}, {
				name: "宁海县",
				zip: "315600"
			}, {
				name: "镇海区",
				zip: "315200"
			}, {
				name: "鄞州区",
				zip: "315100"
			}]
		}, {
			name: "杭州市",
			district: [{
				name: "临安市",
				zip: "311300"
			}, {
				name: "建德市",
				zip: "311600"
			}, {
				name: "富阳市",
				zip: "311400"
			}, {
				name: "淳安县",
				zip: "311700"
			}, {
				name: "桐庐县",
				zip: "311500"
			}, {
				name: "上城区",
				zip: "311500"
			}, {
				name: "下城区",
				zip: "310006"
			}, {
				name: "江干区",
				zip: "310002"
			}, {
				name: "拱墅区",
				zip: "310011"
			}, {
				name: "西湖区",
				zip: "310013"
			}, {
				name: "滨江区",
				zip: "310051"
			}, {
				name: "萧山区",
				zip: "311200"
			}, {
				name: "余杭区",
				zip: "311100"
			}]
		}, {
			name: "舟山市",
			district: [{
				name: "普陀区",
				zip: "316100"
			}, {
				name: "定海区",
				zip: "316000"
			}, {
				name: "嵊泗县",
				zip: "202400"
			}, {
				name: "岱山县",
				zip: "316200"
			}]
		}, {
			name: "台州市",
			district: [{
				name: "黄岩区",
				zip: "318020"
			}, {
				name: "椒江区",
				zip: "318000"
			}, {
				name: "路桥区",
				zip: "318050"
			}, {
				name: "仙居县",
				zip: "317300"
			}, {
				name: "天台县",
				zip: "317200"
			}, {
				name: "三门县",
				zip: "317100"
			}, {
				name: "玉环县",
				zip: "317600"
			}, {
				name: "临海市",
				zip: "317000"
			}, {
				name: "温岭市",
				zip: "317500"
			}]
		}, {
			name: "衢州市",
			district: [{
				name: "江山市",
				zip: "324100"
			}, {
				name: "柯城区",
				zip: "324000"
			}, {
				name: "衢江区",
				zip: "324000"
			}, {
				name: "龙游县",
				zip: "324400"
			}, {
				name: "开化县",
				zip: "324300"
			}, {
				name: "常山县",
				zip: "324200"
			}]
		}, {
			name: "丽水市",
			district: [{
				name: "龙泉市",
				zip: "323700"
			}, {
				name: "莲都区",
				zip: "323000"
			}, {
				name: "青田县",
				zip: "323900"
			}, {
				name: "缙云县",
				zip: "321400"
			}, {
				name: "遂昌县",
				zip: "323300"
			}, {
				name: "松阳县",
				zip: "323400"
			}, {
				name: "云和县",
				zip: "323600"
			}, {
				name: "庆元县",
				zip: "323800"
			}, {
				name: "景宁畲族自治县",
				zip: "323500"
			}]
		}]
	}, {
		name: "安徽",
		city: [{
			name: "蚌埠市",
			district: [{
				name: "五河县",
				zip: "233300"
			}, {
				name: "固镇县",
				zip: "233200"
			}, {
				name: "怀远县",
				zip: "233400"
			}, {
				name: "龙子湖区",
				zip: "233000"
			}, {
				name: "蚌山区",
				zip: "233000"
			}, {
				name: "禹会区",
				zip: "233010"
			}, {
				name: "淮上区",
				zip: "233002"
			}]
		}, {
			name: "淮南市",
			district: [{
				name: "潘集区",
				zip: "232082"
			}, {
				name: "谢家集区",
				zip: "232052"
			}, {
				name: "八公山区",
				zip: "232072"
			}, {
				name: "大通区",
				zip: "232033"
			}, {
				name: "田家庵区",
				zip: "232000"
			}, {
				name: "凤台县",
				zip: "232100"
			}]
		}, {
			name: "合肥市",
			district: [{
				name: "高新区",
				zip: ""
			}, {
				name: "肥东县",
				zip: "231600"
			}, {
				name: "肥西县",
				zip: "231200"
			}, {
				name: "长丰县",
				zip: "231100"
			}, {
				name: "包河区",
				zip: "230041"
			}, {
				name: "蜀山区",
				zip: "230061"
			}, {
				name: "庐阳区",
				zip: "230001"
			}, {
				name: "瑶海区",
				zip: "230011"
			}, {
				name: "中区",
				zip: ""
			}, {
				name: "庐江县",
				zip: "231500"
			}, {
				name: "巢湖市",
				zip: "238000"
			}, {
				name: "居巢区",
				zip: "238000"
			}]
		}, {
			name: "芜湖市",
			district: [{
				name: "芜湖县",
				zip: "241100"
			}, {
				name: "繁昌县",
				zip: "241200"
			}, {
				name: "南陵县",
				zip: "242400"
			}, {
				name: "三山区",
				zip: "241000"
			}, {
				name: "弋江区",
				zip: "241000"
			}, {
				name: "镜湖区",
				zip: "241000"
			}, {
				name: "鸠江区",
				zip: "241000"
			}, {
				name: "无为县",
				zip: "238300"
			}]
		}, {
			name: "安庆市",
			district: [{
				name: "迎江区",
				zip: "246003"
			}, {
				name: "大观区",
				zip: "246004"
			}, {
				name: "宜秀区",
				zip: "246003"
			}, {
				name: "枞阳县",
				zip: "246700"
			}, {
				name: "怀宁县",
				zip: "246100"
			}, {
				name: "岳西县",
				zip: "246600"
			}, {
				name: "太湖县",
				zip: "246400"
			}, {
				name: "潜山县",
				zip: "246300"
			}, {
				name: "望江县",
				zip: "246200"
			}, {
				name: "宿松县",
				zip: "246500"
			}, {
				name: "桐城市",
				zip: "231400"
			}]
		}, {
			name: "马鞍山市",
			district: [{
				name: "花山区",
				zip: "243000"
			}, {
				name: "金家庄区",
				zip: "243021"
			}, {
				name: "博望区",
				zip: ""
			}, {
				name: "雨山区",
				zip: "243071"
			}, {
				name: "当涂县",
				zip: "243100"
			}, {
				name: "含山县",
				zip: "238100"
			}, {
				name: "和县",
				zip: "238200"
			}]
		}, {
			name: "淮北市",
			district: [{
				name: "烈山区",
				zip: "235025"
			}, {
				name: "相山区",
				zip: "235000"
			}, {
				name: "杜集区",
				zip: "235047"
			}, {
				name: "濉溪县",
				zip: "235100"
			}]
		}, {
			name: "铜陵市",
			district: [{
				name: "铜官山区",
				zip: "244000"
			}, {
				name: "狮子山区",
				zip: "244031"
			}, {
				name: "铜陵县",
				zip: "244100"
			}, {
				name: "郊区",
				zip: "244000"
			}]
		}, {
			name: "宿州市",
			district: [{
				name: "砀山县",
				zip: "235300"
			}, {
				name: "萧县",
				zip: "235200"
			}, {
				name: "灵璧县",
				zip: "234200"
			}, {
				name: "泗县",
				zip: "234300"
			}, {
				name: "埇桥区",
				zip: "234000"
			}]
		}, {
			name: "六安市",
			district: [{
				name: "金安区",
				zip: "237005"
			}, {
				name: "裕安区",
				zip: "237010"
			}, {
				name: "舒城县",
				zip: "231300"
			}, {
				name: "霍邱县",
				zip: "237400"
			}, {
				name: "寿县",
				zip: "232200"
			}, {
				name: "霍山县",
				zip: "237200"
			}, {
				name: "金寨县",
				zip: "237300"
			}]
		}, {
			name: "滁州市",
			district: [{
				name: "琅琊区",
				zip: "239000"
			}, {
				name: "南谯区",
				zip: "239000"
			}, {
				name: "天长市",
				zip: "239300"
			}, {
				name: "明光市",
				zip: "239400"
			}, {
				name: "凤阳县",
				zip: "233100"
			}, {
				name: "定远县",
				zip: "233200"
			}, {
				name: "全椒县",
				zip: "239500"
			}, {
				name: "来安县",
				zip: "239200"
			}]
		}, {
			name: "黄山市",
			district: [{
				name: "祁门县",
				zip: "245600"
			}, {
				name: "黟县",
				zip: "245500"
			}, {
				name: "休宁县",
				zip: "245400"
			}, {
				name: "歙县",
				zip: "245200"
			}, {
				name: "徽州区",
				zip: "245061"
			}, {
				name: "屯溪区",
				zip: "245000"
			}, {
				name: "黄山区",
				zip: "242700"
			}]
		}, {
			name: "阜阳市",
			district: [{
				name: "界首市",
				zip: "236500"
			}, {
				name: "临泉县",
				zip: "236400"
			}, {
				name: "太和县",
				zip: "236600"
			}, {
				name: "阜南县",
				zip: "236300"
			}, {
				name: "颍上县",
				zip: "236200"
			}, {
				name: "颍州区",
				zip: "236001"
			}, {
				name: "颍东区",
				zip: "236058"
			}, {
				name: "颍泉区",
				zip: "236045"
			}]
		}, {
			name: "宣城市",
			district: [{
				name: "绩溪县",
				zip: "245300"
			}, {
				name: "旌德县",
				zip: "242600"
			}, {
				name: "宁国市",
				zip: "242300"
			}, {
				name: "宣州区",
				zip: "242000"
			}, {
				name: "泾县",
				zip: "242500"
			}, {
				name: "广德县",
				zip: "242200"
			}, {
				name: "郎溪县",
				zip: "242100"
			}]
		}, {
			name: "亳州市",
			district: [{
				name: "涡阳县",
				zip: "233600"
			}, {
				name: "利辛县",
				zip: "236700"
			}, {
				name: "蒙城县",
				zip: "233500"
			}, {
				name: "谯城区",
				zip: "236800"
			}]
		}, {
			name: "池州市",
			district: [{
				name: "石台县",
				zip: "245100"
			}, {
				name: "青阳县",
				zip: "242800"
			}, {
				name: "东至县",
				zip: "247200"
			}, {
				name: "贵池区",
				zip: "247100"
			}]
		}]
	}, {
		name: "福建",
		city: [{
			name: "龙岩市",
			district: [{
				name: "新罗区",
				zip: "364000"
			}, {
				name: "永定县",
				zip: "364100"
			}, {
				name: "上杭县",
				zip: "364200"
			}, {
				name: "长汀县",
				zip: "366300"
			}, {
				name: "武平县",
				zip: "364300"
			}, {
				name: "连城县",
				zip: "366200"
			}, {
				name: "漳平市",
				zip: "364400"
			}]
		}, {
			name: "宁德市",
			district: [{
				name: "福鼎市",
				zip: "355200"
			}, {
				name: "福安市",
				zip: "355000"
			}, {
				name: "霞浦县",
				zip: "355100"
			}, {
				name: "屏南县",
				zip: "352300"
			}, {
				name: "古田县",
				zip: "352200"
			}, {
				name: "周宁县",
				zip: "355400"
			}, {
				name: "寿宁县",
				zip: "355500"
			}, {
				name: "柘荣县",
				zip: "355300"
			}, {
				name: "蕉城区",
				zip: "352100"
			}]
		}, {
			name: "泉州市",
			district: [{
				name: "石狮市",
				zip: "362700"
			}, {
				name: "南安市",
				zip: "362300"
			}, {
				name: "晋江市",
				zip: "362200"
			}, {
				name: "惠安县",
				zip: "362100"
			}, {
				name: "永春县",
				zip: "362600"
			}, {
				name: "安溪县",
				zip: "362400"
			}, {
				name: "金门县",
				zip: "362000"
			}, {
				name: "德化县",
				zip: "362500"
			}, {
				name: "鲤城区",
				zip: "362000"
			}, {
				name: "丰泽区",
				zip: "362000"
			}, {
				name: "洛江区",
				zip: "362011"
			}, {
				name: "泉港区",
				zip: "362801"
			}]
		}, {
			name: "南平市",
			district: [{
				name: "建阳市",
				zip: "354200"
			}, {
				name: "松溪县",
				zip: "353500"
			}, {
				name: "政和县",
				zip: "353600"
			}, {
				name: "顺昌县",
				zip: "353200"
			}, {
				name: "浦城县",
				zip: "353400"
			}, {
				name: "光泽县",
				zip: "354100"
			}, {
				name: "建瓯市",
				zip: "353100"
			}, {
				name: "武夷山市",
				zip: "354300"
			}, {
				name: "邵武市",
				zip: "354000"
			}, {
				name: "延平区",
				zip: "353000"
			}]
		}, {
			name: "漳州市",
			district: [{
				name: "龙海市",
				zip: "363100"
			}, {
				name: "长泰县",
				zip: "363900"
			}, {
				name: "诏安县",
				zip: "363500"
			}, {
				name: "南靖县",
				zip: "363600"
			}, {
				name: "东山县",
				zip: "363400"
			}, {
				name: "华安县",
				zip: "363800"
			}, {
				name: "平和县",
				zip: "363700"
			}, {
				name: "云霄县",
				zip: "363300"
			}, {
				name: "漳浦县",
				zip: "363200"
			}, {
				name: "龙文区",
				zip: "363005"
			}, {
				name: "芗城区",
				zip: "363000"
			}]
		}, {
			name: "莆田市",
			district: [{
				name: "荔城区",
				zip: "351100"
			}, {
				name: "秀屿区",
				zip: "351152"
			}, {
				name: "仙游县",
				zip: "351200"
			}, {
				name: "涵江区",
				zip: "351111"
			}, {
				name: "城厢区",
				zip: "351100"
			}]
		}, {
			name: "三明市",
			district: [{
				name: "永安市",
				zip: "366000"
			}, {
				name: "三元区",
				zip: "365001"
			}, {
				name: "梅列区",
				zip: "365000"
			}, {
				name: "将乐县",
				zip: "353300"
			}, {
				name: "泰宁县",
				zip: "354400"
			}, {
				name: "建宁县",
				zip: "354500"
			}, {
				name: "宁化县",
				zip: "365400"
			}, {
				name: "大田县",
				zip: "366100"
			}, {
				name: "尤溪县",
				zip: "365100"
			}, {
				name: "沙县",
				zip: "365500"
			}, {
				name: "明溪县",
				zip: "365200"
			}, {
				name: "清流县",
				zip: "365300"
			}]
		}, {
			name: "福州市",
			district: [{
				name: "鼓楼区",
				zip: "350001"
			}, {
				name: "台江区",
				zip: "350004"
			}, {
				name: "晋安区",
				zip: "350011"
			}, {
				name: "仓山区",
				zip: "350007"
			}, {
				name: "马尾区",
				zip: "350015"
			}, {
				name: "永泰县",
				zip: "350700"
			}, {
				name: "闽清县",
				zip: "350800"
			}, {
				name: "闽侯县",
				zip: "350100"
			}, {
				name: "罗源县",
				zip: "350600"
			}, {
				name: "连江县",
				zip: "350500"
			}, {
				name: "平潭县",
				zip: "350400"
			}, {
				name: "福清市",
				zip: "350300"
			}, {
				name: "长乐市",
				zip: "350200"
			}]
		}, {
			name: "厦门市",
			district: [{
				name: "集美区",
				zip: "361021"
			}, {
				name: "同安区",
				zip: "361100"
			}, {
				name: "翔安区",
				zip: "361101"
			}, {
				name: "思明区",
				zip: "361001"
			}, {
				name: "海沧区",
				zip: "361026"
			}, {
				name: "湖里区",
				zip: "361006"
			}]
		}]
	}, {
		name: "江西",
		city: [{
			name: "萍乡市",
			district: [{
				name: "湘东区",
				zip: "337032"
			}, {
				name: "安源区",
				zip: "337000"
			}, {
				name: "莲花县",
				zip: "337100"
			}, {
				name: "芦溪县",
				zip: "337200"
			}, {
				name: "上栗县",
				zip: "337009"
			}]
		}, {
			name: "景德镇市",
			district: [{
				name: "乐平市",
				zip: "333300"
			}, {
				name: "浮梁县",
				zip: "333400"
			}, {
				name: "昌江区",
				zip: "333000"
			}, {
				name: "珠山区",
				zip: "333000"
			}]
		}, {
			name: "九江市",
			district: [{
				name: "庐山区",
				zip: "332005"
			}, {
				name: "浔阳区",
				zip: "332000"
			}, {
				name: "武宁县",
				zip: "332300"
			}, {
				name: "九江县",
				zip: "332100"
			}, {
				name: "星子县",
				zip: "332800"
			}, {
				name: "德安县",
				zip: "330400"
			}, {
				name: "永修县",
				zip: "330300"
			}, {
				name: "修水县",
				zip: "332400"
			}, {
				name: "彭泽县",
				zip: "332700"
			}, {
				name: "湖口县",
				zip: "332500"
			}, {
				name: "都昌县",
				zip: "332600"
			}, {
				name: "共青城市",
				zip: ""
			}, {
				name: "瑞昌市",
				zip: "332200"
			}]
		}, {
			name: "南昌市",
			district: [{
				name: "西湖区",
				zip: "330009"
			}, {
				name: "东湖区",
				zip: "330006"
			}, {
				name: "青山湖区",
				zip: "330006"
			}, {
				name: "湾里区",
				zip: "330004"
			}, {
				name: "青云谱区",
				zip: "330001"
			}, {
				name: "进贤县",
				zip: "331700"
			}, {
				name: "红谷滩新区",
				zip: ""
			}, {
				name: "经济技术开发区",
				zip: ""
			}, {
				name: "昌北区",
				zip: ""
			}, {
				name: "南昌县",
				zip: "330200"
			}, {
				name: "新建县",
				zip: "330100"
			}, {
				name: "安义县",
				zip: "330500"
			}]
		}, {
			name: "抚州市",
			district: [{
				name: "东乡县",
				zip: "331800"
			}, {
				name: "资溪县",
				zip: "335300"
			}, {
				name: "广昌县",
				zip: "344900"
			}, {
				name: "乐安县",
				zip: "344300"
			}, {
				name: "崇仁县",
				zip: "344200"
			}, {
				name: "金溪县",
				zip: "344800"
			}, {
				name: "宜黄县",
				zip: "344400"
			}, {
				name: "黎川县",
				zip: "344600"
			}, {
				name: "南丰县",
				zip: "344500"
			}, {
				name: "南城县",
				zip: "344700"
			}, {
				name: "临川区",
				zip: "344100"
			}]
		}, {
			name: "上饶市",
			district: [{
				name: "德兴市",
				zip: "334200"
			}, {
				name: "广丰县",
				zip: "334600"
			}, {
				name: "玉山县",
				zip: "334700"
			}, {
				name: "上饶县",
				zip: "334100"
			}, {
				name: "弋阳县",
				zip: "334400"
			}, {
				name: "余干县",
				zip: "335100"
			}, {
				name: "铅山县",
				zip: "334500"
			}, {
				name: "横峰县",
				zip: "334300"
			}, {
				name: "婺源县",
				zip: "333200"
			}, {
				name: "鄱阳县",
				zip: "333100"
			}, {
				name: "万年县",
				zip: "335500"
			}, {
				name: "信州区",
				zip: "334000"
			}]
		}, {
			name: "吉安市",
			district: [{
				name: "青原区",
				zip: "343009"
			}, {
				name: "吉州区",
				zip: "343000"
			}, {
				name: "新干县",
				zip: "331300"
			}, {
				name: "永丰县",
				zip: "331500"
			}, {
				name: "泰和县",
				zip: "343700"
			}, {
				name: "遂川县",
				zip: "343900"
			}, {
				name: "万安县",
				zip: "343800"
			}, {
				name: "安福县",
				zip: "343200"
			}, {
				name: "永新县",
				zip: "343400"
			}, {
				name: "吉安县",
				zip: "343100"
			}, {
				name: "吉水县",
				zip: "331600"
			}, {
				name: "峡江县",
				zip: "331400"
			}, {
				name: "井冈山市",
				zip: "343600"
			}]
		}, {
			name: "宜春市",
			district: [{
				name: "丰城市",
				zip: "331100"
			}, {
				name: "樟树市",
				zip: "331200"
			}, {
				name: "高安市",
				zip: "330800"
			}, {
				name: "袁州区",
				zip: "336000"
			}, {
				name: "上高县",
				zip: "336400"
			}, {
				name: "万载县",
				zip: "336100"
			}, {
				name: "奉新县",
				zip: "330700"
			}, {
				name: "铜鼓县",
				zip: "336200"
			}, {
				name: "靖安县",
				zip: "330600"
			}, {
				name: "宜丰县",
				zip: "336300"
			}]
		}, {
			name: "新余市",
			district: [{
				name: "分宜县",
				zip: "336600"
			}, {
				name: "渝水区",
				zip: "338025"
			}]
		}, {
			name: "赣州市",
			district: [{
				name: "南康市",
				zip: "341400"
			}, {
				name: "瑞金市",
				zip: "342500"
			}, {
				name: "寻乌县",
				zip: "342200"
			}, {
				name: "石城县",
				zip: "342700"
			}, {
				name: "兴国县",
				zip: "342400"
			}, {
				name: "会昌县",
				zip: "342600"
			}, {
				name: "宁都县",
				zip: "342800"
			}, {
				name: "于都县",
				zip: "342300"
			}, {
				name: "定南县",
				zip: "341900"
			}, {
				name: "全南县",
				zip: "341800"
			}, {
				name: "安远县",
				zip: "342100"
			}, {
				name: "龙南县",
				zip: "341700"
			}, {
				name: "上犹县",
				zip: "341200"
			}, {
				name: "崇义县",
				zip: "341300"
			}, {
				name: "信丰县",
				zip: "341600"
			}, {
				name: "大余县",
				zip: "341500"
			}, {
				name: "赣县",
				zip: "341100"
			}, {
				name: "黄金区",
				zip: ""
			}, {
				name: "章贡区",
				zip: ""
			}]
		}, {
			name: "鹰潭市",
			district: [{
				name: "贵溪市",
				zip: "335400"
			}, {
				name: "月湖区",
				zip: "335000"
			}, {
				name: "余江县",
				zip: "335200"
			}]
		}]
	}, {
		name: "河南",
		city: [{
			name: "郑州市",
			district: [{
				name: "中原区",
				zip: "251400"
			}, {
				name: "二七区",
				zip: "251600"
			}, {
				name: "管城回族区",
				zip: "250100"
			}, {
				name: "金水区",
				zip: "250300"
			}, {
				name: "上街去",
				zip: "250031"
			}, {
				name: "邙山区",
				zip: "250021"
			}, {
				name: "中牟县",
				zip: "250002"
			}, {
				name: "巩义市",
				zip: "250013"
			}, {
				name: "荣阳市",
				zip: "250200"
			}, {
				name: "新密市",
				zip: "250200"
			}, {
				name: "新郑市",
				zip: "250200"
			}, {
				name: "登封市",
				zip: "250200"
			}]
		}, {
			name: "开封市",
			district: [{
				name: "龙亭区",
				zip: "257029"
			}, {
				name: "顺河回族区",
				zip: "257500"
			}, {
				name: "鼓楼区",
				zip: "257400"
			}, {
				name: "南关区",
				zip: "257300"
			}, {
				name: "郊区",
				zip: ""
			}, {
				name: "杞县",
				zip: ""
			}, {
				name: "通许县",
				zip: ""
			}, {
				name: "尉氏县",
				zip: ""
			}, {
				name: "开封县",
				zip: ""
			}, {
				name: "兰考县",
				zip: ""
			}]
		}, {
			name: "洛阳市",
			district: [{
				name: "老城区",
				zip: "265300"
			}, {
				name: "西工区",
				zip: "265400"
			}, {
				name: "廛河回族区",
				zip: "265600"
			}, {
				name: "涧西区",
				zip: "261400"
			}, {
				name: "吉利区",
				zip: "265200"
			}, {
				name: "洛龙区",
				zip: "265700"
			}, {
				name: "孟津县",
				zip: "265800"
			}, {
				name: "新安县",
				zip: "264001"
			}, {
				name: "栾川县",
				zip: "265500"
			}, {
				name: "崇县",
				zip: "264600"
			}, {
				name: "汝阳县",
				zip: "264100"
			}, {
				name: "宜阳县",
				zip: "264100"
			}, {
				name: "洛宁县",
				zip: "264100"
			}, {
				name: "伊川县",
				zip: "264100"
			}, {
				name: "偃师县",
				zip: "264100"
			}]
		}, {
			name: "平顶山市",
			district: [{
				name: "新华区",
				zip: "255022"
			}, {
				name: "卫东区",
				zip: "256100"
			}, {
				name: "石龙区",
				zip: "256300"
			}, {
				name: "湛河区",
				zip: "256400"
			}, {
				name: "宝丰县",
				zip: "255300"
			}, {
				name: "叶县",
				zip: "255200"
			}, {
				name: "鲁山县",
				zip: "255400"
			}, {
				name: "郏县",
				zip: "255400"
			}, {
				name: "舞钢市",
				zip: "255400"
			}, {
				name: "汝州市",
				zip: "255400"
			}]
		}, {
			name: "安阳市",
			district: [{
				name: "文峰区",
				zip: "266400"
			}, {
				name: "北关区",
				zip: "266300"
			}, {
				name: "殷都区",
				zip: "266700"
			}, {
				name: "龙安区",
				zip: "266200"
			}, {
				name: "安阳县",
				zip: ""
			}, {
				name: "汤阴县",
				zip: "266500"
			}, {
				name: "滑县",
				zip: "266100"
			}, {
				name: "内黄县",
				zip: "266100"
			}, {
				name: "林州市",
				zip: "266041"
			}]
		}, {
			name: "鹤壁市",
			district: [{
				name: "鹤山区",
				zip: "277300"
			}, {
				name: "山城区",
				zip: "277400"
			}, {
				name: "淇滨区",
				zip: "277200"
			}, {
				name: "浚县",
				zip: "277101"
			}, {
				name: "淇县",
				zip: "277000"
			}]
		}, {
			name: "新乡市",
			district: [{
				name: "红旗区",
				zip: "276800"
			}, {
				name: "卫滨区",
				zip: "276500"
			}, {
				name: "凤泉区",
				zip: "262300"
			}, {
				name: "牧野县",
				zip: "262300"
			}, {
				name: "新乡县",
				zip: "262300"
			}, {
				name: "获嘉县",
				zip: "262300"
			}, {
				name: "原阳县",
				zip: "262300"
			}, {
				name: "延津县",
				zip: "262300"
			}, {
				name: "封丘县",
				zip: "262300"
			}, {
				name: "长垣县",
				zip: "262300"
			}, {
				name: "卫辉市",
				zip: "262300"
			}, {
				name: "辉县市",
				zip: "262300"
			}]
		}, {
			name: "焦作市",
			district: [{
				name: "解放区",
				zip: "264300"
			}, {
				name: "中站区",
				zip: "264500"
			}, {
				name: "马村区",
				zip: "264200"
			}, {
				name: "山阳区",
				zip: "264200"
			}, {
				name: "修武县",
				zip: "264200"
			}, {
				name: "博爱县",
				zip: "264200"
			}, {
				name: "武陟县",
				zip: "264200"
			}, {
				name: "温县",
				zip: "264200"
			}, {
				name: "济源市",
				zip: "264200"
			}, {
				name: "沁阳市",
				zip: "264200"
			}, {
				name: "孟州市",
				zip: "264200"
			}]
		}, {
			name: "濮阳市",
			district: [{
				name: "华龙区",
				zip: "271200"
			}, {
				name: "清丰县",
				zip: "271000"
			}, {
				name: "南乐县",
				zip: "271000"
			}, {
				name: "范县",
				zip: "271400"
			}, {
				name: "台前县",
				zip: "271500"
			}, {
				name: "濮阳县",
				zip: "271500"
			}]
		}, {
			name: "许昌市",
			district: [{
				name: "魏都区",
				zip: "261021"
			}, {
				name: "许昌县",
				zip: "261200"
			}, {
				name: "鄢陵县",
				zip: "261041"
			}, {
				name: "襄城县",
				zip: "262400"
			}, {
				name: "禹州市",
				zip: "262600"
			}, {
				name: "长葛市",
				zip: ""
			}]
		}, {
			name: "漯河市",
			district: [{
				name: "源汇区",
				zip: "273200"
			}, {
				name: "郾城区",
				zip: "272200"
			}, {
				name: "召陵区",
				zip: "272400"
			}, {
				name: "舞阳县",
				zip: "277600"
			}, {
				name: "临颍县",
				zip: "272300"
			}]
		}, {
			name: "三门峡市",
			district: [{
				name: "湖滨区",
				zip: "251900"
			}, {
				name: "绳池县",
				zip: "257100"
			}, {
				name: "陕县",
				zip: "256200"
			}, {
				name: "卢氏县",
				zip: "256800"
			}, {
				name: "义马市",
				zip: "256500"
			}, {
				name: "灵宝市",
				zip: "256600"
			}]
		}, {
			name: "南阳市",
			district: [{
				name: "宛城区",
				zip: "274100"
			}, {
				name: "卧龙区",
				zip: "274600"
			}, {
				name: "南召县",
				zip: "274700"
			}, {
				name: "方城县",
				zip: "274900"
			}, {
				name: "西峡县",
				zip: "274200"
			}, {
				name: "镇平县",
				zip: "274300"
			}, {
				name: "内乡县",
				zip: "274400"
			}, {
				name: "淅川县",
				zip: "274500"
			}, {
				name: "社旗县",
				zip: "274500"
			}, {
				name: "唐河县",
				zip: "274500"
			}, {
				name: "新野县",
				zip: "274500"
			}, {
				name: "桐柏县",
				zip: "274500"
			}, {
				name: "邓州市",
				zip: "274500"
			}]
		}, {
			name: "商丘市",
			district: [{
				name: "梁园区",
				zip: "252600"
			}, {
				name: "睢阳区",
				zip: "252500"
			}, {
				name: "民权县",
				zip: "252200"
			}, {
				name: "睢县",
				zip: "252800"
			}, {
				name: "宁陵县",
				zip: "252300"
			}, {
				name: "柘城县",
				zip: "252100"
			}, {
				name: "虞城县",
				zip: "252400"
			}, {
				name: "夏邑县",
				zip: "252400"
			}, {
				name: "永城市",
				zip: "252400"
			}]
		}, {
			name: "信阳市",
			district: [{
				name: "师河区",
				zip: "251200"
			}, {
				name: "平桥区",
				zip: "253300"
			}, {
				name: "罗山县",
				zip: "251500"
			}, {
				name: "光山县",
				zip: "251100"
			}, {
				name: "新县",
				zip: "253121"
			}, {
				name: "商城县",
				zip: "253200"
			}, {
				name: "固始县",
				zip: ""
			}, {
				name: "潢川县",
				zip: "253011"
			}, {
				name: "淮滨县",
				zip: "253700"
			}, {
				name: "息县",
				zip: "253400"
			}]
		}, {
			name: "周口市",
			district: [{
				name: "川汇区",
				zip: "271100"
			}, {
				name: "扶沟县",
				zip: "271100"
			}, {
				name: "西华县",
				zip: "271100"
			}, {
				name: "商水县",
				zip: "271100"
			}, {
				name: "沈丘县",
				zip: "271100"
			}, {
				name: "郸城县",
				zip: "271100"
			}, {
				name: "淮阳县",
				zip: "271100"
			}, {
				name: "太康县",
				zip: "271100"
			}, {
				name: "鹿邑县",
				zip: "271100"
			}, {
				name: "项城市",
				zip: "271100"
			}]
		}, {
			name: "驻马店市",
			district: [{
				name: "驿城区",
				zip: "276700"
			}, {
				name: "西平县",
				zip: "276022"
			}, {
				name: "上蔡县",
				zip: "276002"
			}, {
				name: "平舆县",
				zip: "277700"
			}, {
				name: "正阳县",
				zip: "273400"
			}, {
				name: "确山县",
				zip: "273300"
			}, {
				name: "泌阳县",
				zip: "276600"
			}, {
				name: "汝南县",
				zip: "276300"
			}, {
				name: "遂平县",
				zip: "276100"
			}, {
				name: "新蔡县",
				zip: "276400"
			}]
		}]
	},{
		name: "四川",
		city: [{
			name: "成都市",
			district: [{
				name: "锦江区",
				zip: "251400"
			}, {
				name: "青羊区",
				zip: "251600"
			}, {
				name: "金牛区",
				zip: "250100"
			}, {
				name: "武侯区",
				zip: "250300"
			}, {
				name: "成华区",
				zip: "250031"
			}, {
				name: "龙泉驿区",
				zip: "250021"
			}, {
				name: "青白江区",
				zip: "250002"
			}, {
				name: "新都区",
				zip: "250013"
			}, {
				name: "温江县",
				zip: "250200"
			}, {
				name: "金堂县",
				zip: "250200"
			}, {
				name: "双流县",
				zip: "250200"
			}, {
				name: "郫　县",
				zip: "250200"
			}, {
				name: "大邑县",
				zip: "250200"
			}, {
				name: "浦江县",
				zip: "250200"
			}, {
				name: "新津县",
				zip: "250200"
			}, {
				name: "都江堰市",
				zip: "250200"
			}, {
				name: "彭州市",
				zip: "250200"
			}, {
				name: "邛崃市",
				zip: "250200"
			}, {
				name: "崇州市",
				zip: "250200"
			}]
		}, {
			name: "自贡市",
			district: [{
				name: "自流井区",
				zip: "257029"
			}, {
				name: "贡井区",
				zip: "257500"
			}, {
				name: "大安区",
				zip: "257400"
			}, {
				name: "沿滩区",
				zip: "257300"
			}, {
				name: "荣县",
				zip: ""
			}, {
				name: "富顺县",
				zip: ""
			}]
		}, {
			name: "攀枝花市",
			district: [{
				name: "东区",
				zip: "265300"
			}, {
				name: "西区",
				zip: "265400"
			}, {
				name: "仁和区",
				zip: "265600"
			}, {
				name: "米易县",
				zip: "261400"
			}, {
				name: "盐边县",
				zip: "265200"
			}]
		}, {
			name: "泸州市",
			district: [{
				name: "江阳区",
				zip: "255022"
			}, {
				name: "纳溪区",
				zip: "256100"
			}, {
				name: "龙马潭区",
				zip: "256300"
			}, {
				name: "泸县",
				zip: "256400"
			}, {
				name: "合江县",
				zip: "255300"
			}, {
				name: "叙永县",
				zip: "255200"
			}, {
				name: "古蔺县",
				zip: "255400"
			}]
		}, {
			name: "德阳市",
			district: [{
				name: "旌阳区",
				zip: "266400"
			}, {
				name: "中江县",
				zip: "266300"
			}, {
				name: "罗江县",
				zip: "266700"
			}, {
				name: "广汉市",
				zip: "266200"
			}, {
				name: "什邡市",
				zip: ""
			}, {
				name: "绵竹市",
				zip: "266500"
			}]
		}, {
			name: "绵阳市",
			district: [{
				name: "涪城区",
				zip: "277300"
			}, {
				name: "游仙区",
				zip: "277400"
			}, {
				name: "三台县",
				zip: "277200"
			}, {
				name: "盐亭县",
				zip: "277101"
			}, {
				name: "安县",
				zip: "277000"
			}, {
				name: "梓潼县",
				zip: "277000"
			}, {
				name: "北川羌族自治县",
				zip: "277000"
			}, {
				name: "平武县",
				zip: "277000"
			}, {
				name: "江油市",
				zip: "277000"
			}]
		}, {
			name: "广元市",
			district: [{
				name: "市中区",
				zip: "276800"
			}, {
				name: "元坝区",
				zip: "276500"
			}, {
				name: "朝天区",
				zip: "262300"
			}, {
				name: "旺苍县",
				zip: "262300"
			}, {
				name: "青川县",
				zip: "262300"
			}, {
				name: "剑阁县",
				zip: "262300"
			}, {
				name: "苍溪县",
				zip: "262300"
			}]
		}, {
			name: "遂宁市",
			district: [{
				name: "船山区",
				zip: "264300"
			}, {
				name: "安居区",
				zip: "264500"
			}, {
				name: "蓬溪县",
				zip: "264200"
			}, {
				name: "射洪县",
				zip: "264200"
			}, {
				name: "大英县",
				zip: "264200"
			}]
		}, {
			name: "内江市",
			district: [{
				name: "市中区",
				zip: "271200"
			}, {
				name: "东兴区",
				zip: "271000"
			}, {
				name: "威远县",
				zip: "271000"
			}, {
				name: "资中县",
				zip: "271400"
			}, {
				name: "隆昌县",
				zip: "271500"
			}]
		}, {
			name: "乐山市",
			district: [{
				name: "市中区",
				zip: "261021"
			}, {
				name: "沙湾区",
				zip: "261200"
			}, {
				name: "五通桥区",
				zip: "261041"
			}, {
				name: "金口河区",
				zip: "262400"
			}, {
				name: "犍为县",
				zip: "262600"
			}, {
				name: "井研县",
				zip: ""
			}, {
				name: "夹江县",
				zip: ""
			}, {
				name: "峨边彝族自治县",
				zip: ""
			}, {
				name: "马边彝族自治县",
				zip: ""
			}, {
				name: "沐川县",
				zip: ""
			}, {
				name: "峨眉山市",
				zip: ""
			}]
		}, {
			name: "南充市",
			district: [{
				name: "顺庆区",
				zip: "273200"
			}, {
				name: "高坪区",
				zip: "272200"
			}, {
				name: "嘉陵区",
				zip: "272400"
			}, {
				name: "南部县",
				zip: "277600"
			}, {
				name: "营山县",
				zip: "272300"
			}, {
				name: "蓬安县",
				zip: "272300"
			}, {
				name: "仪陇县",
				zip: "272300"
			}, {
				name: "西充县",
				zip: "272300"
			}, {
				name: "阆中市",
				zip: "272300"
			}]
		}, {
			name: "眉山市",
			district: [{
				name: "东坡区",
				zip: "251900"
			}, {
				name: "仁寿县",
				zip: "257100"
			}, {
				name: "彭山县",
				zip: "256200"
			}, {
				name: "洪雅县",
				zip: "256800"
			}, {
				name: "丹棱县",
				zip: "256500"
			}, {
				name: "青神县",
				zip: "256600"
			}]
		}, {
			name: "宜宾市",
			district: [{
				name: "翠屏区",
				zip: "274100"
			}, {
				name: "宜宾县",
				zip: "274600"
			}, {
				name: "南溪县",
				zip: "274700"
			}, {
				name: "江安县",
				zip: "274900"
			}, {
				name: "长宁县",
				zip: "274200"
			}, {
				name: "高县",
				zip: "274300"
			}, {
				name: "珙　县",
				zip: "274400"
			}, {
				name: "筠连县",
				zip: "274500"
			}, {
				name: "兴文县",
				zip: "274500"
			}, {
				name: "平山县",
				zip: "274500"
			}]
		}, {
			name: "广安市",
			district: [{
				name: "广安区",
				zip: "252600"
			}, {
				name: "岳池县",
				zip: "252500"
			}, {
				name: "武胜县",
				zip: "252200"
			}, {
				name: "邻水县",
				zip: "252800"
			}, {
				name: "华莹市",
				zip: "252300"
			}]
		}, {
			name: "达州市",
			district: [{
				name: "通川区",
				zip: "251200"
			}, {
				name: "达县",
				zip: "253300"
			}, {
				name: "宣汉县",
				zip: "251500"
			}, {
				name: "开江县",
				zip: "251100"
			}, {
				name: "大竹县",
				zip: "253121"
			}, {
				name: "渠县",
				zip: "253200"
			}, {
				name: "万源市",
				zip: ""
			}]
		}, {
			name: "雅安市",
			district: [{
				name: "雨城区",
				zip: "271100"
			}, {
				name: "名山县",
				zip: "271100"
			}, {
				name: "荥经县",
				zip: "271100"
			}, {
				name: "汉源县",
				zip: "271100"
			}, {
				name: "石棉县",
				zip: "271100"
			}, {
				name: "天全县",
				zip: "271100"
			}, {
				name: "芦山县",
				zip: "271100"
			}, {
				name: "宝兴县",
				zip: "271100"
			}]
		}, {
			name: "巴中市",
			district: [{
				name: "巴州区",
				zip: "276700"
			}, {
				name: "通江县",
				zip: "276022"
			}, {
				name: "南江县",
				zip: "276002"
			}, {
				name: "平昌县",
				zip: "277700"
			}]
		}, {
			name: "资阳市",
			district: [{
				name: "雁江区",
				zip: "276700"
			}, {
				name: "安岳县",
				zip: "276022"
			}, {
				name: "乐至县",
				zip: "276002"
			}, {
				name: "简阳县",
				zip: "277700"
			}]
		}, {
			name: "阿坝藏族羌族自治州",
			district: [{
				name: "汶川县",
				zip: "276200"
			}, {
				name: "理县",
				zip: "276700"
			}, {
				name: "茂县",
				zip: "276022"
			}, {
				name: "松潘县",
				zip: "276002"
			}, {
				name: "九寨沟县",
				zip: "277700"
			}, {
				name: "金川县",
				zip: "277700"
			}, {
				name: "小金县",
				zip: "277700"
			}, {
				name: "黑水县",
				zip: "277700"
			}, {
				name: "马尔康县",
				zip: "277700"
			}, {
				name: "壤塘县",
				zip: "277700"
			}, {
				name: "阿坝县",
				zip: "277700"
			}, {
				name: "若尔盖县",
				zip: "277700"
			}, {
				name: "红原县",
				zip: "277700"
			}]
		}, {
			name: "甘孜藏族自治州",
			district: [{
				name: "康定县",
				zip: "276200"
			}, {
				name: "泸定县",
				zip: "276700"
			}, {
				name: "丹巴县",
				zip: "276022"
			}, {
				name: "九龙县",
				zip: "276002"
			}, {
				name: "雅江县",
				zip: "277700"
			}, {
				name: "道孚县",
				zip: "277700"
			}, {
				name: "炉霍县",
				zip: "277700"
			}, {
				name: "甘孜县",
				zip: "277700"
			}, {
				name: "新龙县",
				zip: "277700"
			}, {
				name: "德格县",
				zip: "277700"
			}, {
				name: "白玉县",
				zip: "277700"
			}, {
				name: "石渠县",
				zip: "277700"
			}, {
				name: "色达县",
				zip: "277700"
			}, {
				name: "理塘县",
				zip: "277700"
			}, {
				name: "巴塘县",
				zip: "277700"
			}, {
				name: "乡城县",
				zip: "277700"
			}, {
				name: "稻城县",
				zip: "277700"
			}, {
				name: "得荣县",
				zip: "277700"
			}]
		}, {
			name: "凉山彝族自治州",
			district: [{
				name: "西昌市",
				zip: "276200"
			}, {
				name: "木里藏族自治县",
				zip: "276700"
			}, {
				name: "盐源县",
				zip: "276022"
			}, {
				name: "德昌县",
				zip: "276002"
			}, {
				name: "会理县",
				zip: "277700"
			}, {
				name: "会东县",
				zip: "277700"
			}, {
				name: "宁南县",
				zip: "277700"
			}, {
				name: "普格县",
				zip: "277700"
			}, {
				name: "布拖县",
				zip: "277700"
			}, {
				name: "金阳县",
				zip: "277700"
			}, {
				name: "昭觉县",
				zip: "277700"
			}, {
				name: "喜德县",
				zip: "277700"
			}, {
				name: "冕宁县",
				zip: "277700"
			}, {
				name: "越西县",
				zip: "277700"
			}, {
				name: "甘洛县",
				zip: "277700"
			}, {
				name: "美姑县",
				zip: "277700"
			}, {
				name: "雷波县",
				zip: "277700"
			}]
		}]
	}, {
		name: "湖北",
		city: [{
			name: "宜昌市",
			district: [{
				name: "点军区",
				zip: "443006"
			}, {
				name: "猇亭区",
				zip: "443007"
			}, {
				name: "夷陵区",
				zip: "443100"
			}, {
				name: "西陵区",
				zip: "443000"
			}, {
				name: "伍家岗区",
				zip: "443001"
			}, {
				name: "长阳土家族自治县",
				zip: "443500"
			}, {
				name: "五峰土家族自治县",
				zip: "443400"
			}, {
				name: "秭归县",
				zip: "443600"
			}, {
				name: "兴山县",
				zip: "443700"
			}, {
				name: "远安县",
				zip: "444200"
			}, {
				name: "开发区",
				zip: ""
			}, {
				name: "葛洲坝区",
				zip: ""
			}, {
				name: "枝江市",
				zip: "443200"
			}, {
				name: "当阳市",
				zip: "444100"
			}, {
				name: "宜都市",
				zip: "443300"
			}]
		}, {
			name: "襄阳市",
			district: [{
				name: "襄城区",
				zip: "441021"
			}, {
				name: "樊城区",
				zip: "441001"
			}, {
				name: "襄州区",
				zip: "441100"
			}, {
				name: "保康县",
				zip: "441600"
			}, {
				name: "谷城县",
				zip: "441700"
			}, {
				name: "南漳县",
				zip: "441500"
			}, {
				name: "老河口市",
				zip: "441800"
			}, {
				name: "枣阳市",
				zip: "441200"
			}, {
				name: "宜城市",
				zip: "441400"
			}]
		}, {
			name: "鄂州市",
			district: [{
				name: "华容区",
				zip: "436030"
			}, {
				name: "梁子湖区",
				zip: "436064"
			}, {
				name: "鄂城区",
				zip: "436000"
			}]
		}, {
			name: "荆门市",
			district: [{
				name: "东宝区",
				zip: "448004"
			}, {
				name: "掇刀区",
				zip: "448124"
			}, {
				name: "沙洋县",
				zip: "448200"
			}, {
				name: "京山县",
				zip: "431800"
			}, {
				name: "钟祥市",
				zip: "431900"
			}]
		}, {
			name: "武汉市",
			district: [{
				name: "江岸区",
				zip: "430014"
			}, {
				name: "江汉区",
				zip: "430015"
			}, {
				name: "武昌区",
				zip: "430061"
			}, {
				name: "青山区",
				zip: "430080"
			}, {
				name: "硚口区",
				zip: "430033"
			}, {
				name: "汉阳区",
				zip: "430050"
			}, {
				name: "洪山区",
				zip: "430070"
			}, {
				name: "江夏区",
				zip: "430200"
			}, {
				name: "蔡甸区",
				zip: "430100"
			}, {
				name: "汉南区",
				zip: "430090"
			}, {
				name: "东西湖区",
				zip: "430040"
			}, {
				name: "新洲区",
				zip: "431400"
			}, {
				name: "黄陂区",
				zip: "430300"
			}]
		}, {
			name: "黄石市",
			district: [{
				name: "下陆区",
				zip: "435004"
			}, {
				name: "铁山区",
				zip: "435006"
			}, {
				name: "黄石港区",
				zip: "435002"
			}, {
				name: "西塞山区",
				zip: "435001"
			}, {
				name: "阳新县",
				zip: "435200"
			}, {
				name: "大冶市",
				zip: "435100"
			}]
		}, {
			name: "十堰市",
			district: [{
				name: "丹江口市",
				zip: "441900"
			}, {
				name: "城区",
				zip: ""
			}, {
				name: "房县",
				zip: "442100"
			}, {
				name: "竹溪县",
				zip: "442300"
			}, {
				name: "郧县",
				zip: "442500"
			}, {
				name: "竹山县",
				zip: "442200"
			}, {
				name: "郧西县",
				zip: "442600"
			}, {
				name: "张湾区",
				zip: "442001"
			}, {
				name: "茅箭区",
				zip: "442012"
			}]
		}, {
			name: "孝感市",
			district: [{
				name: "汉川市",
				zip: "431600"
			}, {
				name: "安陆市",
				zip: "432600"
			}, {
				name: "应城市",
				zip: "432400"
			}, {
				name: "云梦县",
				zip: "432500"
			}, {
				name: "大悟县",
				zip: "432800"
			}, {
				name: "孝昌县",
				zip: "432900"
			}, {
				name: "孝南区",
				zip: "432100"
			}]
		}, {
			name: "黄冈市",
			district: [{
				name: "黄州区",
				zip: "438000"
			}, {
				name: "麻城市",
				zip: "438300"
			}, {
				name: "武穴市",
				zip: "435400"
			}, {
				name: "红安县",
				zip: "431500"
			}, {
				name: "罗田县",
				zip: "438600"
			}, {
				name: "团风县",
				zip: "438000"
			}, {
				name: "蕲春县",
				zip: "436300"
			}, {
				name: "黄梅县",
				zip: "435500"
			}, {
				name: "英山县",
				zip: "438700"
			}, {
				name: "浠水县",
				zip: "438200"
			}]
		}, {
			name: "荆州市",
			district: [{
				name: "松滋市",
				zip: "434200"
			}, {
				name: "石首市",
				zip: "434400"
			}, {
				name: "洪湖市",
				zip: "433200"
			}, {
				name: "江陵县",
				zip: "434100"
			}, {
				name: "公安县",
				zip: "434300"
			}, {
				name: "监利县",
				zip: "433300"
			}, {
				name: "荆州区",
				zip: "434020"
			}, {
				name: "沙市区",
				zip: "434000"
			}]
		}, {
			name: "咸宁市",
			district: [{
				name: "通山县",
				zip: "437600"
			}, {
				name: "嘉鱼县",
				zip: "437200"
			}, {
				name: "通城县",
				zip: "437400"
			}, {
				name: "崇阳县",
				zip: "437500"
			}, {
				name: "咸安区",
				zip: "437000"
			}, {
				name: "赤壁市",
				zip: "437300"
			}, {
				name: "温泉城区",
				zip: ""
			}]
		}, {
			name: "随州市",
			district: [{
				name: "广水市",
				zip: "432700"
			}, {
				name: "随县",
				zip: "441300"
			}, {
				name: "曾都区",
				zip: "441300"
			}]
		}, {
			name: "恩施土家族苗族自治州",
			district: [{
				name: "鹤峰县",
				zip: "445800"
			}, {
				name: "宣恩县",
				zip: "445500"
			}, {
				name: "咸丰县",
				zip: "445600"
			}, {
				name: "来凤县",
				zip: "445700"
			}, {
				name: "建始县",
				zip: "445300"
			}, {
				name: "巴东县",
				zip: "444300"
			}, {
				name: "利川市",
				zip: "445400"
			}, {
				name: "恩施市",
				zip: "445000"
			}]
		}, {
			name: "潜江市",
			district: [{
				name: "园林办事处",
				zip: ""
			}, {
				name: "杨市办事处",
				zip: ""
			}, {
				name: "周矶办事处",
				zip: ""
			}, {
				name: "广华办事处",
				zip: ""
			}, {
				name: "泰丰办事处",
				zip: ""
			}, {
				name: "高场办事处",
				zip: ""
			}, {
				name: "竹根滩镇",
				zip: ""
			}, {
				name: "渔洋镇",
				zip: ""
			}, {
				name: "王场镇",
				zip: ""
			}, {
				name: "高石碑镇",
				zip: ""
			}, {
				name: "熊口镇",
				zip: ""
			}, {
				name: "老新镇",
				zip: ""
			}, {
				name: "浩口镇",
				zip: ""
			}, {
				name: "积玉口镇",
				zip: ""
			}, {
				name: "张金镇",
				zip: ""
			}, {
				name: "龙湾镇",
				zip: ""
			}, {
				name: "江汉石油管理局",
				zip: ""
			}, {
				name: "潜江经济开发区",
				zip: ""
			}, {
				name: "周矶管理区",
				zip: ""
			}, {
				name: "后湖管理区",
				zip: ""
			}, {
				name: "熊口管理区",
				zip: ""
			}, {
				name: "总口管理区",
				zip: ""
			}, {
				name: "白鹭湖管理区",
				zip: ""
			}, {
				name: "运粮湖管理区",
				zip: ""
			}, {
				name: "浩口原种场",
				zip: ""
			}]
		}, {
			name: "仙桃市",
			district: [{
				name: "沙嘴街道办事处",
				zip: ""
			}, {
				name: "干河街道办事处",
				zip: ""
			}, {
				name: "龙华山办事处",
				zip: ""
			}, {
				name: "郑场镇",
				zip: ""
			}, {
				name: "毛嘴镇",
				zip: ""
			}, {
				name: "豆河镇",
				zip: ""
			}, {
				name: "三伏潭镇",
				zip: ""
			}, {
				name: "胡场镇",
				zip: ""
			}, {
				name: "长倘口镇",
				zip: ""
			}, {
				name: "西流河镇",
				zip: ""
			}, {
				name: "沙湖镇",
				zip: ""
			}, {
				name: "杨林尾镇",
				zip: ""
			}, {
				name: "彭场镇",
				zip: ""
			}, {
				name: "张沟镇",
				zip: ""
			}, {
				name: "郭河镇",
				zip: ""
			}, {
				name: "沔城回族镇",
				zip: ""
			}, {
				name: "通海口镇",
				zip: ""
			}, {
				name: "陈场镇",
				zip: ""
			}, {
				name: "工业园区",
				zip: ""
			}, {
				name: "九合垸原种场",
				zip: ""
			}, {
				name: "沙湖原种场",
				zip: ""
			}, {
				name: "五湖渔场",
				zip: ""
			}, {
				name: "赵西垸林场",
				zip: ""
			}, {
				name: "畜禽良种场",
				zip: ""
			}, {
				name: "排湖风景区",
				zip: ""
			}]
		}, {
			name: "天门市",
			district: [{
				name: "竟陵街道办事处",
				zip: ""
			}, {
				name: "侨乡街道开发区",
				zip: ""
			}, {
				name: "杨林街道办事处",
				zip: ""
			}, {
				name: "多宝镇",
				zip: ""
			}, {
				name: "拖市镇",
				zip: ""
			}, {
				name: "张港镇",
				zip: ""
			}, {
				name: "蒋场镇",
				zip: ""
			}, {
				name: "汪场镇",
				zip: ""
			}, {
				name: "渔薪镇",
				zip: ""
			}, {
				name: "黄潭镇",
				zip: ""
			}, {
				name: "岳口镇",
				zip: ""
			}, {
				name: "横林镇",
				zip: ""
			}, {
				name: "彭市镇",
				zip: ""
			}, {
				name: "麻洋镇",
				zip: ""
			}, {
				name: "多祥镇",
				zip: ""
			}, {
				name: "干驿镇",
				zip: ""
			}, {
				name: "马湾镇",
				zip: ""
			}, {
				name: "卢市镇",
				zip: ""
			}, {
				name: "小板镇",
				zip: ""
			}, {
				name: "九真镇",
				zip: ""
			}, {
				name: "皂市镇",
				zip: ""
			}, {
				name: "胡市镇",
				zip: ""
			}, {
				name: "石河镇",
				zip: ""
			}, {
				name: "佛子山镇",
				zip: ""
			}, {
				name: "净潭乡",
				zip: ""
			}, {
				name: "蒋湖农场",
				zip: ""
			}, {
				name: "白茅湖农场",
				zip: ""
			}, {
				name: "沉湖管委会",
				zip: ""
			}]
		}, {
			name: "神农架林区",
			district: [{
				name: "松柏镇",
				zip: ""
			}, {
				name: "阳日镇",
				zip: ""
			}, {
				name: "木鱼镇",
				zip: ""
			}, {
				name: "红坪镇",
				zip: ""
			}, {
				name: "新华镇",
				zip: ""
			}, {
				name: "九湖镇",
				zip: ""
			}, {
				name: "宋洛乡",
				zip: ""
			}, {
				name: "下谷坪土家族乡",
				zip: ""
			}]
		}]
	}, {
		name: "湖南",
		city: [{
			name: "怀化市",
			district: [{
				name: "鹤城区",
				zip: "418000"
			}, {
				name: "中方县",
				zip: "418005"
			}, {
				name: "沅陵县",
				zip: "419600"
			}, {
				name: "辰溪县",
				zip: "419500"
			}, {
				name: "溆浦县",
				zip: "419300"
			}, {
				name: "会同县",
				zip: "418300"
			}, {
				name: "麻阳苗族自治县",
				zip: "419400"
			}, {
				name: "新晃侗族自治县",
				zip: "419200"
			}, {
				name: "芷江侗族自治县",
				zip: "419100"
			}, {
				name: "靖州苗族侗族自治县",
				zip: "418400"
			}, {
				name: "通道侗族自治县",
				zip: "418500"
			}, {
				name: "洪江市",
				zip: "418200"
			}]
		}, {
			name: "娄底市",
			district: [{
				name: "娄星区",
				zip: "417000"
			}, {
				name: "新化县",
				zip: "417600"
			}, {
				name: "双峰县",
				zip: "417700"
			}, {
				name: "冷水江市",
				zip: "417500"
			}, {
				name: "涟源市",
				zip: "417100"
			}]
		}, {
			name: "株洲市",
			district: [{
				name: "荷塘区",
				zip: "412000"
			}, {
				name: "芦淞区",
				zip: "412000"
			}, {
				name: "石峰区",
				zip: "412005"
			}, {
				name: "醴陵市",
				zip: "412200"
			}, {
				name: "天元区",
				zip: "412000"
			}, {
				name: "攸县",
				zip: "412300"
			}, {
				name: "株洲县",
				zip: "412100"
			}, {
				name: "炎陵县",
				zip: "412500"
			}, {
				name: "茶陵县",
				zip: "412400"
			}]
		}, {
			name: "长沙市",
			district: [{
				name: "浏阳市",
				zip: "410300"
			}, {
				name: "雨花区",
				zip: "410007"
			}, {
				name: "岳麓区",
				zip: "410006"
			}, {
				name: "开福区",
				zip: "410005"
			}, {
				name: "芙蓉区",
				zip: "410006"
			}, {
				name: "天心区",
				zip: "410002"
			}, {
				name: "宁乡县",
				zip: "410600"
			}, {
				name: "长沙县",
				zip: "410100"
			}, {
				name: "望城区",
				zip: "410200"
			}]
		}, {
			name: "湘潭市",
			district: [{
				name: "雨湖区",
				zip: "411100"
			}, {
				name: "岳塘区",
				zip: "411101"
			}, {
				name: "湘潭县",
				zip: "411200"
			}, {
				name: "韶山市",
				zip: "411300"
			}, {
				name: "湘乡市",
				zip: "411400"
			}]
		}, {
			name: "衡阳市",
			district: [{
				name: "衡东县",
				zip: "421400"
			}, {
				name: "祁东县",
				zip: "421600"
			}, {
				name: "衡阳县",
				zip: "421200"
			}, {
				name: "衡南县",
				zip: "421100"
			}, {
				name: "衡山县",
				zip: "421300"
			}, {
				name: "蒸湘区",
				zip: "421001"
			}, {
				name: "南岳区",
				zip: "421900"
			}, {
				name: "珠晖区",
				zip: "421002"
			}, {
				name: "石鼓区",
				zip: "421001"
			}, {
				name: "雁峰区",
				zip: "421001"
			}, {
				name: "耒阳市",
				zip: "421800"
			}, {
				name: "常宁市",
				zip: "421500"
			}]
		}, {
			name: "邵阳市",
			district: [{
				name: "新宁县",
				zip: "422700"
			}, {
				name: "城步苗族自治县",
				zip: "422500"
			}, {
				name: "武冈市",
				zip: "422400"
			}, {
				name: "绥宁县",
				zip: "422600"
			}, {
				name: "洞口县",
				zip: "422300"
			}, {
				name: "隆回县",
				zip: "422200"
			}, {
				name: "邵阳县",
				zip: "422100"
			}, {
				name: "新邵县",
				zip: "422900"
			}, {
				name: "邵东县",
				zip: "422800"
			}, {
				name: "双清区",
				zip: "422001"
			}, {
				name: "大祥区",
				zip: "422000"
			}, {
				name: "北塔区",
				zip: "422001"
			}]
		}, {
			name: "常德市",
			district: [{
				name: "鼎城区",
				zip: "415100"
			}, {
				name: "武陵区",
				zip: "415000"
			}, {
				name: "津市市",
				zip: "415400"
			}, {
				name: "汉寿县",
				zip: "415900"
			}, {
				name: "澧县",
				zip: "415500"
			}, {
				name: "安乡县",
				zip: "415600"
			}, {
				name: "石门县",
				zip: "415300"
			}, {
				name: "临澧县",
				zip: "415200"
			}, {
				name: "桃源县",
				zip: "415700"
			}]
		}, {
			name: "岳阳市",
			district: [{
				name: "临湘市",
				zip: "414300"
			}, {
				name: "汨罗市",
				zip: "414400"
			}, {
				name: "湘阴县",
				zip: "414600"
			}, {
				name: "平江县",
				zip: "410400"
			}, {
				name: "云溪区",
				zip: "414003"
			}, {
				name: "岳阳楼区",
				zip: "414000"
			}, {
				name: "华容县",
				zip: "414200"
			}, {
				name: "岳阳县",
				zip: "414100"
			}, {
				name: "君山区",
				zip: "414005"
			}]
		}, {
			name: "张家界市",
			district: [{
				name: "慈利县",
				zip: "427200"
			}, {
				name: "桑植县",
				zip: "427100"
			}, {
				name: "永定区",
				zip: "427000"
			}, {
				name: "武陵源区",
				zip: "427400"
			}]
		}, {
			name: "益阳市",
			district: [{
				name: "南县",
				zip: "413200"
			}, {
				name: "安化县",
				zip: "413500"
			}, {
				name: "桃江县",
				zip: "413400"
			}, {
				name: "资阳区",
				zip: "413000"
			}, {
				name: "赫山区",
				zip: "413002"
			}, {
				name: "沅江市",
				zip: "413100"
			}]
		}, {
			name: "永州市",
			district: [{
				name: "宁远县",
				zip: "425600"
			}, {
				name: "蓝山县",
				zip: "425800"
			}, {
				name: "道县",
				zip: "425300"
			}, {
				name: "江永县",
				zip: "425400"
			}, {
				name: "东安县",
				zip: "425900"
			}, {
				name: "双牌县",
				zip: "425200"
			}, {
				name: "祁阳县",
				zip: "426100"
			}, {
				name: "新田县",
				zip: "425700"
			}, {
				name: "江华瑶族自治县",
				zip: "425500"
			}, {
				name: "冷水滩区",
				zip: "425000"
			}, {
				name: "零陵区",
				zip: "425007"
			}]
		}, {
			name: "郴州市",
			district: [{
				name: "资兴市",
				zip: "423400"
			}, {
				name: "桂东县",
				zip: "423500"
			}, {
				name: "汝城县",
				zip: "424100"
			}, {
				name: "临武县",
				zip: "424300"
			}, {
				name: "嘉禾县",
				zip: "424500"
			}, {
				name: "安仁县",
				zip: "423600"
			}, {
				name: "宜章县",
				zip: "424200"
			}, {
				name: "永兴县",
				zip: "423300"
			}, {
				name: "桂阳县",
				zip: "424400"
			}, {
				name: "苏仙区",
				zip: "423000"
			}, {
				name: "北湖区",
				zip: "423000"
			}]
		}, {
			name: "湘西土家族苗族自治州",
			district: [{
				name: "古丈县",
				zip: "416300"
			}, {
				name: "永顺县",
				zip: "416700"
			}, {
				name: "花垣县",
				zip: "416400"
			}, {
				name: "保靖县",
				zip: "416500"
			}, {
				name: "泸溪县",
				zip: "416100"
			}, {
				name: "凤凰县",
				zip: "416200"
			}, {
				name: "龙山县",
				zip: "416800"
			}, {
				name: "吉首市",
				zip: "416000"
			}]
		}]
	}, {
		name: "广东",
		city: [{
			name: "云浮市",
			district: [{
				name: "云城区",
				zip: "527300"
			}, {
				name: "罗定市",
				zip: "527200"
			}, {
				name: "新兴县",
				zip: "527400"
			}, {
				name: "云安县",
				zip: "527500"
			}, {
				name: "郁南县",
				zip: "527100"
			}]
		}, {
			name: "揭阳市",
			district: [{
				name: "普宁市",
				zip: "515300"
			}, {
				name: "东山区",
				zip: ""
			}, {
				name: "榕城区",
				zip: "522095"
			}, {
				name: "惠来县",
				zip: "515200"
			}, {
				name: "揭西县",
				zip: "515400"
			}, {
				name: "揭东区",
				zip: "515500"
			}]
		}, {
			name: "潮州市",
			district: [{
				name: "枫溪区",
				zip: ""
			}, {
				name: "饶平县",
				zip: "515700"
			}, {
				name: "潮安区",
				zip: "515600"
			}, {
				name: "湘桥区",
				zip: "521000"
			}]
		}, {
			name: "广州市",
			district: [{
				name: "增城市",
				zip: "511300"
			}, {
				name: "东山区",
				zip: ""
			}, {
				name: "从化市",
				zip: "510900"
			}, {
				name: "黄埔区",
				zip: "510700"
			}, {
				name: "番禺区",
				zip: "511400"
			}, {
				name: "花都区",
				zip: "510800"
			}, {
				name: "南沙区",
				zip: ""
			}, {
				name: "萝岗区",
				zip: ""
			}, {
				name: "荔湾区",
				zip: "510145"
			}, {
				name: "海珠区",
				zip: "510220"
			}, {
				name: "越秀区",
				zip: "510180"
			}, {
				name: "天河区",
				zip: "510510"
			}, {
				name: "白云区",
				zip: "510440"
			}]
		}, {
			name: "深圳市",
			district: [{
				name: "盐田区",
				zip: "518083"
			}, {
				name: "南山区",
				zip: "518052"
			}, {
				name: "福田区",
				zip: "518031"
			}, {
				name: "龙岗区",
				zip: "518116"
			}, {
				name: "宝安区",
				zip: "518101"
			}, {
				name: "罗湖区",
				zip: "518002"
			}, {
				name: "坪山新区",
				zip: "518118"
			}, {
				name: "光明新区",
				zip: "518107"
			}, {
				name: "龙华新区",
				zip: "518131"
			}, {
				name: "大鹏新区",
				zip: "518108"
			}]
		}, {
			name: "韶关市",
			district: [{
				name: "南雄市",
				zip: "512400"
			}, {
				name: "乐昌市",
				zip: "512200"
			}, {
				name: "乳源瑶族自治县",
				zip: "512700"
			}, {
				name: "新丰县",
				zip: "511100"
			}, {
				name: "仁化县",
				zip: "512300"
			}, {
				name: "翁源县",
				zip: "512600"
			}, {
				name: "始兴县",
				zip: "512500"
			}, {
				name: "武江区",
				zip: "512026"
			}, {
				name: "浈江区",
				zip: "512023"
			}, {
				name: "曲江区",
				zip: "512100"
			}]
		}, {
			name: "中山市",
			district: [{
				name: "石岐区街道",
				zip: ""
			}, {
				name: "东区街道",
				zip: ""
			}, {
				name: "火炬开发区街道",
				zip: ""
			}, {
				name: "西区街道",
				zip: ""
			}, {
				name: "南区街道",
				zip: ""
			}, {
				name: "五桂山街道",
				zip: ""
			}, {
				name: "小榄镇",
				zip: ""
			}, {
				name: "黄圃镇",
				zip: ""
			}, {
				name: "民众镇",
				zip: ""
			}, {
				name: "东凤镇",
				zip: ""
			}, {
				name: "东升镇",
				zip: ""
			}, {
				name: "古镇镇",
				zip: ""
			}, {
				name: "沙溪镇",
				zip: ""
			}, {
				name: "坦洲镇",
				zip: ""
			}, {
				name: "港口镇",
				zip: ""
			}, {
				name: "三角镇",
				zip: ""
			}, {
				name: "横栏镇",
				zip: ""
			}, {
				name: "南头镇",
				zip: ""
			}, {
				name: "阜沙镇",
				zip: ""
			}, {
				name: "南朗镇",
				zip: ""
			}, {
				name: "三乡镇",
				zip: ""
			}, {
				name: "板芙镇",
				zip: ""
			}, {
				name: "大涌镇",
				zip: ""
			}, {
				name: "神湾镇",
				zip: ""
			}]
		}, {
			name: "东莞市",
			district: [{
				name: "东城街道",
				zip: ""
			}, {
				name: "南城街道",
				zip: ""
			}, {
				name: "万江街道",
				zip: ""
			}, {
				name: "莞城街道",
				zip: ""
			}, {
				name: "石碣镇",
				zip: ""
			}, {
				name: "石龙镇",
				zip: ""
			}, {
				name: "茶山镇",
				zip: ""
			}, {
				name: "石排镇",
				zip: ""
			}, {
				name: "企石镇",
				zip: ""
			}, {
				name: "横沥镇",
				zip: ""
			}, {
				name: "桥头镇",
				zip: ""
			}, {
				name: "谢岗镇",
				zip: ""
			}, {
				name: "东坑镇",
				zip: ""
			}, {
				name: "常平镇",
				zip: ""
			}, {
				name: "寮步镇",
				zip: ""
			}, {
				name: "樟木头镇",
				zip: ""
			}, {
				name: "大朗镇",
				zip: ""
			}, {
				name: "黄江镇",
				zip: ""
			}, {
				name: "清溪镇",
				zip: ""
			}, {
				name: "塘厦镇",
				zip: ""
			}, {
				name: "凤岗镇",
				zip: ""
			}, {
				name: "大岭山镇",
				zip: ""
			}, {
				name: "长安镇",
				zip: ""
			}, {
				name: "虎门镇",
				zip: ""
			}, {
				name: "厚街镇",
				zip: ""
			}, {
				name: "沙田镇",
				zip: ""
			}, {
				name: "道滘镇",
				zip: ""
			}, {
				name: "洪梅镇",
				zip: ""
			}, {
				name: "麻涌镇",
				zip: ""
			}, {
				name: "望牛墩镇",
				zip: ""
			}, {
				name: "中堂镇",
				zip: ""
			}, {
				name: "高埗镇",
				zip: ""
			}, {
				name: "松山湖管委会",
				zip: ""
			}, {
				name: "虎门港管委会",
				zip: ""
			}, {
				name: "东莞生态园",
				zip: ""
			}]
		}, {
			name: "汕尾市",
			district: [{
				name: "陆丰市",
				zip: "516500"
			}, {
				name: "城区",
				zip: "516601"
			}, {
				name: "陆河县",
				zip: "516700"
			}, {
				name: "海丰县",
				zip: "516400"
			}]
		}, {
			name: "梅州市",
			district: [{
				name: "兴宁市",
				zip: "514500"
			}, {
				name: "五华县",
				zip: "514400"
			}, {
				name: "平远县",
				zip: "514600"
			}, {
				name: "蕉岭县",
				zip: "514100"
			}, {
				name: "梅县",
				zip: "514700"
			}, {
				name: "丰顺县",
				zip: "514300"
			}, {
				name: "大埔县",
				zip: "514200"
			}, {
				name: "梅江区",
				zip: "514000"
			}]
		}, {
			name: "清远市",
			district: [{
				name: "英德市",
				zip: "513000"
			}, {
				name: "连州市",
				zip: "513400"
			}, {
				name: "清城区",
				zip: "511500"
			}, {
				name: "阳山县",
				zip: "513100"
			}, {
				name: "佛冈县",
				zip: "511600"
			}, {
				name: "连山壮族瑶族自治县",
				zip: "513200"
			}, {
				name: "连南瑶族自治县",
				zip: "513300"
			}, {
				name: "清新区",
				zip: "511800"
			}]
		}, {
			name: "阳江市",
			district: [{
				name: "阳春市",
				zip: "529600"
			}, {
				name: "江城区",
				zip: "529525"
			}, {
				name: "阳西县",
				zip: "529800"
			}, {
				name: "阳东县",
				zip: "529931"
			}]
		}, {
			name: "河源市",
			district: [{
				name: "源城区",
				zip: "517000"
			}, {
				name: "龙川县",
				zip: "517300"
			}, {
				name: "连平县",
				zip: "517100"
			}, {
				name: "紫金县",
				zip: "517400"
			}, {
				name: "和平县",
				zip: "517200"
			}, {
				name: "东源县",
				zip: "517500"
			}]
		}, {
			name: "茂名市",
			district: [{
				name: "高州市",
				zip: "525200"
			}, {
				name: "信宜市",
				zip: "525300"
			}, {
				name: "化州市",
				zip: "525100"
			}, {
				name: "电白县",
				zip: "525400"
			}, {
				name: "茂港区",
				zip: "525027"
			}, {
				name: "茂南区",
				zip: "525011"
			}]
		}, {
			name: "惠州市",
			district: [{
				name: "龙门县",
				zip: "516800"
			}, {
				name: "博罗县",
				zip: "516100"
			}, {
				name: "惠东县",
				zip: "516300"
			}, {
				name: "惠阳区",
				zip: "516200"
			}, {
				name: "惠城区",
				zip: "516001"
			}]
		}, {
			name: "肇庆市",
			district: [{
				name: "广宁县",
				zip: "526300"
			}, {
				name: "德庆县",
				zip: "526600"
			}, {
				name: "怀集县",
				zip: "526400"
			}, {
				name: "封开县",
				zip: "526500"
			}, {
				name: "四会市",
				zip: "526200"
			}, {
				name: "高要市",
				zip: "526100"
			}, {
				name: "端州区",
				zip: "526040"
			}, {
				name: "鼎湖区",
				zip: "526070"
			}]
		}, {
			name: "汕头市",
			district: [{
				name: "金平区",
				zip: "515041"
			}, {
				name: "龙湖区",
				zip: "515041"
			}, {
				name: "濠江区",
				zip: "515071"
			}, {
				name: "潮阳区",
				zip: "515100"
			}, {
				name: "潮南区",
				zip: "515144"
			}, {
				name: "澄海区",
				zip: "515800"
			}, {
				name: "南澳县",
				zip: "515900"
			}]
		}, {
			name: "珠海市",
			district: [{
				name: "金唐区",
				zip: ""
			}, {
				name: "南湾区",
				zip: ""
			}, {
				name: "香洲区",
				zip: "519000"
			}, {
				name: "斗门区",
				zip: "519100"
			}, {
				name: "金湾区",
				zip: "519090"
			}]
		}, {
			name: "湛江市",
			district: [{
				name: "雷州市",
				zip: "524200"
			}, {
				name: "吴川市",
				zip: "524500"
			}, {
				name: "廉江市",
				zip: "524400"
			}, {
				name: "赤坎区",
				zip: "524033"
			}, {
				name: "霞山区",
				zip: "524002"
			}, {
				name: "坡头区",
				zip: "524057"
			}, {
				name: "麻章区",
				zip: "524003"
			}, {
				name: "遂溪县",
				zip: "524300"
			}, {
				name: "徐闻县",
				zip: "524100"
			}]
		}, {
			name: "佛山市",
			district: [{
				name: "顺德区",
				zip: "528300"
			}, {
				name: "三水区",
				zip: "528100"
			}, {
				name: "禅城区",
				zip: "528000"
			}, {
				name: "南海区",
				zip: "528200"
			}, {
				name: "高明区",
				zip: "528500"
			}]
		}, {
			name: "江门市",
			district: [{
				name: "江海区",
				zip: "529000"
			}, {
				name: "新会区",
				zip: "529100"
			}, {
				name: "开平市",
				zip: "529300"
			}, {
				name: "台山市",
				zip: "529200"
			}, {
				name: "恩平市",
				zip: "529400"
			}, {
				name: "鹤山市",
				zip: "529700"
			}, {
				name: "蓬江区",
				zip: "529051"
			}]
		}]
	}, {
		name: "甘肃",
		city: [{
			name: "甘南藏族自治州",
			district: [{
				name: "合作市",
				zip: "747000"
			}, {
				name: "临潭县",
				zip: "747500"
			}, {
				name: "舟曲县",
				zip: "746300"
			}, {
				name: "卓尼县",
				zip: "747600"
			}, {
				name: "迭部县",
				zip: "747400"
			}, {
				name: "玛曲县",
				zip: "747300"
			}, {
				name: "碌曲县",
				zip: "747200"
			}, {
				name: "夏河县",
				zip: "747100"
			}]
		}, {
			name: "临夏回族自治州",
			district: [{
				name: "临夏市",
				zip: "731100"
			}, {
				name: "东乡族自治县",
				zip: "731400"
			}, {
				name: "积石山保安族东乡族撒拉族自治县",
				zip: "731700"
			}, {
				name: "广河县",
				zip: "731300"
			}, {
				name: "和政县",
				zip: "731200"
			}, {
				name: "康乐县",
				zip: "731500"
			}, {
				name: "永靖县",
				zip: "731600"
			}, {
				name: "临夏县",
				zip: "731800"
			}]
		}, {
			name: "金昌市",
			district: [{
				name: "金川区",
				zip: "737103"
			}, {
				name: "永昌县",
				zip: "737200"
			}]
		}, {
			name: "白银市",
			district: [{
				name: "白银区",
				zip: "730900"
			}, {
				name: "平川区",
				zip: "730910"
			}, {
				name: "靖远县",
				zip: "730600"
			}, {
				name: "会宁县",
				zip: "730700"
			}, {
				name: "景泰县",
				zip: "730400"
			}]
		}, {
			name: "天水市",
			district: [{
				name: "麦积区",
				zip: "741020"
			}, {
				name: "秦州区",
				zip: "741000"
			}, {
				name: "武山县",
				zip: "741300"
			}, {
				name: "张家川回族自治县",
				zip: "741500"
			}, {
				name: "秦安县",
				zip: "741600"
			}, {
				name: "甘谷县",
				zip: "741200"
			}, {
				name: "清水县",
				zip: "741400"
			}]
		}, {
			name: "兰州市",
			district: [{
				name: "皋兰县",
				zip: "730200"
			}, {
				name: "榆中县",
				zip: "730100"
			}, {
				name: "永登县",
				zip: "730300"
			}, {
				name: "七里河区",
				zip: "730050"
			}, {
				name: "城关区",
				zip: "730030"
			}, {
				name: "红古区",
				zip: "730080"
			}, {
				name: "安宁区",
				zip: "730070"
			}, {
				name: "西固区",
				zip: "730060"
			}]
		}, {
			name: "嘉峪关市",
			district: [{
				name: "新城镇",
				zip: ""
			}, {
				name: "峪泉镇",
				zip: ""
			}, {
				name: "文殊镇",
				zip: ""
			}, {
				name: "雄关区",
				zip: ""
			}, {
				name: "镜铁区",
				zip: ""
			}, {
				name: "长城区",
				zip: ""
			}]
		}, {
			name: "庆阳市",
			district: [{
				name: "宁县",
				zip: "745200"
			}, {
				name: "镇原县",
				zip: "744500"
			}, {
				name: "合水县",
				zip: "745400"
			}, {
				name: "正宁县",
				zip: "745300"
			}, {
				name: "庆城县",
				zip: "745100"
			}, {
				name: "华池县",
				zip: "745600"
			}, {
				name: "环县",
				zip: "745700"
			}, {
				name: "西峰区",
				zip: "745000"
			}]
		}, {
			name: "酒泉市",
			district: [{
				name: "敦煌市",
				zip: "736200"
			}, {
				name: "玉门市",
				zip: "735200"
			}, {
				name: "瓜州县",
				zip: "743000"
			}, {
				name: "肃北蒙古族自治县",
				zip: "736300"
			}, {
				name: "金塔县",
				zip: "735300"
			}, {
				name: "阿克塞哈萨克族自治县",
				zip: "736400"
			}, {
				name: "肃州区",
				zip: "735000"
			}]
		}, {
			name: "平凉市",
			district: [{
				name: "华亭县",
				zip: "744100"
			}, {
				name: "庄浪县",
				zip: "744600"
			}, {
				name: "静宁县",
				zip: "743400"
			}, {
				name: "泾川县",
				zip: "744300"
			}, {
				name: "灵台县",
				zip: "744400"
			}, {
				name: "崇信县",
				zip: "744200"
			}, {
				name: "崆峒区",
				zip: "744000"
			}]
		}, {
			name: "张掖市",
			district: [{
				name: "山丹县",
				zip: "734100"
			}, {
				name: "高台县",
				zip: "734300"
			}, {
				name: "肃南裕固族自治县",
				zip: "734400"
			}, {
				name: "临泽县",
				zip: "734200"
			}, {
				name: "民乐县",
				zip: "734500"
			}, {
				name: "甘州区",
				zip: "734000"
			}]
		}, {
			name: "武威市",
			district: [{
				name: "天祝藏族自治县",
				zip: "733200"
			}, {
				name: "古浪县",
				zip: "733100"
			}, {
				name: "民勤县",
				zip: "733300"
			}, {
				name: "凉州区",
				zip: "733000"
			}]
		}, {
			name: "陇南市",
			district: [{
				name: "武都区",
				zip: "746000"
			}, {
				name: "成县",
				zip: "742500"
			}, {
				name: "文县",
				zip: "746400"
			}, {
				name: "宕昌县",
				zip: "748500"
			}, {
				name: "康县",
				zip: "746500"
			}, {
				name: "西和县",
				zip: "742100"
			}, {
				name: "礼县",
				zip: "742200"
			}, {
				name: "徽县",
				zip: "742300"
			}, {
				name: "两当县",
				zip: "742400"
			}]
		}, {
			name: "定西市",
			district: [{
				name: "岷县",
				zip: "748400"
			}, {
				name: "漳县",
				zip: "748300"
			}, {
				name: "临洮县",
				zip: "730500"
			}, {
				name: "渭源县",
				zip: "748200"
			}, {
				name: "陇西县",
				zip: "748100"
			}, {
				name: "通渭县",
				zip: "743300"
			}, {
				name: "安定区",
				zip: "743000"
			}]
		}]
	}, {
		name: "陕西",
		city: [{
			name: "西安市",
			district: [{
				name: "新城区",
				zip: "223600"
			}, {
				name: "碑林区",
				zip: "223700"
			}, {
				name: "莲湖区",
				zip: "223800"
			}, {
				name: "灞桥区",
				zip: "223800"
			}, {
				name: "未央区",
				zip: "223800"
			}, {
				name: "雁塔区",
				zip: "223800"
			}, {
				name: "阎良区",
				zip: "223800"
			}, {
				name: "临潼区",
				zip: "223800"
			}, {
				name: "长安区",
				zip: "223800"
			}, {
				name: "蓝田县",
				zip: "223800"
			}, {
				name: "周至县",
				zip: "223800"
			}, {
				name: "卢县",
				zip: "223800"
			}, {
				name: "高陵县",
				zip: "223800"
			}]
		}, {
			name: "铜川市",
			district: [{
				name: "王益区",
				zip: "225700"
			}, {
				name: "印台区",
				zip: "214500"
			}, {
				name: "耀州区",
				zip: "225400"
			}, {
				name: "宜君县",
				zip: "225300"
			}]
		}, {
			name: "宝鸡市",
			district: [{
				name: "渭滨区",
				zip: "212400"
			}, {
				name: "金台区",
				zip: "212300"
			}, {
				name: "陈仓区",
				zip: "212001"
			}, {
				name: "凤翔县",
				zip: "212004"
			}, {
				name: "岐山县",
				zip: "212001"
			}, {
				name: "扶风县",
				zip: "212001"
			}, {
				name: "眉县",
				zip: "212001"
			}, {
				name: "陇县",
				zip: "212001"
			}, {
				name: "千阳县",
				zip: "212001"
			}, {
				name: "麟游县",
				zip: "212001"
			}, {
				name: "凤县",
				zip: "212001"
			}, {
				name: "太白县",
				zip: "212001"
			}]
		}, {
			name: "咸阳市",
			district: [{
				name: "秦都区",
				zip: ""
			}, {
				name: "杨凌区",
				zip: "225600"
			}, {
				name: "渭城区",
				zip: "211400"
			}, {
				name: "三原县",
				zip: "225800"
			}, {
				name: "泾阳县",
				zip: "225002"
			}, {
				name: "乾县",
				zip: "225100"
			}, {
				name: "礼泉县",
				zip: "225002"
			}, {
				name: "永寿县",
				zip: "225002"
			}, {
				name: "彬县",
				zip: "225002"
			}, {
				name: "长武县",
				zip: "225002"
			}, {
				name: "旬邑县",
				zip: "225002"
			}, {
				name: "淳化县",
				zip: "225002"
			}, {
				name: "武功县",
				zip: "225002"
			}, {
				name: "兴平市",
				zip: "225002"
			}]
		}, {
			name: "渭南市",
			district: [{
				name: "临渭区",
				zip: "224100"
			}, {
				name: "华县",
				zip: "224600"
			}, {
				name: "潼关县",
				zip: "224500"
			}, {
				name: "大荔县",
				zip: "224400"
			}, {
				name: "合阳县",
				zip: "224300"
			}, {
				name: "澄城县",
				zip: "224700"
			}, {
				name: "蒲城县",
				zip: "224055"
			}, {
				name: "白水县",
				zip: "224005"
			}, {
				name: "富平县",
				zip: "224005"
			}, {
				name: "韩城市",
				zip: "224005"
			}, {
				name: "华阴市",
				zip: "224005"
			}]
		}, {
			name: "延安市",
			district: [{
				name: "宝塔区",
				zip: "211600"
			}, {
				name: "延长县",
				zip: "211700"
			}, {
				name: "延川县",
				zip: "223100"
			}, {
				name: "子长县",
				zip: "223001"
			}, {
				name: "安塞县",
				zip: "223200"
			}, {
				name: "志丹县",
				zip: "223000"
			}, {
				name: "吴旗县",
				zip: "223002"
			}, {
				name: "甘泉县",
				zip: "223002"
			}, {
				name: "富县",
				zip: "223002"
			}, {
				name: "洛川县",
				zip: "223002"
			}, {
				name: "宜川县",
				zip: "223002"
			}, {
				name: "黄龙县",
				zip: "223002"
			}, {
				name: "黄陵县",
				zip: "223002"
			}]
		}, {
			name: "汉中市",
			district: [{
				name: "汉台区",
				zip: "222003"
			}, {
				name: "南郑县",
				zip: "222500"
			}, {
				name: "城固县",
				zip: "222300"
			}, {
				name: "洋县",
				zip: "222200"
			}, {
				name: "西乡县",
				zip: "222100"
			}, {
				name: "勉县",
				zip: "222042"
			}, {
				name: "宁强县",
				zip: "222042"
			}, {
				name: "略阳县",
				zip: "222042"
			}, {
				name: "镇巴县",
				zip: "222042"
			}, {
				name: "留坝县",
				zip: "222042"
			}, {
				name: "佛坪县",
				zip: "222042"
			}]
		}, {
			name: "榆林市",
			district: [{
				name: "榆阳区",
				zip: "226300"
			}, {
				name: "神木县",
				zip: "226500"
			}, {
				name: "府谷县",
				zip: "226100"
			}, {
				name: "横山县",
				zip: ""
			}, {
				name: "靖边县",
				zip: "226001"
			}, {
				name: "定边县",
				zip: "226321"
			}, {
				name: "绥德县",
				zip: "226001"
			}, {
				name: "米脂县",
				zip: "226600"
			}, {
				name: "佳县",
				zip: "226400"
			}, {
				name: "吴堡县",
				zip: "226400"
			}, {
				name: "清涧县",
				zip: "226400"
			}, {
				name: "子洲县",
				zip: "226400"
			}]
		}, {
			name: "安康市",
			district: [{
				name: "汉滨区",
				zip: "213011"
			}, {
				name: "汉阴县",
				zip: "213003"
			}, {
				name: "石泉县",
				zip: "213161"
			}, {
				name: "宁陕县",
				zip: "213001"
			}, {
				name: "紫阳县",
				zip: "213200"
			}, {
				name: "岚皋县",
				zip: "213300"
			}, {
				name: "平利县",
				zip: "213300"
			}, {
				name: "镇坪县",
				zip: "213300"
			}, {
				name: "旬阳县",
				zip: "213300"
			}, {
				name: "白河县",
				zip: "213300"
			}]
		}, {
			name: "商洛市",
			district: [{
				name: "商州区",
				zip: "215300"
			}, {
				name: "洛南县",
				zip: "215500"
			}, {
				name: "丹凤县",
				zip: "215200"
			}, {
				name: "商南县",
				zip: "215400"
			}, {
				name: "山阳县",
				zip: ""
			}, {
				name: "镇安县",
				zip: ""
			}, {
				name: "柞水县",
				zip: "215006"
			}]
		}]
	}, {
		name: "贵州",
		city: [{
			name: "安顺市",
			district: [{
				name: "西秀区",
				zip: "561000"
			}, {
				name: "镇宁布依族苗族自治县",
				zip: "561200"
			}, {
				name: "普定县",
				zip: "562100"
			}, {
				name: "平坝县",
				zip: "561100"
			}, {
				name: "紫云苗族布依族自治县",
				zip: "550800"
			}, {
				name: "关岭布依族苗族自治县",
				zip: "561300"
			}]
		}, {
			name: "六盘水市",
			district: [{
				name: "水城县",
				zip: "553600"
			}, {
				name: "盘县",
				zip: "561601"
			}, {
				name: "钟山区",
				zip: "553000"
			}, {
				name: "六枝特区",
				zip: "553400"
			}]
		}, {
			name: "遵义市",
			district: [{
				name: "遵义县",
				zip: "563100"
			}, {
				name: "绥阳县",
				zip: "563300"
			}, {
				name: "桐梓县",
				zip: "563200"
			}, {
				name: "道真仡佬族苗族自治县",
				zip: "563500"
			}, {
				name: "正安县",
				zip: "563400"
			}, {
				name: "凤冈县",
				zip: "564200"
			}, {
				name: "务川仡佬族苗族自治县",
				zip: "564300"
			}, {
				name: "余庆县",
				zip: "564400"
			}, {
				name: "湄潭县",
				zip: "564100"
			}, {
				name: "习水县",
				zip: "564600"
			}, {
				name: "仁怀市",
				zip: "564500"
			}, {
				name: "赤水市",
				zip: "564700"
			}, {
				name: "红花岗区",
				zip: "563000"
			}, {
				name: "汇川区",
				zip: "563000"
			}]
		}, {
			name: "铜仁市",
			district: [{
				name: "玉屏侗族自治县",
				zip: "554000"
			}, {
				name: "江口县",
				zip: "554400"
			}, {
				name: "石阡县",
				zip: "555100"
			}, {
				name: "思南县",
				zip: "565100"
			}, {
				name: "印江土家族苗族自治县",
				zip: "555200"
			}, {
				name: "德江县",
				zip: "565200"
			}, {
				name: "沿河土家族自治县",
				zip: "565300"
			}, {
				name: "松桃苗族自治县",
				zip: "554100"
			}, {
				name: "万山区",
				zip: "554200"
			}, {
				name: "碧江区",
				zip: "554300"
			}]
		}, {
			name: "黔西南布依族苗族自治州",
			district: [{
				name: "兴义市",
				zip: "562400"
			}, {
				name: "安龙县",
				zip: "552400"
			}, {
				name: "贞丰县",
				zip: "562200"
			}, {
				name: "晴隆县",
				zip: "561400"
			}, {
				name: "册亨县",
				zip: "552200"
			}, {
				name: "望谟县",
				zip: "552300"
			}, {
				name: "普安县",
				zip: "561500"
			}, {
				name: "兴仁县",
				zip: "562300"
			}]
		}, {
			name: "毕节市",
			district: [{
				name: "七星关区",
				zip: "551700"
			}, {
				name: "大方县",
				zip: "551600"
			}, {
				name: "黔西县",
				zip: "551500"
			}, {
				name: "纳雍县",
				zip: "553300"
			}, {
				name: "威宁彝族回族苗族自治县",
				zip: "553100"
			}, {
				name: "金沙县",
				zip: "551800"
			}, {
				name: "织金县",
				zip: "552100"
			}, {
				name: "赫章县",
				zip: "553200"
			}]
		}, {
			name: "黔东南苗族侗族自治州",
			district: [{
				name: "施秉县",
				zip: "556200"
			}, {
				name: "黄平县",
				zip: "556100"
			}, {
				name: "凯里市",
				zip: "556000"
			}, {
				name: "天柱县",
				zip: "556600"
			}, {
				name: "岑巩县",
				zip: "557800"
			}, {
				name: "镇远县",
				zip: "557700"
			}, {
				name: "三穗县",
				zip: "556500"
			}, {
				name: "黎平县",
				zip: "557300"
			}, {
				name: "台江县",
				zip: "556300"
			}, {
				name: "剑河县",
				zip: "556400"
			}, {
				name: "锦屏县",
				zip: "556700"
			}, {
				name: "麻江县",
				zip: "557600"
			}, {
				name: "雷山县",
				zip: "557100"
			}, {
				name: "从江县",
				zip: "557400"
			}, {
				name: "榕江县",
				zip: "557200"
			}, {
				name: "丹寨县",
				zip: "557500"
			}]
		}, {
			name: "黔南布依族苗族自治州",
			district: [{
				name: "福泉市",
				zip: "550500"
			}, {
				name: "都匀市",
				zip: "558000"
			}, {
				name: "瓮安县",
				zip: "550400"
			}, {
				name: "平塘县",
				zip: "558300"
			}, {
				name: "独山县",
				zip: "558200"
			}, {
				name: "贵定县",
				zip: "551300"
			}, {
				name: "荔波县",
				zip: "558400"
			}, {
				name: "三都水族自治县",
				zip: "558100"
			}, {
				name: "长顺县",
				zip: "550700"
			}, {
				name: "罗甸县",
				zip: "550100"
			}, {
				name: "惠水县",
				zip: "550600"
			}, {
				name: "龙里县",
				zip: "551200"
			}]
		}, {
			name: "贵阳市",
			district: [{
				name: "观山湖区",
				zip: ""
			}, {
				name: "清镇市",
				zip: "551400"
			}, {
				name: "开阳县",
				zip: "550300"
			}, {
				name: "修文县",
				zip: "550200"
			}, {
				name: "息烽县",
				zip: "551100"
			}, {
				name: "白云区",
				zip: "550014"
			}, {
				name: "乌当区",
				zip: "550018"
			}, {
				name: "小河区",
				zip: "550009"
			}, {
				name: "花溪区",
				zip: "550025"
			}, {
				name: "南明区",
				zip: "550002"
			}, {
				name: "云岩区",
				zip: "550001"
			}]
		}]
	}, {
		name: "海南",
		city: [{
			name: "昌江黎族自治县",
			district: [{
				name: "石碌镇",
				zip: ""
			}, {
				name: "叉河镇",
				zip: ""
			}, {
				name: "十月田镇",
				zip: ""
			}, {
				name: "乌烈镇",
				zip: ""
			}, {
				name: "昌化镇",
				zip: ""
			}, {
				name: "海尾镇",
				zip: ""
			}, {
				name: "七叉镇",
				zip: ""
			}, {
				name: "王下乡",
				zip: ""
			}, {
				name: "国营红林农场",
				zip: ""
			}, {
				name: "国营霸王岭林场",
				zip: ""
			}, {
				name: "海南矿业联合有限公司",
				zip: ""
			}]
		}, {
			name: "白沙黎族自治县",
			district: [{
				name: "牙叉镇",
				zip: ""
			}, {
				name: "七坊镇",
				zip: ""
			}, {
				name: "邦溪镇",
				zip: ""
			}, {
				name: "打安镇",
				zip: ""
			}, {
				name: "细水乡",
				zip: ""
			}, {
				name: "元门乡",
				zip: ""
			}, {
				name: "南开乡",
				zip: ""
			}, {
				name: "阜龙乡",
				zip: ""
			}, {
				name: "青松乡",
				zip: ""
			}, {
				name: "金波乡",
				zip: ""
			}, {
				name: "荣邦乡",
				zip: ""
			}, {
				name: "国营白沙农场",
				zip: ""
			}, {
				name: "国营龙江农场",
				zip: ""
			}, {
				name: "国营邦溪农场",
				zip: ""
			}]
		}, {
			name: "临高县",
			district: [{
				name: "临城镇",
				zip: ""
			}, {
				name: "波莲镇",
				zip: ""
			}, {
				name: "东英镇",
				zip: ""
			}, {
				name: "博厚镇",
				zip: ""
			}, {
				name: "皇桐镇",
				zip: ""
			}, {
				name: "多文镇",
				zip: ""
			}, {
				name: "和舍镇",
				zip: ""
			}, {
				name: "南宝镇",
				zip: ""
			}, {
				name: "新盈镇",
				zip: ""
			}, {
				name: "调楼镇",
				zip: ""
			}, {
				name: "国营红华农场",
				zip: ""
			}, {
				name: "国营加来农场",
				zip: ""
			}]
		}, {
			name: "澄迈县",
			district: [{
				name: "金江镇",
				zip: ""
			}, {
				name: "老城镇",
				zip: ""
			}, {
				name: "瑞溪镇",
				zip: ""
			}, {
				name: "永发镇",
				zip: ""
			}, {
				name: "加乐镇",
				zip: ""
			}, {
				name: "文儒镇",
				zip: ""
			}, {
				name: "中兴镇",
				zip: ""
			}, {
				name: "仁兴镇",
				zip: ""
			}, {
				name: "福山镇",
				zip: ""
			}, {
				name: "桥头镇",
				zip: ""
			}, {
				name: "大丰镇",
				zip: ""
			}, {
				name: "国营红光农场",
				zip: ""
			}, {
				name: "国营西达农场",
				zip: ""
			}, {
				name: "国营金安农场",
				zip: ""
			}]
		}, {
			name: "屯昌县",
			district: [{
				name: "屯城镇",
				zip: ""
			}, {
				name: "新兴镇",
				zip: ""
			}, {
				name: "枫木镇",
				zip: ""
			}, {
				name: "乌坡镇",
				zip: ""
			}, {
				name: "南吕镇",
				zip: ""
			}, {
				name: "南坤镇",
				zip: ""
			}, {
				name: "坡心镇",
				zip: ""
			}, {
				name: "西昌镇",
				zip: ""
			}, {
				name: "国营中建农场",
				zip: ""
			}, {
				name: "国营中坤农场",
				zip: ""
			}]
		}, {
			name: "定安县",
			district: [{
				name: "定城镇",
				zip: ""
			}, {
				name: "新竹镇",
				zip: ""
			}, {
				name: "龙湖镇",
				zip: ""
			}, {
				name: "黄竹镇",
				zip: ""
			}, {
				name: "雷鸣镇",
				zip: ""
			}, {
				name: "龙门镇",
				zip: ""
			}, {
				name: "龙河镇",
				zip: ""
			}, {
				name: "岭口镇",
				zip: ""
			}, {
				name: "翰林镇",
				zip: ""
			}, {
				name: "富文镇",
				zip: ""
			}, {
				name: "国营中瑞农场",
				zip: ""
			}, {
				name: "国营南海农场",
				zip: ""
			}, {
				name: "国营金鸡岭农场",
				zip: ""
			}]
		}, {
			name: "中沙群岛的岛礁及其海域",
			district: [{
				name: "中沙岛礁",
				zip: ""
			}]
		}, {
			name: "南沙群岛",
			district: [{
				name: "永暑岛",
				zip: ""
			}]
		}, {
			name: "西沙群岛",
			district: [{
				name: "永兴岛",
				zip: ""
			}]
		}, {
			name: "琼中黎族苗族自治县",
			district: [{
				name: "营根镇",
				zip: ""
			}, {
				name: "湾岭镇",
				zip: ""
			}, {
				name: "黎母山镇",
				zip: ""
			}, {
				name: "和平镇",
				zip: ""
			}, {
				name: "长征镇",
				zip: ""
			}, {
				name: "红毛镇",
				zip: ""
			}, {
				name: "中平镇",
				zip: ""
			}, {
				name: "吊罗山乡",
				zip: ""
			}, {
				name: "上安乡",
				zip: ""
			}, {
				name: "什运乡",
				zip: ""
			}, {
				name: "国营阳江农场",
				zip: ""
			}, {
				name: "国营乌石农场",
				zip: ""
			}, {
				name: "国营加钗农场",
				zip: ""
			}, {
				name: "国营长征农场",
				zip: ""
			}, {
				name: "国营黎母山林业公司",
				zip: ""
			}]
		}, {
			name: "保亭黎族苗族自治县",
			district: [{
				name: "保城镇",
				zip: ""
			}, {
				name: "什玲镇",
				zip: ""
			}, {
				name: "加茂镇",
				zip: ""
			}, {
				name: "响水镇",
				zip: ""
			}, {
				name: "新政镇",
				zip: ""
			}, {
				name: "三道镇",
				zip: ""
			}, {
				name: "六弓乡",
				zip: ""
			}, {
				name: "南林乡",
				zip: ""
			}, {
				name: "毛感乡",
				zip: ""
			}, {
				name: "国营新星农场",
				zip: ""
			}, {
				name: "海南保亭热带作物研究所",
				zip: ""
			}, {
				name: "国营金江农场",
				zip: ""
			}, {
				name: "国营三道农场",
				zip: ""
			}]
		}, {
			name: "陵水黎族自治县",
			district: [{
				name: "椰林镇",
				zip: ""
			}, {
				name: "光坡镇",
				zip: ""
			}, {
				name: "三才镇",
				zip: ""
			}, {
				name: "英州镇",
				zip: ""
			}, {
				name: "隆广镇",
				zip: ""
			}, {
				name: "文罗镇",
				zip: ""
			}, {
				name: "本号镇",
				zip: ""
			}, {
				name: "新村镇",
				zip: ""
			}, {
				name: "黎安镇",
				zip: ""
			}, {
				name: "提蒙乡",
				zip: ""
			}, {
				name: "群英乡",
				zip: ""
			}, {
				name: "国营岭门农场",
				zip: ""
			}, {
				name: "国营南平农场",
				zip: ""
			}, {
				name: "国营吊罗山林业公司",
				zip: ""
			}]
		}, {
			name: "乐东黎族自治县",
			district: [{
				name: "抱由镇",
				zip: ""
			}, {
				name: "万冲镇",
				zip: ""
			}, {
				name: "大安镇",
				zip: ""
			}, {
				name: "志仲镇",
				zip: ""
			}, {
				name: "千家镇",
				zip: ""
			}, {
				name: "九所镇",
				zip: ""
			}, {
				name: "利国镇",
				zip: ""
			}, {
				name: "黄流镇",
				zip: ""
			}, {
				name: "佛罗镇",
				zip: ""
			}, {
				name: "尖峰镇",
				zip: ""
			}, {
				name: "莺歌海镇",
				zip: ""
			}, {
				name: "国营山荣农场",
				zip: ""
			}, {
				name: "国营乐光农场",
				zip: ""
			}, {
				name: "国营保国农场",
				zip: ""
			}, {
				name: "国营尖峰岭林业公司",
				zip: ""
			}, {
				name: "国营莺歌海盐场",
				zip: ""
			}]
		}, {
			name: "文昌市",
			district: [{
				name: "文城镇",
				zip: ""
			}, {
				name: "重兴镇",
				zip: ""
			}, {
				name: "蓬莱镇",
				zip: ""
			}, {
				name: "会文镇",
				zip: ""
			}, {
				name: "东路镇",
				zip: ""
			}, {
				name: "潭牛镇",
				zip: ""
			}, {
				name: "东阁镇",
				zip: ""
			}, {
				name: "文教镇",
				zip: ""
			}, {
				name: "东郊镇",
				zip: ""
			}, {
				name: "龙楼镇",
				zip: ""
			}, {
				name: "昌洒镇",
				zip: ""
			}, {
				name: "翁田镇",
				zip: ""
			}, {
				name: "抱罗镇",
				zip: ""
			}, {
				name: "冯坡镇",
				zip: ""
			}, {
				name: "锦山镇",
				zip: ""
			}, {
				name: "铺前镇",
				zip: ""
			}, {
				name: "公坡镇",
				zip: ""
			}, {
				name: "国营东路农场",
				zip: ""
			}, {
				name: "国营南阳农场",
				zip: ""
			}, {
				name: "国营罗豆农场",
				zip: ""
			}]
		}, {
			name: "东方市",
			district: [{
				name: "八所镇",
				zip: ""
			}, {
				name: "东河镇",
				zip: ""
			}, {
				name: "大田镇",
				zip: ""
			}, {
				name: "感城镇",
				zip: ""
			}, {
				name: "板桥镇",
				zip: ""
			}, {
				name: "三家镇",
				zip: ""
			}, {
				name: "四更镇",
				zip: ""
			}, {
				name: "新龙镇",
				zip: ""
			}, {
				name: "天安乡",
				zip: ""
			}, {
				name: "江边乡",
				zip: ""
			}, {
				name: "国营广坝农场",
				zip: ""
			}, {
				name: "东方华侨农场",
				zip: ""
			}]
		}, {
			name: "万宁市",
			district: [{
				name: "万城镇",
				zip: ""
			}, {
				name: "龙滚镇",
				zip: ""
			}, {
				name: "和乐镇",
				zip: ""
			}, {
				name: "后安镇",
				zip: ""
			}, {
				name: "大茂镇",
				zip: ""
			}, {
				name: "东澳镇",
				zip: ""
			}, {
				name: "礼纪镇",
				zip: ""
			}, {
				name: "长丰镇",
				zip: ""
			}, {
				name: "山根镇",
				zip: ""
			}, {
				name: "北大镇",
				zip: ""
			}, {
				name: "南桥镇",
				zip: ""
			}, {
				name: "三更罗镇",
				zip: ""
			}, {
				name: "国营东兴农场",
				zip: ""
			}, {
				name: "国营东和农场",
				zip: ""
			}, {
				name: "国营新中农场",
				zip: ""
			}, {
				name: "兴隆华侨农场",
				zip: ""
			}, {
				name: "地方国营六连林场",
				zip: ""
			}]
		}, {
			name: "五指山市",
			district: [{
				name: "通什镇",
				zip: ""
			}, {
				name: "南圣镇",
				zip: ""
			}, {
				name: "毛阳镇",
				zip: ""
			}, {
				name: "番阳镇",
				zip: ""
			}, {
				name: "畅好乡",
				zip: ""
			}, {
				name: "毛道乡",
				zip: ""
			}, {
				name: "水满乡",
				zip: ""
			}, {
				name: "国营畅好农场",
				zip: ""
			}]
		}, {
			name: "儋州市",
			district: [{
				name: "那大镇",
				zip: ""
			}, {
				name: "和庆镇",
				zip: ""
			}, {
				name: "南丰镇",
				zip: ""
			}, {
				name: "大成镇",
				zip: ""
			}, {
				name: "雅星镇",
				zip: ""
			}, {
				name: "兰洋镇",
				zip: ""
			}, {
				name: "光村镇",
				zip: ""
			}, {
				name: "木棠镇",
				zip: ""
			}, {
				name: "海头镇",
				zip: ""
			}, {
				name: "峨蔓镇",
				zip: ""
			}, {
				name: "三都镇",
				zip: ""
			}, {
				name: "王五镇",
				zip: ""
			}, {
				name: "白马井镇",
				zip: ""
			}, {
				name: "中和镇",
				zip: ""
			}, {
				name: "排浦镇",
				zip: ""
			}, {
				name: "东成镇",
				zip: ""
			}, {
				name: "新州镇",
				zip: ""
			}, {
				name: "国营西培农场",
				zip: ""
			}, {
				name: "国营西联农场",
				zip: ""
			}, {
				name: "国营蓝洋农场",
				zip: ""
			}, {
				name: "国营八一农场",
				zip: ""
			}, {
				name: "洋浦经济开发区",
				zip: ""
			}, {
				name: "华南热作学院",
				zip: ""
			}]
		}, {
			name: "琼海市",
			district: [{
				name: "嘉积镇",
				zip: ""
			}, {
				name: "万泉镇",
				zip: ""
			}, {
				name: "石壁镇",
				zip: ""
			}, {
				name: "中原镇",
				zip: ""
			}, {
				name: "博鳌镇",
				zip: ""
			}, {
				name: "阳江镇",
				zip: ""
			}, {
				name: "龙江镇",
				zip: ""
			}, {
				name: "潭门镇",
				zip: ""
			}, {
				name: "塔洋镇",
				zip: ""
			}, {
				name: "长坡镇",
				zip: ""
			}, {
				name: "大路镇",
				zip: ""
			}, {
				name: "会山镇",
				zip: ""
			}, {
				name: "国营东太农场",
				zip: ""
			}, {
				name: "国营东红农场",
				zip: ""
			}, {
				name: "国营东升农场",
				zip: ""
			}, {
				name: "彬村山华侨农场",
				zip: ""
			}]
		}, {
			name: "三沙市",
			district: [{
				name: "西沙群岛",
				zip: ""
			}, {
				name: "中沙群岛的岛礁及其海域",
				zip: ""
			}, {
				name: "南沙群岛",
				zip: ""
			}]
		}, {
			name: "海口市",
			district: [{
				name: "秀英区",
				zip: "570311"
			}, {
				name: "琼山区",
				zip: "571100"
			}, {
				name: "龙华区",
				zip: "570105"
			}, {
				name: "美兰区",
				zip: "570203"
			}]
		}, {
			name: "三亚市",
			district: [{
				name: "海棠湾镇",
				zip: ""
			}, {
				name: "吉阳镇",
				zip: ""
			}, {
				name: "凤凰镇",
				zip: ""
			}, {
				name: "崖城镇",
				zip: ""
			}, {
				name: "天涯镇",
				zip: ""
			}, {
				name: "育才镇",
				zip: ""
			}, {
				name: "国营南田农场",
				zip: ""
			}, {
				name: "国营南新农场",
				zip: ""
			}, {
				name: "国营立才农场",
				zip: ""
			}, {
				name: "国营南滨农场",
				zip: ""
			}, {
				name: "河西区街道办",
				zip: ""
			}, {
				name: "河东区街道办",
				zip: ""
			}]
		}]
	}, {
		name: "云南",
		city: [{
			name: "玉溪市",
			district: [{
				name: "华宁县",
				zip: "652800"
			}, {
				name: "易门县",
				zip: "651100"
			}, {
				name: "峨山彝族自治县",
				zip: "653200"
			}, {
				name: "新平彝族傣族自治县",
				zip: "653400"
			}, {
				name: "元江哈尼族彝族傣族自治县",
				zip: "653300"
			}, {
				name: "江川县",
				zip: "652600"
			}, {
				name: "澄江县",
				zip: "652500"
			}, {
				name: "通海县",
				zip: "652700"
			}, {
				name: "红塔区",
				zip: "653100"
			}]
		}, {
			name: "曲靖市",
			district: [{
				name: "宣威市",
				zip: "655400"
			}, {
				name: "沾益县",
				zip: "655331"
			}, {
				name: "会泽县",
				zip: "654200"
			}, {
				name: "罗平县",
				zip: "655800"
			}, {
				name: "富源县",
				zip: "655500"
			}, {
				name: "陆良县",
				zip: "655600"
			}, {
				name: "师宗县",
				zip: "655700"
			}, {
				name: "马龙县",
				zip: "655100"
			}, {
				name: "麒麟区",
				zip: "655000"
			}]
		}, {
			name: "昆明市",
			district: [{
				name: "安宁市",
				zip: "650300"
			}, {
				name: "呈贡区",
				zip: "650500"
			}, {
				name: "晋宁县",
				zip: "650600"
			}, {
				name: "宜良县",
				zip: "652100"
			}, {
				name: "富民县",
				zip: "650400"
			}, {
				name: "嵩明县",
				zip: "651700"
			}, {
				name: "石林彝族自治县",
				zip: "652200"
			}, {
				name: "东川区",
				zip: "654100"
			}, {
				name: "西山区",
				zip: "650100"
			}, {
				name: "禄劝彝族苗族自治县",
				zip: "651500"
			}, {
				name: "寻甸回族彝族自治县",
				zip: "655200"
			}, {
				name: "官渡区",
				zip: "650220"
			}, {
				name: "五华区",
				zip: "650032"
			}, {
				name: "盘龙区",
				zip: "650051"
			}]
		}, {
			name: "丽江市",
			district: [{
				name: "玉龙纳西族自治县",
				zip: "674100"
			}, {
				name: "永胜县",
				zip: "674200"
			}, {
				name: "华坪县",
				zip: "674800"
			}, {
				name: "宁蒗彝族自治县",
				zip: "674300"
			}, {
				name: "古城区",
				zip: "674100"
			}]
		}, {
			name: "普洱市",
			district: [{
				name: "思茅区",
				zip: "665000"
			}, {
				name: "景东彝族自治县",
				zip: "676200"
			}, {
				name: "墨江哈尼族自治县",
				zip: "654800"
			}, {
				name: "宁洱哈尼族彝族自治县",
				zip: "665100"
			}, {
				name: "孟连傣族拉祜族佤族自治县",
				zip: "665800"
			}, {
				name: "江城哈尼族彝族自治县",
				zip: "665900"
			}, {
				name: "镇沅彝族哈尼族拉祜族自治县",
				zip: "666500"
			}, {
				name: "景谷傣族彝族自治县",
				zip: "666400"
			}, {
				name: "西盟佤族自治县",
				zip: "665700"
			}, {
				name: "澜沧拉祜族自治县",
				zip: "665600"
			}]
		}, {
			name: "临沧市",
			district: [{
				name: "双江拉祜族佤族布朗族傣族自治县",
				zip: "677300"
			}, {
				name: "镇康县",
				zip: "677700"
			}, {
				name: "沧源佤族自治县",
				zip: "677400"
			}, {
				name: "耿马傣族佤族自治县",
				zip: "677500"
			}, {
				name: "凤庆县",
				zip: "675900"
			}, {
				name: "永德县",
				zip: "677600"
			}, {
				name: "云县",
				zip: "675800"
			}, {
				name: "临翔区",
				zip: "677000"
			}]
		}, {
			name: "保山市",
			district: [{
				name: "昌宁县",
				zip: "678100"
			}, {
				name: "施甸县",
				zip: "678200"
			}, {
				name: "龙陵县",
				zip: "678300"
			}, {
				name: "腾冲县",
				zip: "679100"
			}, {
				name: "隆阳区",
				zip: "678000"
			}]
		}, {
			name: "昭通市",
			district: [{
				name: "巧家县",
				zip: "654600"
			}, {
				name: "盐津县",
				zip: "657500"
			}, {
				name: "鲁甸县",
				zip: "657100"
			}, {
				name: "昭阳区",
				zip: "657000"
			}, {
				name: "威信县",
				zip: "657900"
			}, {
				name: "彝良县",
				zip: "657600"
			}, {
				name: "水富县",
				zip: "657800"
			}, {
				name: "永善县",
				zip: "657300"
			}, {
				name: "大关县",
				zip: "657400"
			}, {
				name: "镇雄县",
				zip: "657200"
			}, {
				name: "绥江县",
				zip: "657700"
			}]
		}, {
			name: "楚雄彝族自治州",
			district: [{
				name: "楚雄市",
				zip: "675000"
			}, {
				name: "双柏县",
				zip: "675100"
			}, {
				name: "牟定县",
				zip: "675500"
			}, {
				name: "南华县",
				zip: "675200"
			}, {
				name: "姚安县",
				zip: "675300"
			}, {
				name: "大姚县",
				zip: "675400"
			}, {
				name: "永仁县",
				zip: "651400"
			}, {
				name: "元谋县",
				zip: "651300"
			}, {
				name: "武定县",
				zip: "651600"
			}, {
				name: "禄丰县",
				zip: "651200"
			}]
		}, {
			name: "德宏傣族景颇族自治州",
			district: [{
				name: "陇川县",
				zip: "678700"
			}, {
				name: "梁河县",
				zip: "679200"
			}, {
				name: "盈江县",
				zip: "679300"
			}, {
				name: "芒市",
				zip: "678400"
			}, {
				name: "瑞丽市",
				zip: "678600"
			}]
		}, {
			name: "迪庆藏族自治州",
			district: [{
				name: "德钦县",
				zip: "674500"
			}, {
				name: "维西傈僳族自治县",
				zip: "674600"
			}, {
				name: "香格里拉县",
				zip: "674400"
			}]
		}, {
			name: "怒江傈僳族自治州",
			district: [{
				name: "泸水县",
				zip: "673100"
			}, {
				name: "福贡县",
				zip: "673400"
			}, {
				name: "兰坪白族普米族自治县",
				zip: "671400"
			}, {
				name: "贡山独龙族怒族自治县",
				zip: "673500"
			}]
		}, {
			name: "文山壮族苗族自治州",
			district: [{
				name: "砚山县",
				zip: "663100"
			}, {
				name: "西畴县",
				zip: "663500"
			}, {
				name: "文山市",
				zip: "663000"
			}, {
				name: "广南县",
				zip: "663300"
			}, {
				name: "丘北县",
				zip: "663200"
			}, {
				name: "马关县",
				zip: "663700"
			}, {
				name: "麻栗坡县",
				zip: "663600"
			}, {
				name: "富宁县",
				zip: "663400"
			}]
		}, {
			name: "红河哈尼族彝族自治州",
			district: [{
				name: "开远市",
				zip: "661600"
			}, {
				name: "个旧市",
				zip: "661000"
			}, {
				name: "屏边苗族自治县",
				zip: "661200"
			}, {
				name: "蒙自市",
				zip: "661100"
			}, {
				name: "石屏县",
				zip: "662200"
			}, {
				name: "建水县",
				zip: "654300"
			}, {
				name: "泸西县",
				zip: "652400"
			}, {
				name: "弥勒市",
				zip: "652300"
			}, {
				name: "元阳县",
				zip: "662400"
			}, {
				name: "红河县",
				zip: "654400"
			}, {
				name: "金平苗族瑶族傣族自治县",
				zip: "661500"
			}, {
				name: "绿春县",
				zip: "662500"
			}, {
				name: "河口瑶族自治县",
				zip: "661300"
			}]
		}, {
			name: "大理白族自治州",
			district: [{
				name: "鹤庆县",
				zip: "671500"
			}, {
				name: "永平县",
				zip: "672600"
			}, {
				name: "云龙县",
				zip: "672700"
			}, {
				name: "洱源县",
				zip: "671200"
			}, {
				name: "剑川县",
				zip: "671300"
			}, {
				name: "祥云县",
				zip: "672100"
			}, {
				name: "漾濞彝族自治县",
				zip: "672500"
			}, {
				name: "巍山彝族回族自治县",
				zip: "672400"
			}, {
				name: "南涧彝族自治县",
				zip: "675700"
			}, {
				name: "弥渡县",
				zip: "675600"
			}, {
				name: "宾川县",
				zip: "671600"
			}, {
				name: "大理市",
				zip: "671000"
			}]
		}, {
			name: "西双版纳傣族自治州",
			district: [{
				name: "勐海县",
				zip: "666200"
			}, {
				name: "勐腊县",
				zip: "666300"
			}, {
				name: "景洪市",
				zip: "666100"
			}]
		}]
	}, {
		name: "青海",
		city: [{
			name: "西宁市",
			district: [{
				name: "大通回族土族自治县",
				zip: "810100"
			}, {
				name: "湟源县",
				zip: "812100"
			}, {
				name: "湟中县",
				zip: "811600"
			}, {
				name: "城西区",
				zip: "810001"
			}, {
				name: "城北区",
				zip: "810001"
			}, {
				name: "城东区",
				zip: "810000"
			}, {
				name: "城中区",
				zip: "810000"
			}]
		}, {
			name: "海北藏族自治州",
			district: [{
				name: "门源回族自治县",
				zip: "810300"
			}, {
				name: "祁连县",
				zip: "810400"
			}, {
				name: "海晏县",
				zip: "812200"
			}, {
				name: "刚察县",
				zip: "812300"
			}]
		}, {
			name: "黄南藏族自治州",
			district: [{
				name: "河南蒙古族自治县",
				zip: "811500"
			}, {
				name: "尖扎县",
				zip: "811200"
			}, {
				name: "泽库县",
				zip: "811400"
			}, {
				name: "同仁县",
				zip: "811300"
			}]
		}, {
			name: "海东市",
			district: [{
				name: "乐都区",
				zip: "810700"
			}, {
				name: "民和回族土族自治县",
				zip: "810800"
			}, {
				name: "平安县",
				zip: "810600"
			}, {
				name: "化隆回族自治县",
				zip: "810900"
			}, {
				name: "互助土族自治县",
				zip: "810500"
			}, {
				name: "循化撒拉族自治县",
				zip: "811100"
			}]
		}, {
			name: "海西蒙古族藏族自治州",
			district: [{
				name: "德令哈市",
				zip: "817000"
			}, {
				name: "格尔木市",
				zip: "816000"
			}, {
				name: "都兰县",
				zip: "816100"
			}, {
				name: "天峻县",
				zip: "817200"
			}, {
				name: "乌兰县",
				zip: "817100"
			}]
		}, {
			name: "果洛藏族自治州",
			district: [{
				name: "班玛县",
				zip: "814300"
			}, {
				name: "甘德县",
				zip: "814100"
			}, {
				name: "玛沁县",
				zip: "814000"
			}, {
				name: "玛多县",
				zip: "813500"
			}, {
				name: "久治县",
				zip: "624700"
			}, {
				name: "达日县",
				zip: "814200"
			}]
		}, {
			name: "玉树藏族自治州",
			district: [{
				name: "治多县",
				zip: "815400"
			}, {
				name: "囊谦县",
				zip: "815200"
			}, {
				name: "曲麻莱县",
				zip: "815500"
			}, {
				name: "玉树市",
				zip: "815000"
			}, {
				name: "杂多县",
				zip: "815300"
			}, {
				name: "称多县",
				zip: "815100"
			}]
		}, {
			name: "海南藏族自治州",
			district: [{
				name: "贵德县",
				zip: "811700"
			}, {
				name: "同德县",
				zip: "813200"
			}, {
				name: "共和县",
				zip: "813000"
			}, {
				name: "贵南县",
				zip: "813100"
			}, {
				name: "兴海县",
				zip: "813300"
			}]
		}]
	}, {
		name: "山东",
		city: [{
			name: "济南市",
			district: [{
				name: "平阴县",
				zip: "250400"
			}, {
				name: "济阳县",
				zip: "251400"
			}, {
				name: "商河县",
				zip: "251600"
			}, {
				name: "历城区",
				zip: "250100"
			}, {
				name: "长清区",
				zip: "250300"
			}, {
				name: "天桥区",
				zip: "250031"
			}, {
				name: "槐荫区",
				zip: "250021"
			}, {
				name: "市中区",
				zip: "250002"
			}, {
				name: "历下区",
				zip: "250013"
			}, {
				name: "章丘市",
				zip: "250200"
			}]
		}, {
			name: "东营市",
			district: [{
				name: "河口区",
				zip: "257200"
			}, {
				name: "东营区",
				zip: "257029"
			}, {
				name: "垦利县",
				zip: "257500"
			}, {
				name: "利津县",
				zip: "257400"
			}, {
				name: "广饶县",
				zip: "257300"
			}, {
				name: "西城区",
				zip: ""
			}, {
				name: "东城区",
				zip: ""
			}]
		}, {
			name: "烟台市",
			district: [{
				name: "海阳市",
				zip: "265100"
			}, {
				name: "栖霞市",
				zip: "265300"
			}, {
				name: "招远市",
				zip: "265400"
			}, {
				name: "蓬莱市",
				zip: "265600"
			}, {
				name: "莱州市",
				zip: "261400"
			}, {
				name: "莱阳市",
				zip: "265200"
			}, {
				name: "龙口市",
				zip: "265700"
			}, {
				name: "长岛县",
				zip: "265800"
			}, {
				name: "芝罘区",
				zip: "264001"
			}, {
				name: "福山区",
				zip: "265500"
			}, {
				name: "莱山区",
				zip: "264600"
			}, {
				name: "牟平区",
				zip: "264100"
			}]
		}, {
			name: "淄博市",
			district: [{
				name: "淄川区",
				zip: "255100"
			}, {
				name: "张店区",
				zip: "255022"
			}, {
				name: "沂源县",
				zip: "256100"
			}, {
				name: "高青县",
				zip: "256300"
			}, {
				name: "桓台县",
				zip: "256400"
			}, {
				name: "周村区",
				zip: "255300"
			}, {
				name: "博山区",
				zip: "255200"
			}, {
				name: "临淄区",
				zip: "255400"
			}]
		}, {
			name: "青岛市",
			district: [{
				name: "莱西市",
				zip: "266600"
			}, {
				name: "胶南市",
				zip: "266400"
			}, {
				name: "胶州市",
				zip: "266300"
			}, {
				name: "平度市",
				zip: "266700"
			}, {
				name: "即墨市",
				zip: "266200"
			}, {
				name: "开发区",
				zip: ""
			}, {
				name: "黄岛区",
				zip: "266500"
			}, {
				name: "李沧区",
				zip: "266100"
			}, {
				name: "崂山区",
				zip: "266100"
			}, {
				name: "城阳区",
				zip: "266041"
			}, {
				name: "市南区",
				zip: "266001"
			}, {
				name: "市北区",
				zip: "266011"
			}, {
				name: "四方区",
				zip: "266031"
			}]
		}, {
			name: "枣庄市",
			district: [{
				name: "滕州市",
				zip: "277500"
			}, {
				name: "峄城区",
				zip: "277300"
			}, {
				name: "台儿庄区",
				zip: "277400"
			}, {
				name: "山亭区",
				zip: "277200"
			}, {
				name: "市中区",
				zip: "277101"
			}, {
				name: "薛城区",
				zip: "277000"
			}]
		}, {
			name: "日照市",
			district: [{
				name: "岚山区",
				zip: "276808"
			}, {
				name: "东港区",
				zip: "276800"
			}, {
				name: "莒县",
				zip: "276500"
			}, {
				name: "五莲县",
				zip: "262300"
			}]
		}, {
			name: "威海市",
			district: [{
				name: "文登市",
				zip: "264400"
			}, {
				name: "荣成市",
				zip: "264300"
			}, {
				name: "乳山市",
				zip: "264500"
			}, {
				name: "环翠区",
				zip: "264200"
			}]
		}, {
			name: "泰安市",
			district: [{
				name: "肥城市",
				zip: "271600"
			}, {
				name: "新泰市",
				zip: "271200"
			}, {
				name: "岱岳区",
				zip: "271000"
			}, {
				name: "泰山区",
				zip: "271000"
			}, {
				name: "宁阳县",
				zip: "271400"
			}, {
				name: "东平县",
				zip: "271500"
			}]
		}, {
			name: "潍坊市",
			district: [{
				name: "寒亭区",
				zip: "261100"
			}, {
				name: "潍城区",
				zip: "261021"
			}, {
				name: "坊子区",
				zip: "261200"
			}, {
				name: "奎文区",
				zip: "261041"
			}, {
				name: "昌乐县",
				zip: "262400"
			}, {
				name: "临朐县",
				zip: "262600"
			}, {
				name: "开发区",
				zip: ""
			}, {
				name: "诸城市",
				zip: "262200"
			}, {
				name: "寿光市",
				zip: "262700"
			}, {
				name: "青州市",
				zip: "262500"
			}, {
				name: "高密市",
				zip: "261500"
			}, {
				name: "安丘市",
				zip: "262100"
			}, {
				name: "昌邑市",
				zip: "261300"
			}]
		}, {
			name: "济宁市",
			district: [{
				name: "汶上县",
				zip: "272501"
			}, {
				name: "泗水县",
				zip: "273200"
			}, {
				name: "金乡县",
				zip: "272200"
			}, {
				name: "嘉祥县",
				zip: "272400"
			}, {
				name: "微山县",
				zip: "277600"
			}, {
				name: "鱼台县",
				zip: "272300"
			}, {
				name: "梁山县",
				zip: "272600"
			}, {
				name: "兖州市",
				zip: "272000"
			}, {
				name: "邹城市",
				zip: "273500"
			}, {
				name: "曲阜市",
				zip: "273100"
			}, {
				name: "任城区",
				zip: "272113"
			}, {
				name: "市中区",
				zip: "272133"
			}]
		}, {
			name: "滨州市",
			district: [{
				name: "阳信县",
				zip: "251800"
			}, {
				name: "无棣县",
				zip: "251900"
			}, {
				name: "惠民县",
				zip: "257100"
			}, {
				name: "邹平县",
				zip: "256200"
			}, {
				name: "沾化县",
				zip: "256800"
			}, {
				name: "博兴县",
				zip: "256500"
			}, {
				name: "滨城区",
				zip: "256600"
			}]
		}, {
			name: "菏泽市",
			district: [{
				name: "牡丹区",
				zip: "274009"
			}, {
				name: "定陶县",
				zip: "274100"
			}, {
				name: "鄄城县",
				zip: "274600"
			}, {
				name: "郓城县",
				zip: "274700"
			}, {
				name: "巨野县",
				zip: "274900"
			}, {
				name: "成武县",
				zip: "274200"
			}, {
				name: "单县",
				zip: "274300"
			}, {
				name: "曹县",
				zip: "274400"
			}, {
				name: "东明县",
				zip: "274500"
			}]
		}, {
			name: "聊城市",
			district: [{
				name: "东昌府区",
				zip: "252000"
			}, {
				name: "临清市",
				zip: "252600"
			}, {
				name: "冠县",
				zip: "252500"
			}, {
				name: "东阿县",
				zip: "252200"
			}, {
				name: "高唐县",
				zip: "252800"
			}, {
				name: "阳谷县",
				zip: "252300"
			}, {
				name: "茌平县",
				zip: "252100"
			}, {
				name: "莘县",
				zip: "252400"
			}]
		}, {
			name: "德州市",
			district: [{
				name: "乐陵市",
				zip: "253600"
			}, {
				name: "禹城市",
				zip: "251200"
			}, {
				name: "武城县",
				zip: "253300"
			}, {
				name: "临邑县",
				zip: "251500"
			}, {
				name: "齐河县",
				zip: "251100"
			}, {
				name: "平原县",
				zip: "253121"
			}, {
				name: "夏津县",
				zip: "253200"
			}, {
				name: "开发区",
				zip: ""
			}, {
				name: "德城区",
				zip: "253011"
			}, {
				name: "庆云县",
				zip: "253700"
			}, {
				name: "宁津县",
				zip: "253400"
			}, {
				name: "陵县",
				zip: "253500"
			}]
		}, {
			name: "莱芜市",
			district: [{
				name: "钢城区",
				zip: "271100"
			}, {
				name: "莱城区",
				zip: "271100"
			}]
		}, {
			name: "临沂市",
			district: [{
				name: "蒙阴县",
				zip: "276200"
			}, {
				name: "临沭县",
				zip: "276700"
			}, {
				name: "罗庄区",
				zip: "276022"
			}, {
				name: "兰山区",
				zip: "276002"
			}, {
				name: "苍山县",
				zip: "277700"
			}, {
				name: "费县",
				zip: "273400"
			}, {
				name: "平邑县",
				zip: "273300"
			}, {
				name: "莒南县",
				zip: "276600"
			}, {
				name: "沂南县",
				zip: "276300"
			}, {
				name: "郯城县",
				zip: "276100"
			}, {
				name: "沂水县",
				zip: "276400"
			}, {
				name: "河东区",
				zip: "276034"
			}]
		}]
	}, {
		name: "广西",
		city: [{
			name: "崇左市",
			district: [{
				name: "凭祥市",
				zip: "532600"
			}, {
				name: "龙州县",
				zip: "532400"
			}, {
				name: "宁明县",
				zip: "532500"
			}, {
				name: "扶绥县",
				zip: "532100"
			}, {
				name: "江州区",
				zip: "532200"
			}, {
				name: "大新县",
				zip: "532300"
			}, {
				name: "天等县",
				zip: "532800"
			}]
		}, {
			name: "河池市",
			district: [{
				name: "金城江区",
				zip: "547000"
			}, {
				name: "南丹县",
				zip: "547200"
			}, {
				name: "天峨县",
				zip: "547300"
			}, {
				name: "凤山县",
				zip: "547600"
			}, {
				name: "都安瑶族自治县",
				zip: "530700"
			}, {
				name: "大化瑶族自治县",
				zip: "530800"
			}, {
				name: "东兰县",
				zip: "547400"
			}, {
				name: "罗城仫佬族自治县",
				zip: "546400"
			}, {
				name: "环江毛南族自治县",
				zip: "547100"
			}, {
				name: "巴马瑶族自治县",
				zip: "547500"
			}, {
				name: "宜州市",
				zip: "546300"
			}]
		}, {
			name: "来宾市",
			district: [{
				name: "合山市",
				zip: "546500"
			}, {
				name: "兴宾区",
				zip: "546100"
			}, {
				name: "象州县",
				zip: "545800"
			}, {
				name: "武宣县",
				zip: "545900"
			}, {
				name: "忻城县",
				zip: "546200"
			}, {
				name: "金秀瑶族自治县",
				zip: "545700"
			}]
		}, {
			name: "贺州市",
			district: [{
				name: "八步区",
				zip: "542800"
			}, {
				name: "平桂管理区",
				zip: ""
			}, {
				name: "富川瑶族自治县",
				zip: "542700"
			}, {
				name: "钟山县",
				zip: "542600"
			}, {
				name: "昭平县",
				zip: "546800"
			}]
		}, {
			name: "百色市",
			district: [{
				name: "右江区",
				zip: "533000"
			}, {
				name: "西林县",
				zip: "533500"
			}, {
				name: "隆林各族自治县",
				zip: "533400"
			}, {
				name: "乐业县",
				zip: "533200"
			}, {
				name: "田林县",
				zip: "533300"
			}, {
				name: "那坡县",
				zip: "533900"
			}, {
				name: "凌云县",
				zip: "533100"
			}, {
				name: "德保县",
				zip: "533700"
			}, {
				name: "靖西县",
				zip: "533800"
			}, {
				name: "平果县",
				zip: "531400"
			}, {
				name: "田东县",
				zip: "531500"
			}, {
				name: "田阳县",
				zip: "533600"
			}]
		}, {
			name: "玉林市",
			district: [{
				name: "北流市",
				zip: "537400"
			}, {
				name: "兴业县",
				zip: "537800"
			}, {
				name: "容县",
				zip: "537500"
			}, {
				name: "陆川县",
				zip: "537700"
			}, {
				name: "博白县",
				zip: "537600"
			}, {
				name: "福绵区",
				zip: ""
			}, {
				name: "玉州区",
				zip: "537000"
			}]
		}, {
			name: "钦州市",
			district: [{
				name: "浦北县",
				zip: "535300"
			}, {
				name: "灵山县",
				zip: "535400"
			}, {
				name: "钦北区",
				zip: "535000"
			}, {
				name: "钦南区",
				zip: "535000"
			}]
		}, {
			name: "贵港市",
			district: [{
				name: "平南县",
				zip: "537300"
			}, {
				name: "桂平市",
				zip: "537200"
			}, {
				name: "覃塘区",
				zip: "537121"
			}, {
				name: "港北区",
				zip: "537100"
			}, {
				name: "港南区",
				zip: "537132"
			}]
		}, {
			name: "防城港市",
			district: [{
				name: "港口区",
				zip: "538001"
			}, {
				name: "防城区",
				zip: "538021"
			}, {
				name: "上思县",
				zip: "535500"
			}, {
				name: "东兴市",
				zip: "538100"
			}]
		}, {
			name: "南宁市",
			district: [{
				name: "兴宁区",
				zip: "530012"
			}, {
				name: "青秀区",
				zip: "530022"
			}, {
				name: "良庆区",
				zip: "530200"
			}, {
				name: "邕宁区",
				zip: "530200"
			}, {
				name: "江南区",
				zip: "530031"
			}, {
				name: "西乡塘区",
				zip: "530001"
			}, {
				name: "隆安县",
				zip: "532700"
			}, {
				name: "武鸣县",
				zip: "530100"
			}, {
				name: "横县",
				zip: "530300"
			}, {
				name: "宾阳县",
				zip: "530400"
			}, {
				name: "上林县",
				zip: "530500"
			}, {
				name: "马山县",
				zip: "530600"
			}]
		}, {
			name: "柳州市",
			district: [{
				name: "融水苗族自治县",
				zip: "545300"
			}, {
				name: "融安县",
				zip: "545400"
			}, {
				name: "三江侗族自治县",
				zip: "545500"
			}, {
				name: "柳江县",
				zip: "545100"
			}, {
				name: "柳城县",
				zip: "545200"
			}, {
				name: "鹿寨县",
				zip: "545600"
			}, {
				name: "柳北区",
				zip: "545001"
			}, {
				name: "柳南区",
				zip: "545005"
			}, {
				name: "鱼峰区",
				zip: "545005"
			}, {
				name: "城中区",
				zip: "545001"
			}]
		}, {
			name: "桂林市",
			district: [{
				name: "叠彩区",
				zip: "541001"
			}, {
				name: "秀峰区",
				zip: "541001"
			}, {
				name: "雁山区",
				zip: "541006"
			}, {
				name: "七星区",
				zip: "541004"
			}, {
				name: "象山区",
				zip: "541002"
			}, {
				name: "全州县",
				zip: "541500"
			}, {
				name: "兴安县",
				zip: "541300"
			}, {
				name: "永福县",
				zip: "541800"
			}, {
				name: "灌阳县",
				zip: "541600"
			}, {
				name: "阳朔县",
				zip: "541900"
			}, {
				name: "临桂区",
				zip: "541100"
			}, {
				name: "灵川县",
				zip: "541200"
			}, {
				name: "恭城瑶族自治县",
				zip: "542500"
			}, {
				name: "龙胜各族自治县",
				zip: "541700"
			}, {
				name: "资源县",
				zip: "541400"
			}, {
				name: "平乐县",
				zip: "542400"
			}, {
				name: "荔浦县",
				zip: "546600"
			}]
		}, {
			name: "梧州市",
			district: [{
				name: "万秀区",
				zip: "543000"
			}, {
				name: "龙圩区",
				zip: ""
			}, {
				name: "长洲区",
				zip: "543002"
			}, {
				name: "蝶山区",
				zip: "543002"
			}, {
				name: "藤县",
				zip: "543300"
			}, {
				name: "蒙山县",
				zip: "546700"
			}, {
				name: "苍梧县",
				zip: "543100"
			}, {
				name: "岑溪市",
				zip: "543200"
			}]
		}, {
			name: "北海市",
			district: [{
				name: "海城区",
				zip: "536000"
			}, {
				name: "银海区",
				zip: "536000"
			}, {
				name: "合浦县",
				zip: "536100"
			}, {
				name: "铁山港区",
				zip: "536017"
			}]
		}]
	}, {
		name: "西藏",
		city: [{
			name: "拉萨市",
			district: [{
				name: "城关区",
				zip: "850000"
			}, {
				name: "曲水县",
				zip: "850600"
			}, {
				name: "堆龙德庆县",
				zip: "851400"
			}, {
				name: "达孜县",
				zip: "850100"
			}, {
				name: "墨竹工卡县",
				zip: "850200"
			}, {
				name: "林周县",
				zip: "851600"
			}, {
				name: "当雄县",
				zip: "851500"
			}, {
				name: "尼木县",
				zip: "851300"
			}]
		}, {
			name: "山南地区",
			district: [{
				name: "乃东县",
				zip: "856100"
			}, {
				name: "贡嘎县",
				zip: "850700"
			}, {
				name: "扎囊县",
				zip: "850800"
			}, {
				name: "桑日县",
				zip: "856200"
			}, {
				name: "琼结县",
				zip: "856800"
			}, {
				name: "曲松县",
				zip: "856300"
			}, {
				name: "措美县",
				zip: "856900"
			}, {
				name: "洛扎县",
				zip: "851200"
			}, {
				name: "加查县",
				zip: "856400"
			}, {
				name: "隆子县",
				zip: "856600"
			}, {
				name: "错那县",
				zip: "856700"
			}, {
				name: "浪卡子县",
				zip: "851100"
			}]
		}, {
			name: "昌都地区",
			district: [{
				name: "边坝县",
				zip: "855500"
			}, {
				name: "洛隆县",
				zip: "855400"
			}, {
				name: "芒康县",
				zip: "854500"
			}, {
				name: "左贡县",
				zip: "854400"
			}, {
				name: "类乌齐县",
				zip: "855600"
			}, {
				name: "丁青县",
				zip: "855700"
			}, {
				name: "察雅县",
				zip: "854300"
			}, {
				name: "八宿县",
				zip: "854600"
			}, {
				name: "昌都县",
				zip: "854000"
			}, {
				name: "江达县",
				zip: "854100"
			}, {
				name: "贡觉县",
				zip: "854200"
			}]
		}, {
			name: "日喀则地区",
			district: [{
				name: "拉孜县",
				zip: "858100"
			}, {
				name: "昂仁县",
				zip: "858500"
			}, {
				name: "定日县",
				zip: "858200"
			}, {
				name: "萨迦县",
				zip: "857800"
			}, {
				name: "南木林县",
				zip: "857100"
			}, {
				name: "江孜县",
				zip: "857400"
			}, {
				name: "亚东县",
				zip: "857600"
			}, {
				name: "吉隆县",
				zip: "858700"
			}, {
				name: "定结县",
				zip: "857900"
			}, {
				name: "仲巴县",
				zip: "858800"
			}, {
				name: "仁布县",
				zip: "857200"
			}, {
				name: "康马县",
				zip: "857500"
			}, {
				name: "谢通门县",
				zip: "858900"
			}, {
				name: "白朗县",
				zip: "857300"
			}, {
				name: "日喀则市",
				zip: "857000"
			}, {
				name: "聂拉木县",
				zip: "858300"
			}, {
				name: "萨嘎县",
				zip: "858600"
			}, {
				name: "岗巴县",
				zip: "857700"
			}]
		}, {
			name: "那曲地区",
			district: [{
				name: "双湖县",
				zip: ""
			}, {
				name: "巴青县",
				zip: "852100"
			}, {
				name: "班戈县",
				zip: "852500"
			}, {
				name: "尼玛县",
				zip: "852600"
			}, {
				name: "安多县",
				zip: "853400"
			}, {
				name: "聂荣县",
				zip: "853500"
			}, {
				name: "索县",
				zip: "852200"
			}, {
				name: "申扎县",
				zip: "853100"
			}, {
				name: "那曲县",
				zip: "852000"
			}, {
				name: "比如县",
				zip: "852300"
			}, {
				name: "嘉黎县",
				zip: "852400"
			}]
		}, {
			name: "阿里地区",
			district: [{
				name: "普兰县",
				zip: "859500"
			}, {
				name: "札达县",
				zip: "859600"
			}, {
				name: "噶尔县",
				zip: "859400"
			}, {
				name: "日土县",
				zip: "859700"
			}, {
				name: "革吉县",
				zip: "859100"
			}, {
				name: "改则县",
				zip: "859200"
			}, {
				name: "措勤县",
				zip: "859300"
			}]
		}, {
			name: "林芝地区",
			district: [{
				name: "墨脱县",
				zip: "860700"
			}, {
				name: "波密县",
				zip: "860300"
			}, {
				name: "察隅县",
				zip: "860600"
			}, {
				name: "朗县",
				zip: "860400"
			}, {
				name: "米林县",
				zip: "860500"
			}, {
				name: "工布江达县",
				zip: "860200"
			}, {
				name: "林芝县",
				zip: "860100"
			}]
		}]
	}, {
		name: "宁夏",
		city: [{
			name: "吴忠市",
			district: [{
				name: "盐池县",
				zip: "751500"
			}, {
				name: "同心县",
				zip: "751300"
			}, {
				name: "青铜峡市",
				zip: "751600"
			}, {
				name: "利通区",
				zip: "751100"
			}, {
				name: "红寺堡区",
				zip: "751900"
			}]
		}, {
			name: "中卫市",
			district: [{
				name: "中宁县",
				zip: "751200"
			}, {
				name: "海原县",
				zip: "756100"
			}, {
				name: "沙坡头区",
				zip: "751700"
			}]
		}, {
			name: "固原市",
			district: [{
				name: "原州区",
				zip: "756000"
			}, {
				name: "隆德县",
				zip: "756300"
			}, {
				name: "西吉县",
				zip: "756200"
			}, {
				name: "彭阳县",
				zip: "756500"
			}, {
				name: "泾源县",
				zip: "756400"
			}]
		}, {
			name: "银川市",
			district: [{
				name: "贺兰县",
				zip: "750200"
			}, {
				name: "永宁县",
				zip: "750100"
			}, {
				name: "金凤区",
				zip: "750011"
			}, {
				name: "兴庆区",
				zip: "750001"
			}, {
				name: "西夏区",
				zip: "750027"
			}, {
				name: "灵武市",
				zip: "751400"
			}]
		}, {
			name: "石嘴山市",
			district: [{
				name: "平罗县",
				zip: "753400"
			}, {
				name: "大武口区",
				zip: "753000"
			}, {
				name: "惠农区",
				zip: "753200"
			}]
		}]
	}, {
		name: "新疆",
		city: [{
			name: "克拉玛依市",
			district: [{
				name: "白碱滩区",
				zip: "834009"
			}, {
				name: "乌尔禾区",
				zip: "834014"
			}, {
				name: "独山子区",
				zip: "838600"
			}, {
				name: "克拉玛依区",
				zip: "834000"
			}]
		}, {
			name: "乌鲁木齐市",
			district: [{
				name: "乌鲁木齐县",
				zip: "830063"
			}, {
				name: "沙依巴克区",
				zip: "830000"
			}, {
				name: "天山区",
				zip: "830002"
			}, {
				name: "米东区",
				zip: "831400"
			}, {
				name: "东山区",
				zip: "830019"
			}, {
				name: "达坂城区",
				zip: "830039"
			}, {
				name: "头屯河区",
				zip: "830023"
			}, {
				name: "水磨沟区",
				zip: "830017"
			}, {
				name: "新市区",
				zip: "830011"
			}]
		}, {
			name: "伊犁哈萨克自治州",
			district: [{
				name: "尼勒克县",
				zip: "835700"
			}, {
				name: "昭苏县",
				zip: "835600"
			}, {
				name: "特克斯县",
				zip: "835500"
			}, {
				name: "巩留县",
				zip: "835400"
			}, {
				name: "新源县",
				zip: "835800"
			}, {
				name: "察布查尔锡伯自治县",
				zip: "835300"
			}, {
				name: "霍城县",
				zip: "835200"
			}, {
				name: "伊宁县",
				zip: "835100"
			}, {
				name: "奎屯市",
				zip: "833200"
			}, {
				name: "伊宁市",
				zip: "835000"
			}]
		}, {
			name: "阿勒泰地区",
			district: [{
				name: "阿勒泰市",
				zip: "836500"
			}, {
				name: "吉木乃县",
				zip: "836800"
			}, {
				name: "青河县",
				zip: "836200"
			}, {
				name: "哈巴河县",
				zip: "836700"
			}, {
				name: "福海县",
				zip: "836400"
			}, {
				name: "富蕴县",
				zip: "836100"
			}, {
				name: "布尔津县",
				zip: "836600"
			}]
		}, {
			name: "塔城地区",
			district: [{
				name: "裕民县",
				zip: "834800"
			}, {
				name: "托里县",
				zip: "834500"
			}, {
				name: "和布克赛尔蒙古自治县",
				zip: "834400"
			}, {
				name: "额敏县",
				zip: "834600"
			}, {
				name: "沙湾县",
				zip: "832100"
			}, {
				name: "乌苏市",
				zip: "833000"
			}, {
				name: "塔城市",
				zip: "834700"
			}]
		}, {
			name: "昌吉回族自治州",
			district: [{
				name: "木垒哈萨克自治县",
				zip: "831900"
			}, {
				name: "奇台县",
				zip: "831800"
			}, {
				name: "玛纳斯县",
				zip: "832200"
			}, {
				name: "吉木萨尔县",
				zip: "831700"
			}, {
				name: "呼图壁县",
				zip: "831200"
			}, {
				name: "米泉市",
				zip: "831400"
			}, {
				name: "阜康市",
				zip: "831500"
			}, {
				name: "昌吉市",
				zip: "831100"
			}]
		}, {
			name: "博尔塔拉蒙古自治州",
			district: [{
				name: "温泉县",
				zip: "833500"
			}, {
				name: "精河县",
				zip: "833300"
			}, {
				name: "博乐市",
				zip: "833400"
			}, {
				name: "阿拉山口市",
				zip: ""
			}]
		}, {
			name: "巴音郭楞蒙古自治州",
			district: [{
				name: "焉耆回族自治县",
				zip: "841100"
			}, {
				name: "和静县",
				zip: "841300"
			}, {
				name: "若羌县",
				zip: "841800"
			}, {
				name: "且末县",
				zip: "841900"
			}, {
				name: "和硕县",
				zip: "841200"
			}, {
				name: "博湖县",
				zip: "841400"
			}, {
				name: "轮台县",
				zip: "841600"
			}, {
				name: "尉犁县",
				zip: "841500"
			}, {
				name: "库尔勒市",
				zip: "841000"
			}]
		}, {
			name: "阿克苏地区",
			district: [{
				name: "沙雅县",
				zip: "842200"
			}, {
				name: "新和县",
				zip: "842100"
			}, {
				name: "拜城县",
				zip: "842300"
			}, {
				name: "乌什县",
				zip: "843400"
			}, {
				name: "温宿县",
				zip: "843100"
			}, {
				name: "库车县",
				zip: "842000"
			}, {
				name: "阿克苏市",
				zip: "843000"
			}, {
				name: "阿瓦提县",
				zip: "843200"
			}, {
				name: "柯坪县",
				zip: "843600"
			}]
		}, {
			name: "克孜勒苏柯尔克孜自治州",
			district: [{
				name: "阿合奇县",
				zip: "843500"
			}, {
				name: "阿克陶县",
				zip: "845550"
			}, {
				name: "阿图什市",
				zip: "845350"
			}, {
				name: "乌恰县",
				zip: "845450"
			}]
		}, {
			name: "喀什地区",
			district: [{
				name: "喀什市",
				zip: "844000"
			}, {
				name: "伽师县",
				zip: "844300"
			}, {
				name: "岳普湖县",
				zip: "844400"
			}, {
				name: "塔什库尔干塔吉克自治县",
				zip: "845250"
			}, {
				name: "巴楚县",
				zip: "843800"
			}, {
				name: "莎车县",
				zip: "844700"
			}, {
				name: "泽普县",
				zip: "844800"
			}, {
				name: "麦盖提县",
				zip: "844600"
			}, {
				name: "叶城县",
				zip: "844900"
			}, {
				name: "疏附县",
				zip: "844100"
			}, {
				name: "英吉沙县",
				zip: "844500"
			}, {
				name: "疏勒县",
				zip: "844200"
			}]
		}, {
			name: "和田地区",
			district: [{
				name: "和田市",
				zip: "848000"
			}, {
				name: "墨玉县",
				zip: "848100"
			}, {
				name: "皮山县",
				zip: "845150"
			}, {
				name: "和田县",
				zip: "848000"
			}, {
				name: "于田县",
				zip: "848400"
			}, {
				name: "民丰县",
				zip: "848500"
			}, {
				name: "洛浦县",
				zip: "848200"
			}, {
				name: "策勒县",
				zip: "848300"
			}]
		}, {
			name: "吐鲁番地区",
			district: [{
				name: "吐鲁番市",
				zip: "838000"
			}, {
				name: "鄯善县",
				zip: "838200"
			}, {
				name: "托克逊县",
				zip: "838100"
			}]
		}, {
			name: "哈密地区",
			district: [{
				name: "哈密市",
				zip: "839000"
			}, {
				name: "伊吾县",
				zip: "839300"
			}, {
				name: "巴里坤哈萨克自治县",
				zip: "839200"
			}]
		}, {
			name: "五家渠市",
			district: [{
				name: "军垦路街道办事处",
				zip: ""
			}, {
				name: "青湖路街道办事处",
				zip: ""
			}, {
				name: "人民路街道办事处",
				zip: ""
			}, {
				name: "兵团一零一团",
				zip: ""
			}, {
				name: "兵团一零二团",
				zip: ""
			}, {
				name: "兵团一零三团",
				zip: ""
			}]
		}, {
			name: "石河子市",
			district: [{
				name: "新城街道办事处",
				zip: ""
			}, {
				name: "向阳街道办事处",
				zip: ""
			}, {
				name: "红山街道办事处",
				zip: ""
			}, {
				name: "老街街道办事处",
				zip: ""
			}, {
				name: "东城街道办事处",
				zip: ""
			}, {
				name: "北泉镇",
				zip: ""
			}, {
				name: "石河子乡",
				zip: ""
			}, {
				name: "兵团一五二团",
				zip: ""
			}]
		}, {
			name: "阿拉尔市",
			district: [{
				name: "金银川路街道办事处",
				zip: ""
			}, {
				name: "幸福路街道办事处",
				zip: ""
			}, {
				name: "青松路街道办事处",
				zip: ""
			}, {
				name: "南口街道办事处",
				zip: ""
			}, {
				name: "托喀依乡",
				zip: ""
			}, {
				name: "工业园区",
				zip: ""
			}, {
				name: "兵团七团",
				zip: ""
			}, {
				name: "兵团八团",
				zip: ""
			}, {
				name: "兵团十团",
				zip: ""
			}, {
				name: "兵团十一团",
				zip: ""
			}, {
				name: "兵团十二团",
				zip: ""
			}, {
				name: "兵团十三团",
				zip: ""
			}, {
				name: "兵团十四团",
				zip: ""
			}, {
				name: "兵团十六团",
				zip: ""
			}, {
				name: "兵团第一师水利水电工程处",
				zip: ""
			}, {
				name: "兵团第一师塔里木灌区水利管理处",
				zip: ""
			}, {
				name: "阿拉尔农场",
				zip: ""
			}, {
				name: "兵团第一师幸福农场",
				zip: ""
			}, {
				name: "中心监狱",
				zip: ""
			}]
		}, {
			name: "图木舒克市",
			district: [{
				name: "齐干却勒街道办事处",
				zip: ""
			}, {
				name: "前海街道办事处",
				zip: ""
			}, {
				name: "永安坝街道办事处",
				zip: ""
			}, {
				name: "兵团四十四团",
				zip: ""
			}, {
				name: "兵团四十九团",
				zip: ""
			}, {
				name: "兵团五十团",
				zip: ""
			}, {
				name: "兵团五十一团",
				zip: ""
			}, {
				name: "兵团五十三团",
				zip: ""
			}, {
				name: "兵团图木舒克市喀拉拜勒镇",
				zip: ""
			}, {
				name: "兵团图木舒克市永安坝",
				zip: ""
			}]
		}]
	}, {
		name: "内蒙古",
		city: [{
			name: "呼和浩特市",
			district: [{
				name: "回民区",
				zip: "010030"
			}, {
				name: "新城区",
				zip: "010050"
			}, {
				name: "赛罕区",
				zip: "010020"
			}, {
				name: "玉泉区",
				zip: "010020"
			}, {
				name: "清水河县",
				zip: "011600"
			}, {
				name: "武川县",
				zip: "011700"
			}, {
				name: "土默特左旗",
				zip: "010100"
			}, {
				name: "托克托县",
				zip: "010200"
			}, {
				name: "和林格尔县",
				zip: "011500"
			}]
		}, {
			name: "包头市",
			district: [{
				name: "东河区",
				zip: "014040"
			}, {
				name: "昆都仑区",
				zip: "014010"
			}, {
				name: "青山区",
				zip: "014030"
			}, {
				name: "石拐区",
				zip: "014070"
			}, {
				name: "白云鄂博矿区",
				zip: "014080"
			}, {
				name: "九原区",
				zip: "014060"
			}, {
				name: "达尔罕茂明安联合旗",
				zip: "014500"
			}, {
				name: "固阳县",
				zip: "014200"
			}, {
				name: "土默特右旗",
				zip: "014100"
			}]
		}, {
			name: "乌海市",
			district: [{
				name: "海南区",
				zip: "016030"
			}, {
				name: "海勃湾区",
				zip: "016000"
			}, {
				name: "乌达区",
				zip: "016040"
			}]
		}, {
			name: "赤峰市",
			district: [{
				name: "元宝山区",
				zip: "024076"
			}, {
				name: "红山区",
				zip: "024020"
			}, {
				name: "松山区",
				zip: "024005"
			}, {
				name: "林西县",
				zip: "025250"
			}, {
				name: "克什克腾旗",
				zip: "025350"
			}, {
				name: "翁牛特旗",
				zip: "024500"
			}, {
				name: "喀喇沁旗",
				zip: "024400"
			}, {
				name: "宁城县",
				zip: "024200"
			}, {
				name: "敖汉旗",
				zip: "024300"
			}, {
				name: "阿鲁科尔沁旗",
				zip: "025550"
			}, {
				name: "巴林左旗",
				zip: "025450"
			}, {
				name: "巴林右旗",
				zip: "025150"
			}]
		}, {
			name: "通辽市",
			district: [{
				name: "科尔沁区",
				zip: "028000"
			}, {
				name: "扎鲁特旗",
				zip: "029100"
			}, {
				name: "库伦旗",
				zip: "028200"
			}, {
				name: "奈曼旗",
				zip: "028300"
			}, {
				name: "科尔沁左翼后旗",
				zip: "028100"
			}, {
				name: "开鲁县",
				zip: "028400"
			}, {
				name: "科尔沁左翼中旗",
				zip: "029300"
			}, {
				name: "霍林郭勒市",
				zip: "029200"
			}]
		}, {
			name: "鄂尔多斯市",
			district: [{
				name: "鄂托克旗",
				zip: "016100"
			}, {
				name: "杭锦旗",
				zip: "017400"
			}, {
				name: "乌审旗",
				zip: "017300"
			}, {
				name: "伊金霍洛旗",
				zip: "017200"
			}, {
				name: "鄂托克前旗",
				zip: "016200"
			}, {
				name: "准格尔旗",
				zip: "017100"
			}, {
				name: "达拉特旗",
				zip: "014300"
			}, {
				name: "东胜区",
				zip: "017000"
			}]
		}, {
			name: "呼伦贝尔市",
			district: [{
				name: "满洲里市",
				zip: "021400"
			}, {
				name: "牙克石市",
				zip: "162650"
			}, {
				name: "扎兰屯市",
				zip: "162650"
			}, {
				name: "鄂伦春自治旗",
				zip: "165450"
			}, {
				name: "莫力达瓦达斡尔族自治旗",
				zip: "162850"
			}, {
				name: "阿荣旗",
				zip: "162750"
			}, {
				name: "新巴尔虎右旗",
				zip: "021300"
			}, {
				name: "新巴尔虎左旗",
				zip: "021200"
			}, {
				name: "陈巴尔虎旗",
				zip: "021500"
			}, {
				name: "鄂温克族自治旗",
				zip: "021100"
			}, {
				name: "扎赉诺尔区",
				zip: ""
			}, {
				name: "海拉尔区",
				zip: "021000"
			}, {
				name: "额尔古纳市",
				zip: "022250"
			}, {
				name: "根河市",
				zip: "022350"
			}]
		}, {
			name: "乌兰察布市",
			district: [{
				name: "集宁区",
				zip: "012000"
			}, {
				name: "丰镇市",
				zip: "012100"
			}, {
				name: "凉城县",
				zip: "013750"
			}, {
				name: "兴和县",
				zip: "013650"
			}, {
				name: "察哈尔右翼中旗",
				zip: "013550"
			}, {
				name: "察哈尔右翼前旗",
				zip: "012200"
			}, {
				name: "卓资县",
				zip: "012300"
			}, {
				name: "商都县",
				zip: "013400"
			}, {
				name: "化德县",
				zip: "013350"
			}, {
				name: "察哈尔右翼后旗",
				zip: "012400"
			}, {
				name: "四子王旗",
				zip: "011800"
			}]
		}, {
			name: "巴彦淖尔市",
			district: [{
				name: "杭锦后旗",
				zip: "015400"
			}, {
				name: "乌拉特中旗",
				zip: "015300"
			}, {
				name: "乌拉特后旗",
				zip: "015500"
			}, {
				name: "磴口县",
				zip: "015200"
			}, {
				name: "乌拉特前旗",
				zip: "015400"
			}, {
				name: "五原县",
				zip: "015100"
			}, {
				name: "临河区",
				zip: "015001"
			}]
		}, {
			name: "锡林郭勒盟",
			district: [{
				name: "锡林浩特市",
				zip: "026000"
			}, {
				name: "二连浩特市",
				zip: "011100"
			}, {
				name: "正镶白旗",
				zip: "013800"
			}, {
				name: "镶黄旗",
				zip: "013250"
			}, {
				name: "多伦县",
				zip: "027300"
			}, {
				name: "正蓝旗",
				zip: "027200"
			}, {
				name: "苏尼特右旗",
				zip: "011200"
			}, {
				name: "东乌珠穆沁旗",
				zip: "026300"
			}, {
				name: "西乌珠穆沁旗",
				zip: "026200"
			}, {
				name: "太仆寺旗",
				zip: "027000"
			}, {
				name: "阿巴嘎旗",
				zip: "011400"
			}, {
				name: "苏尼特左旗",
				zip: "011300"
			}]
		}, {
			name: "兴安盟",
			district: [{
				name: "突泉县",
				zip: "137500"
			}, {
				name: "阿尔山市",
				zip: "137800"
			}, {
				name: "乌兰浩特市",
				zip: "137400"
			}, {
				name: "扎赉特旗",
				zip: "137600"
			}, {
				name: "科尔沁右翼中旗",
				zip: "029400"
			}, {
				name: "科尔沁右翼前旗",
				zip: "137400"
			}]
		}, {
			name: "阿拉善盟",
			district: [{
				name: "阿拉善左旗",
				zip: "750300"
			}, {
				name: "阿拉善右旗",
				zip: "737300"
			}, {
				name: "额济纳旗",
				zip: "735400"
			}]
		}]
	}, {
		name: "香港",
		city: [{
			name: "新界",
			district: [{
				name: "北区",
				zip: "810301"
			}, {
				name: "沙田区",
				zip: "810303"
			}, {
				name: "大埔区",
				zip: "810302"
			}, {
				name: "葵青区",
				zip: "810308"
			}, {
				name: "离岛区",
				zip: "810309"
			}, {
				name: "屯门区",
				zip: "810306"
			}, {
				name: "荃湾区",
				zip: "810307"
			}, {
				name: "西贡区",
				zip: "810304"
			}, {
				name: "元朗区",
				zip: "810305"
			}]
		}, {
			name: "九龙",
			district: [{
				name: "黄大仙区",
				zip: "810204"
			}, {
				name: "观塘区",
				zip: "810205"
			}, {
				name: "九龙城区",
				zip: "810201"
			}, {
				name: "油尖旺区",
				zip: "810202"
			}, {
				name: "深水埗区",
				zip: "810203"
			}]
		}, {
			name: "香港岛",
			district: [{
				name: "东区",
				zip: "810103"
			}, {
				name: "湾仔",
				zip: "810102"
			}, {
				name: "中西区",
				zip: "810101"
			}, {
				name: "南区",
				zip: "810104"
			}]
		}]
	}, {
		name: "澳门",
		city: [{
			name: "离岛",
			district: []
		}, {
			name: "澳门半岛",
			district: []
		}]
	}, {
		name: "台湾",
		city: [{
			name: "台北",
			district: []
		}, {
			name: "台中",
			district: []
		}, {
			name: "台南",
			district: []
		}]
	}];


/***
 * @auth topoadmin
 * @description 中国省市信息联动插件
 * @time 2016-09-25 20:42:54
 * options 参数说明
 * ------------------
 * @param title				String		头部提示
 * @param prov				String 		默认省份
 * @param city				String 		默认城市
 * @param district			String 		默认区/县
 * @param selectNumber		Int			设置省市县可选择个数 1-2,默认为3个都可选
 * @param scrollToCenter	Boolean 	true:已选位置滚动到中央 ,默认为顶部
 * @param autoOpen			Boolean 	自动打开
 * @param customOutput		Function	自定义选择完毕输出，不执行内部填充函数
 * @param selectEnd			Function 	选择完毕回调事件 return {prov,city,district},address
 * ------------------
 * @version 1.0.0
 */
(function(factory) {
	'use strict';
	if(typeof define === 'function' && define.amd) {
		define(['jquery', 'amazeui'], factory);
	} else if(typeof exports !== 'undefined') {
		module.exports = factory(require('jquery'), require('amazeui'));
	} else {
		factory(jQuery, jQuery.AMUI);
	}

}(function($) {
	"use strict";
	
	var addressJson = Sys_Address;
	
	var iScroll;
	if(typeof(IScroll) == "undefined") { // 检测 iScroll 是否存在
		iScroll = $.AMUI.iScroll;
	} else {
		iScroll = IScroll;
	}

	// 构造函数
	function address(elm, options) {
		var self = this;
		self.$element = elm;
		self.options = options;

		// 初始化
		self._ready();

		// 带底部参数
		if(self.options.footer) {
			var $cancelBtn = self.$popup.find('.am-address-cancel');
			var $confirmBtn = self.$popup.find('.am-address-confirm');
			$confirmBtn.on('click', function() {
				self._selectEnd();
				return false;
			});
			$cancelBtn.on('click', function() {
				self.$popup.modal("close");
				return false;
			});
		}

		// 窗口被打开
		self.$popup.on('open.modal.amui', function() {
			self._readyProv();
		});

		// 窗口关闭
		self.$popup.on('close.modal.amui', function() {
			
			// amend by puzd (start)
			
			/*
			var selectProv = self.$prov.find("li.am-active").text();
			if(selectProv != self.options.prov) {
				self.$prov.find("li:contains(" + self.options.prov + ")").addClass("am-active").siblings().removeClass("am-active");
			}
			*/
			
			if (self.options.prov && self.options.prov != '') {
				var selectProv = self.$prov.find("li.am-active").text();
				if (selectProv != self.options.prov) {
					self.$prov.find("li:contains(" + self.options.prov + ")").addClass("am-active").siblings().removeClass("am-active");
				}
			}
			
			// amend by puzd (end)
			
		});

		// 是否自动打开
		if(self.options.autoOpen) {
			self.$popup.modal("open");
		}

		self.$element.on("click", function() {
			self.$popup.modal("open");
		});

		// 给 element 对象绑定选择窗口的id
		self.$element.data("address-popup", self.$popup.attr("id"));
	}

	// 渲染选项列表
	address._readyList = function(json, selectedAddress, showSelAll) {
		
		// amend by puzd (start)
		
		//var index = 0;
		
		var index;
		
		// amend by puzd (end)
		
		var html = [];
		$.each(json, function(i, v) {
			
			// amend by puzd (start)
			
			/*
			// 循环第一个值时，如果没有默认选择值就添加 active
			if(i == 0 && !selectedAddress) {
				html.push('<li class="am-active">' + v.name + '</li>');
				index = i;
				return;
			}
			*/
			
			if (i == 0) {
				if (showSelAll) {
					if (!selectedAddress) {
						html.push('<li class="am-active">以下全选</li>');
						index = -1;
						html.push('<li>' + v.name + '</li>');
						return;
					} else {
						html.push('<li>以下全选</li>');
					}				
				} else {
					if (!selectedAddress) {
						html.push('<li class="am-active">' + v.name + '</li>');
						index = i;
						return;
					}
				}
			}
			
			// amend by puzd (end)
			
			if(selectedAddress && v.name === selectedAddress) {
				html.push('<li class="am-active">' + v.name + '</li>');
				index = i;
			} else {
				html.push('<li>' + v.name + '</li>');
			}
		});
		
		return {
			list: html.join(''),
			index: index
		};
	}

	// 渲染选择窗口
	address.prototype._ready = function() {
		var self = this;
		var html = [];
		html.push('<div class="am-popup am-address-popup" id="' + Math.ceil(Math.random() * (new Date).getTime()) + '">');
		html.push('<div class="am-popup-inner">');
		html.push('<div class="am-popup-hd">');
		html.push('<h4 class="am-popup-title">' + self.options.title + '</h4>');
		html.push('<span data-am-modal-close class="am-close">&times;</span>');
		html.push('</div>');
		html.push('<div class="am-popup-bd">');
		if(self.options.footer) {
			html.push('<div class="am-address-list am-address-list-footer">');
		} else {
			html.push('<div class="am-address-list">');
		}
		html.push('<div class="am-u-sm-4 am-address-prov"><ul class="am-list am-list-static"></div>');
		html.push('<div class="am-u-sm-4 am-address-city"><ul class="am-list am-list-static"></div>');
		html.push('<div class="am-u-sm-4 am-address-district"><ul class="am-list am-list-static"></div>');
		html.push('</div>');

		if(self.options.footer) {
			html.push('<div class="am-modal-footer am-address-footer">');
			html.push('<span class="am-modal-btn am-address-cancel">取消</span>');
			html.push('<span class="am-modal-btn am-address-confirm">确定</span>');
			html.push('</div>');
		}
		html.push('</div>');
		html.push('</div>');
		html.push('</div>');
		self.$popup = $(html.join('')).appendTo('body');

		self.$prov = self.$popup.find('div.am-address-prov');
		self.$city = self.$popup.find('div.am-address-city');
		self.$district = self.$popup.find('div.am-address-district');

		// 初始化数据
		self._readyProv("create");
		
		// 判断是否需要添加市级或县级栏
		if(self.options.selectNumber == 1) {
			self.$popup.find("div.am-address-list>div").removeClass("am-u-sm-4").addClass("am-u-sm-12");
		} else {
			self._readyCity("create");
		}
		if(self.options.selectNumber == 2) {
			self.$popup.find("div.am-address-list>div").removeClass("am-u-sm-4").addClass("am-u-sm-6");
		} else {
			self._readyDistrict("create");
		}
		
	}

	// 函数 _readyxxx 的参数 methods="create" 表示新建 否则负责刷新 IScroll 
	// {渲染,刷新} 省级数据
	address.prototype._readyProv = function(methods) {
		var self = this;
		if(methods == "create") {
			var prov = address._readyList(addressJson, self.options.prov);
			self.$prov.children("ul").html(prov.list).data("address-active", prov.index);
			if(!prov.index) {
				self.$prov.find("li:eq(0)").addClass("am-active").siblings("li").removeClass("am-active");
			}

			self.provIscroll = new iScroll(self.$prov[0], {
				tap: "addressTap",
				mouseWheel: true
			});

			// 点击事件
			self.$prov.on("addressTap", "li", function() {
				var $this = $(this);
				if($this.hasClass("am-active")) {
					return false;
				}
				$this.addClass("am-active").siblings("li").removeClass("am-active");
				if(self.options.selectNumber != 1) {
					self._readyCity(null, "tap");
				} else {
					self._selectEnd();
				}
				self.$element.trigger("provTap", [$this]);
			});
		} else {
			if(self.options.selectNumber != 1) {
				self._readyCity();
			}
			self.provIscroll.refresh();
			self.provIscroll.scrollToElement(self.$prov.find("li.am-active")[0], null, null, self.options.scrollToCenter);
		}
	}

	// {渲染,刷新} 市级数据
	address.prototype._readyCity = function(methods, onEvent) {
		var self = this;
		if(methods == "create") {
			self.cityIscroll = new iScroll(self.$city[0], {
				tap: "addressTap",
				mouseWheel: true
			});
			
			// 点击事件
			self.$city.on("addressTap", "li", function() {
				var $this = $(this);
				
				if($this.hasClass("am-active")) {
					
					// add by puzd (start)
					if (self.options.selectProv) {
						var cityIndex = self.$city.find(".am-active").index();
						cityIndex--;
						if (cityIndex == -1) { // 如果 允许选择城市 且 选中“全省”
							self._selectEnd();
						}
					}
					// add by puzd (end)
					
					return false;
				}
				
				$this.addClass("am-active").siblings("li").removeClass("am-active");
				if(self.options.selectNumber != 2) {
					self._readyDistrict(null, "tap");
				} else {
					self._selectEnd();
				}
				self.$element.trigger("cityTap", [$this]);
			});
		} else {
			
			var provIndex = self.$prov.find(".am-active").index();
			
			var city = address._readyList(addressJson[provIndex].city, self.options.city, self.options.selectProv);
			
			// amend by puzd (start)
			
			/*
			self.$city.children("ul").html(city.list).data("address-active", city.index);
			if(!city.index) { // 没有默认选中项
				self.$city.find("li:eq(0)").addClass("am-active").siblings("li").removeClass("am-active");
			}
			*/
			
			if (typeof(city.index) == 'undefined') {
				if (self.options.selectProv) {
					city.index = -1;
				} else {
					city.index = 0;
				}
			}
			self.$city.children("ul").html(city.list).data("address-active", city.index);
			var tmp = city.index;
			if (self.options.selectProv) tmp ++;
			self.$city.find("li:eq(" + tmp + ")").addClass("am-active").siblings("li").removeClass("am-active");
			
			// amend by puzd (end)
			
			
			// 触发省级 tap 事件时，如果市级只要一个并且是省市级联时，触发选择完毕函数
			if(onEvent === "tap" && addressJson[provIndex].city.length <= 1 && self.options.selectNumber == 2) {
				self._selectEnd();
			}

			self.cityIscroll.refresh();
			self.cityIscroll.scrollToElement(self.$city.find("li.am-active")[0], null, null, self.options.scrollToCenter);

			if(self.options.selectNumber != 2) {
				self._readyDistrict();
			}
		}
	}

	// {渲染,刷新} 县级 数据
	address.prototype._readyDistrict = function(methods, onEvent) {
		var self = this;
		if(methods == "create") {
			self.districtIscroll = new iScroll(self.$district[0], {
				tap: "addressTap",
				mouseWheel: true
			});

			// 点击事件
			self.$district.on("addressTap", "li", function() {
				var $this = $(this);
				$this.addClass("am-active").siblings("li").removeClass("am-active");
				self._selectEnd();
			});
		} else {
			var provIndex = self.$prov.find(".am-active").index();
			
			var cityIndex = self.$city.find(".am-active").index();
			
			// add by puzd (start)
			if (self.options.selectProv)
				cityIndex--;
			if (cityIndex == -1) { // 如果 允许选择城市 且 选中“全省”
				self.$district.children("ul").html("");
				if(onEvent == "tap") {
					self._selectEnd();
				}
				return;
			}
			// add by puzd (end)
			
			var districtJson = addressJson[provIndex].city[cityIndex].district;
			if(districtJson.length <= 0) { // 如果县级数据不存在，设置为空
				self.$district.children("ul").html("");
				if(onEvent == "tap") {
					self._selectEnd();
				}
				return;
			}

			var district = address._readyList(districtJson, self.options.district, self.options.selectCity);
			
			// amend by puzd (start)
			
			/*
			self.$district.children("ul").html(district.list).data("address-active", district.index);
			if(!district.index) {
				self.$district.find("li:eq(0)").addClass("am-active").siblings("li").removeClass("am-active");
			}
			*/
			
			if (typeof(district.index) == 'undefined') {
				if (self.options.selectCity) {
					district.index = -1;
				} else {
					district.index = 0;
				}
			}
			self.$district.children("ul").html(district.list).data("address-active", district.index);
			var tmp = district.index;
			if (self.options.selectCity) tmp ++;
			self.$district.find("li:eq(" + tmp + ")").addClass("am-active").siblings("li").removeClass("am-active");
			
			// amend by puzd (end)
			
			self.districtIscroll.refresh();
			self.districtIscroll.scrollToElement(self.$district.find("li.am-active")[0], null, null, self.options.scrollToCenter);
		}
	}

	// 选择完毕
	address.prototype._selectEnd = function() {
		var self = this;
		// 获取已选省级数据
		var provIndex = self.$prov.find("li.am-active").index();
		var provJson = addressJson[provIndex];

		var sJson = {
			prov: provJson.name,
			city: "",
			district: ""
		}

		if(self.options.selectNumber != 1) { // selectNumber = 1 表示只选省级 
			var cityIndex = self.$city.find("li.am-active").index();
			if (self.options.selectProv)
				cityIndex--;
			if (cityIndex >= 0) {
				var cityJson = addressJson[provIndex].city[cityIndex];
				sJson.city = cityJson.name;
				// 判断是否有 县区 级数据
				if(cityJson.district.length && self.options.selectNumber != 2) { // selectNumber = 2 表示只选省市级
					var districtIndex = self.$district.find("li.am-active").index();
					if (self.options.selectCity)
						districtIndex--;
					if (districtIndex >= 0) {
						var districtJson = addressJson[provIndex].city[cityIndex].district[districtIndex];
						sJson.district = districtJson.name;
					}
				}
			}
		}

		self.options = $.extend(true, self.options, sJson); // 合并参数,下次打开选择窗口时，需读取 options 数据进行定位

		// 自动填充
		if(!self.options.customOutput) {
			self.$element.find("input").val(sJson.prov + sJson.city + sJson.district);
		}

		// 选取完毕回调
		if($.isFunction(self.options.selectEnd)) {
			self.options.selectEnd(sJson, self);
		}
		this.$popup.modal("close");
	}
	
	$.fn.address = function(options) {
		var opt = $.extend(true, {
			title: "请选择地址",
			//prov: "北京",
			//city: "北京市",
			//district: "东城区",
			selectNumber: 0,
			scrollToCenter: false,
			autoOpen: false,
			customOutput: false,
			selectEnd: false,
		}, options);
		new address(this, opt);
		return this;
	}
	
	$.AMUI.ready(function(doc) {
		$('[data-am-address]', doc).each(function() {
			var $this = $(this);
			var options = $.AMUI.utils.parseOptions($this.attr('data-am-address') || $this.data("am-address"));
			$this.address(options);
		});
	});

	return $;
}));