...
<body>
	...
	<div class="view">
		<div id="toolbar" class="btn-group" role="group">
			<c:if test="${not empty AddCoSysDepositOrder }">
		...
		<table id="listView"
		       ...
		       data-url="coSysDepositOrder/coSysDepositOrderListData.json?status=${param.status}"
		       data-id-field="id"
		       data-unique-id="id"
		       data-sort-name="id"
			...
			<thead>
				...
				<th data-field="amount" data-width="15%">预存金额（元）</th>
	...
	$(function() {
			...
			'onClickRow' : function(row, $element, field) {       // 行点击事件
				if ( field != 0 ) {
					 openWin({
				        'title' : '平台预存订单信息详情',
				        'target' : 'menuTab',
				        'url' : 'coSysDepositOrder/coSysDepositOrderRecord.do?id=' + row.id
					 });
				}
			}
		});
			...
			slideUpSearch({
				callback : function() {
					...
					oListView.search({
						...
						'orgOrStoreId' : $('#txt_search_orgOrStoreId').val(),
		...
		$('#btn_add').click(function() {
			openWin({
		        'title' : '新增平台预存单',
		        'target' : 'menuTab',
		        'url' : 'coSysDepositOrder/coSysDepositOrderRecord.do'
		    });
		});
