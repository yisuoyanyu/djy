...
<html class="no-js">
    ...
    <div class="bar h3 tabs_bar posi_fixed border_b">
      <ul class="am-avg-sm-2" style="position:relative;">
         <li>
          <a href="javascript:location.replace('user/mySpreadQrCode.do');" style="color: white;">我的二维码</a></li>
        <li class="on">
          <a href="javascript:void(0);" style="color: white;">推广佣金</a></li>
           </ul>
    </div>
  <div id="box" class="listCstmConsume" style="height: auto;"> 
    
    <ul class="am-list am-margin-xs" id="detaildiv">
    	<c:forEach var="i" items="${spreadCommissions}" varStatus="status">
    	<li name="csli">
		   <div style="margin-left: 0rem;">
		    <div class="am-cf">
		     <span class="am-fl" style="color: black;">${i.consumeOrder.user.wechatUser.nickname}</span>
		     <span class="am-fr red">佣金状态:
             <c:if test="${i.status == 1}">未付款</c:if>
             <c:if test="${i.status == 2}">付款中</c:if>
             <c:if test="${i.status == 3}">付款成功</c:if>
             <c:if test="${i.status == 4}">付款失败</c:if>
             <c:if test="${i.status == 5}">已取消</c:if>
             </span>
		    </div>
		    <div class="am-cf">
		     <span class="am-fl am-text-sm gray"><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${i.insertTime}" /></span>
		     <span class="am-fr am-text-sm gray">佣金金额:￥<fmt:formatNumber value="${i.amount}" pattern="0.00" type="number" /></span>
		    </div>
		   </div>
       </li>
    </c:forEach>
						...
						<div class="pagenav-content">
							<div class="pagenav" id="J_PageNav">
								<div class="p-prev p-gray">
									...
									<c:if test="${lastPage != 0}">
										<a href="spreadCommission/myCommission.do?page=${lastPage}">上一页</a>
									</c:if>
								</div>
								<div class="pagenav-cur" style="vertical-align: bottom">
									...
									<select class="pagenav-select" onchange="location.href=this.options[this.selectedIndex].value;">
										<c:forEach var="i" begin="${startShowPage}" end="${endShowPage}" step="1">
											<c:if test="${i == page}">
												<option selected="selected" value="spreadCommission/myCommission.do?page=${i}">第${i}页</option>
											</c:if>
											<c:if test="${i != page}">
												<option value="spreadCommission/myCommission.do?page=${i}">第${i}页</option>
											</c:if>
										</c:forEach>
									</select>
								</div>
								<div class="p-next">
									...
									<c:if test="${nextPage != 1}">
										<a href="spreadCommission/myCommission.do?page=${nextPage}">下一页</a>
									</c:if>
								</div>
							</div>
						</div>
