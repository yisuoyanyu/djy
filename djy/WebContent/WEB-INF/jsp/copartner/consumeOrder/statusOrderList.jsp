...
<title>
<c:if test="${orderStatus == 1}">未付款订单</c:if>
<c:if test="${orderStatus == 3}">已完成订单</c:if>
<c:if test="${orderStatus == 5}">已取消订单</c:if>
<c:if test="${orderStatus == 6}">已失效订单</c:if>
</title>
...
<body>
    ...
    <div class="bar h3 tabs_bar posi_fixed border_b">
      <ul class="am-avg-sm-4" style="position:relative;">
         <li <c:if test="${orderStatus == 1}">class="on"</c:if>>
          <a href="javascript:location.replace('consumeOrder/getOrderListByStatus.do?orderStatus=1');">待付款</a></li>
        <li <c:if test="${orderStatus == 3}">class="on"</c:if>>
          <a href="javascript:location.replace('consumeOrder/getOrderListByStatus.do?orderStatus=3');">已完成</a></li>
        <li <c:if test="${orderStatus == 5}">class="on"</c:if>>
          <a href="javascript:location.replace('consumeOrder/getOrderListByStatus.do?orderStatus=5');">已取消</a></li>
          <li <c:if test="${orderStatus == 6}">class="on"</c:if>>
          <a href="javascript:location.replace('consumeOrder/getOrderListByStatus.do?orderStatus=6');">已失效</a></li>
      </ul>
    </div>
    <!-- 信息列表 -->
